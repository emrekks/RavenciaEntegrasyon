...
**v10.20/v10.78 kullanıcı akışı notu:** Güvenlik ekranı mevcut backend MFA ve server-side session sözleşmelerine bağlanmıştır; Authenticator kurulumu parola ile yeniden doğrulama, QR/kod onayı ve tek gösterimli kurtarma kodlarıyla tamamlanır, diğer oturumlar tekil veya toplu sonlandırılabilir. Ürün ekleme ve düzenleme aynı çalışma alanı hiyerarşisini kullanır: temel bilgiler, kategori özellikleri, varyant stok/fiyat, ölçü/desi, görsel ve yayın alanları birlikte sunulur. Desi nullable, geriye uyumlu modelde saklanır; eski katalog verileri değiştirilmez. Sipariş ürün adedi rozeti ürün kartının sağ üstündedir.
...
Bu kapanış `storeFrontCode=TR` ve ürün payload'ında `channels=["CORE"]` ile Türkiye CORE mağaza kapsamıdır. Production Türkiye V2 claims GET tek canonical çağrıyı kullanır. Yalnız Stage filtresiz çağrısı `order.not.found` 404 verirse aynı resmî salt-okunur endpoint resmî claim durumlarıyla ayrı sorgulanır; yazma yolları bu fallback kapsamına girmez. `LUXE`, uluslararası storefront veya farklı kanal kimlikleri ayrı ADR, capability evidence, fixture ve kabul testi olmadan etkin değildir. Trendyol E-Faturam mali sağlayıcı kapanışı F4'tür ve F3 CORE kod kapanışına dahil değildir.
