...
| Hedef Windows VPS | F0 runtime/volume/backup çıkışı | Kullanıcı VPS'in daha sonra kiralanacağını bildirdi; erişim ve özellikler henüz yok | Kiralama sonrası runbook; öncesinde production onayı yok | Evet, F0 çıkışı |
...
| Hedef volume, restore süresi ve RTO | DR kanıtı | `PILOT_LOCAL` ve en fazla 6 saat pilot RPO tanımlı; hedef restore çalıştırılmadı | `RISK-DR-001` açık; off-host yalnız resilient profilde zorunlu | Evet, F0 çıkışı |
