...
| Hedef AWS Ubuntu Server 26.04 LTS | F0 runtime/volume/backup çıkışı | Kullanıcı mevcut x86_64, 2 vCPU, 8 GB sınıfı RAM ve 80 GB NVMe hostunu v3.4 hedefi olarak onayladı; SSH ve kapasite ölçümü geçti | Docker/Compose, reboot/volume/restore ve x5 kapasite kanıtı `ubuntu-server-runtime-validation.md` ile tamamlanır | Hayır, yerel geliştirme için; evet, hedef production/F0 çıkışı için |
