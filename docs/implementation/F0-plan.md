...
Kullanıcı 2026-07-31 tarihinde yerel-makine-önce akışını, 2026-08-02 tarihinde ise mevcut AWS hostunun Ubuntu Server 26.04 LTS, 2 vCPU, 8 GB sınıfı RAM ve 80 GB NVMe sınıfıyla bağlayıcı hedef olmasını onayladı. SSH üzerinden OS/mimari/CPU/RAM/disk ve repository ölçümü; ardından Docker Engine/Compose, immutable production image, DNS/TLS, gerçek reboot ve hedef restore kanıtı tamamlandı. x5 kapasite, yönetici login smoke ve şifreli off-host backup kanıtı üretilmeden tam production onayı verilmez.
...
| `F0-REQ-006` | F0 Teslimatlar; Yedekleme Profili Kararı; Tablo 48 | Gerçek hacim ve pik x5 profilinin yanı sıra RPO/RTO, `BACKUP_PROFILE` ve varsa off-host hedef kayıtlıdır. | `docs/implementation/F0-capacity-recovery-profile.md`; risk ve restore kanıtı | Hedef restore-RTO tamamlandı; x5 yük ve şifreli off-host kopya bekliyor | BLOCKED_EXTERNAL |
...
| `BLOCK-HOST-001` | Dış ortam | AWS host profili, SSH, Docker Engine/systemd, exact Compose, immutable production image, reboot/named-volume, domain/DNS/TLS ve hedef restore doğrulandı; x5 kapasite kanıtı henüz tamamlanmadı. | `F0-REQ-009` ve `F0-EXIT-003` kapasite yönünden kısmi kalır. | Hedef runbook'un x5 yük adımını tamamla; 2 vCPU/80 GB nedeniyle performans ve disk doluluğunu ölç. |
...
- **F0 çıkış kapısı: BLOCKED_EXTERNAL.** `BLOCK-HOST-001` host/runtime/reboot/volume, immutable image, DNS/TLS ve hedef restore bölümleri kapanmıştır; x5 yük/kapasite, yönetici login smoke ve şifreli off-host backup kanıtı bekler. Bunlar yerel geliştirmeyi durdurmaz; tam production kabul kapısını korur. `BLOCK-CAPACITY-001` ve `BLOCK-VERSION-001` kapanmıştır.
...
Yerel Windows/WSL/Docker/volume/restore ön doğrulaması tarihsel olarak tamamlanmıştır. Mevcut AWS Ubuntu 26.04 hostunda SSH, kapasite, Docker/Compose, reboot ve named-volume checksum kanıtları tamamlanmıştır; `docs/runbooks/ubuntu-server-runtime-validation.md` üzerindeki production image, restore, DNS/TLS ve kapasite/yük kanıtları sıradaki iştir. v3.4 değişikliği yalnız dağıtım hedefini yeniler; faz kapılarını veya dış platform write izinlerini açmaz.
