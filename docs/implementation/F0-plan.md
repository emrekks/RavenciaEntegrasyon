...
| `BLOCK-HOST-001` | Dış ortam | AWS host profili, SSH, Docker Engine/systemd, exact Compose, immutable production image, reboot/named-volume, domain/DNS/TLS ve hedef restore doğrulandı. x5 ek ölçümü işletme sahibi tarafından 2026-08-03 tarihinde feragatle kapatıldı. | Ölçüm sonucu yoktur; kapasite riski işletme tarafından kabul edilmiştir. | `ACCEPTED_OWNER_WAIVER`; yalnız gerçek gözlemde sorun görülürse host/volume büyütme kararı açılır. |
...
- **F0 çıkış kapısı: BLOCKED_EXTERNAL.** `BLOCK-HOST-001` host/runtime/reboot/volume, immutable image, DNS/TLS ve hedef restore bölümleri kapanmıştır; x5 ek ölçümü `ACCEPTED_OWNER_WAIVER / NOT_MEASURED` olarak kapatılmıştır. Şifreli off-host backup kanıtı hâlâ bekler. Bu yerel geliştirmeyi durdurmaz; tam production kabul kapısını korur. `BLOCK-CAPACITY-001` ve `BLOCK-VERSION-001` kapanmıştır.
