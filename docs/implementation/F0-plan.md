...
| `F0-REQ-006` | F0 Teslimatlar; Yedekleme Profili Kararı; Tablo 48 | Gerçek hacim ve pik x5 profilinin yanı sıra RPO/RTO, `BACKUP_PROFILE` ve varsa off-host hedef kayıtlıdır. | `docs/implementation/F0-capacity-recovery-profile.md`; risk ve restore kanıtı | Hacim baz/x5 ve hedef volume kalıcılığı tamamlandı; hedef restore-RTO bekliyor | BLOCKED_EXTERNAL |
...
| `BLOCK-HOST-001` | Dış ortam | AWS host profili, SSH, Docker Engine/systemd, exact Compose ve reboot/named-volume checksum doğrulandı; domain/DNS, restore, production image ve x5 kapasite kanıtı henüz tamamlanmadı. | `F0-REQ-009` ve `F0-EXIT-003` kısmi kalır. | Hedef runbook'un restore/yük/TLS adımlarını tamamla; 2 vCPU/80 GB nedeniyle performans ve disk doluluğunu ölç. |
...
- **F0 çıkış kapısı: BLOCKED_EXTERNAL.** `BLOCK-HOST-001` host/runtime/reboot/volume bölümü kapanmış, fakat yük/DNS/TLS/production image adımları ve buna bağlı hedef `BLOCK-DR-001` restore kanıtı tamamlanmamıştır. Bunlar yerel geliştirmeyi durdurmaz; production kabul kapısını korur. `BLOCK-CAPACITY-001` ve `BLOCK-VERSION-001` kapanmıştır.
