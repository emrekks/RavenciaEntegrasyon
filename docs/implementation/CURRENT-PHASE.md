...
Capability `UNKNOWN` kayıtları kanıtsız biçimde `SUPPORTED` yapılmadı. Owner/Admin tarafından başlatılan Trendyol `STAGE` etiket canary'si, paket `92257909` / takip `7250000170335942` üzerinde gerçek ortak etiket read-back'iyle başarılı oldu: `LABEL_READ=SUPPORTED`, resmi kaynak URL'si, Stage/store kapsamı, format kısıtı, audit kaydı ve 64 karakterlik SHA-256 fixture kanıtı saklandı. `LABEL_WRITE` için hem eski yerel `ReadyToShip` paket hem de Trendyol belgesindeki seller `2738`, sipariş `1238522676`, takip `7260000167037306` `Picking` fixture'ı gerçek `REMOTE_REQUEST_REJECTED` ile engellendi; dokümanda belirtilen tarihsel tracking fixture'ının create endpointi için artık geçerli olmadığı kanıtlandı. `LABEL_WRITE` `UNKNOWN` kaldı. Sonraki dar adım, Trendyol'ün resmi Stage Test Order API'si ile taze test siparişi oluşturup dönen güncel takip numarasıyla create → read-back kanıtını üretmektir. Normal production/external-write anahtarları değişmedi.
