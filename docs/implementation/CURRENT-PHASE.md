...
Takip eden sözleşme testi, yalnız katalog alanları güncellendiğinde de boş `variantsToCreate` dizisinin açıkça gönderildiğini doğrular; böylece istemci ve append-only API sözleşmesi birlikte korunur. Tam web Vitest kümesi `27/27 PASS` ve Vite production build `PASS`.
