...
| F3 | `ACTIVE_CLOSURE` | Trendyol read/reference omurgası ve güvenli webhook/scheduler sertleştirmesi var; ürün yayınlama orkestrasyonu, create/update ayrımı, birleşik stok-fiyat ve Stage safe-write eksik. |
...
1. v7 değişikliklerini exact .NET `10.0.302`, Node `24.18.1`, npm `11.12.1` ve Docker Compose ortamında restore/build/test/typecheck/smoke ile doğrula.
2. F3 ürün yayınlama application akışını durable job, batch polling ve satır sonucu kaydıyla tamamla.
3. `UpsertAsync` yerine create/update/archive sözleşmelerini ayır.
4. Trendyol stok ve fiyatı tek `price-and-inventory` komutu olarak uygula.
5. Sipariş/paket/iade read akışlarını gerçek Stage fixture ve idempotency testleriyle kapat.
6. F4 taxpayer query, invoice status polling ve cancellation akışlarını ekle.
7. E-Faturam güvenli PDF indiricisini gerçek izinli host ve Stage PDF ile doğrula.
8. Trendyol fatura linki için resmî teyit/read-back imkânı doğrulanırsa `SUBMITTED -> CONFIRMED` reconciliation ekle; aksi halde manuel operasyon prosedürünü kanıtla.
9. Stage E2E, backup/restore ve production read-only smoke kanıtlarını kaydet.
10. F3 ve F4 kapandıktan sonra F5 production pilotunu, F6 stabilizasyonu ve F7 adapter registry hazırlığını yürüt.
11. Yeni platformu ancak F7 çıkış kapısından sonra ayrı ADR ile aç.
