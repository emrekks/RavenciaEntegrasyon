...
`release-2026-08-17-v10.65` source CI `32031018630` ve immutable publish `32031420205` ile geçti; checksum/restore-list doğrulanmış `20260817T125012Z` backup sonrasında app `sha256:e14f6a9284ef58bc0563d8b4b18b67aa2b6c85f8620fb853117cd8c5d3eb7c3e`, edge `sha256:0c6b72ade8b133a7cdfec57b5fc0079f808672cbe9658728457ead08b5d7e5aa` deploy edildi. API/Worker/Caddy/PostgreSQL healthy ve readiness geçti. Güncel bağlantı testi başarılıdır. Mali doğrulaması geçen eski Stage taslağının güncel kodla tekrarı, taze `signIn` sonrasında provider korumalı create endpointinde `EFATURAM_ACCESS_TOKEN_REJECTED / 401` ile durdu ve provider `x-request-id` vermedi. Bu nedenle hesap girişini hata saymak yerine provider problem yanıtındaki yalnız allowlist `/problem/...` `instance` referansını güvenli teşhis alanına alma eklendi; query, serbest metin, ham gövde ve secret saklanmaz. Linux'un kökten başlayan yolu `file:` URI sayması ayrıca kapatıldı. Adapter contract `37/37` PASS; bu yeni teşhisin release/deploy ve provider tekrarı `NOT_RUN`.
