...
| Ortak etiket taşıyıcı uygunluğu | CODED_STATIC_VALIDATED | 2026-08-12: Son taze Stage fixture metaverisi `Yurtiçi Kargo Marketplace` taşıyıcısını gösterdi. Trendyol ortak etiket sözleşmesi yalnız Trendyol öder `Aras Kargo` veya `TEX` gönderilerini kabul ettiğinden, `LABEL_WRITE` canary ve normal ortak etiket kuyruğu artık uyumsuz taşıyıcı için uzak çağrı yapmadan `COMMON_LABEL_CARRIER_UNSUPPORTED` ile fail-closed durur. Eski provider reddi kanıt değildir; LABEL_WRITE ve SHIPMENT_WRITE `UNKNOWN` kalır. Infrastructure derlemesi geçti; uygun Stage fixture kabulü beklenir. |
