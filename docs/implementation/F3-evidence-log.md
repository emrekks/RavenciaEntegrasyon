...
| İade alıcı API fallback'i | PASS_TARGETED | 2026-08-12: Trendyol Stage response'unda üst seviye alıcı alanları `Adı Soyadı` placeholder olduğunda aynı order snapshot'ındaki invoice address, o yoksa shipment address ad/soyadı kullanılır. Gerçek üst seviye ad önceliğini korur; değer uydurulmaz. `F3ReturnCustomerNameTests` 3/3 ve web TypeScript `PASS`. Normal panel sync işi `3cb60e4e9e674ed39e1b091020af6ad1` ilk denemede `SUCCEEDED`; immutable deploy sonrası kullanıcı görünümü yeniden doğrulanacak. |
