...
- **Live re-acceptance:** At the scheduled `2026-08-17 17:25:49 UTC` read-back, the durable job made attempt `18` and again received `PRODUCT_APPROVAL_PENDING`. It is `RETRY_SCHEDULED` for `2026-08-17 17:30:52 UTC`, confirming the corrected approximately five-minute interval. No duplicate product-create request was submitted to accelerate that check.

## 2026-08-17 - Stage product approval reconciliation follow-up

- **Read-only runtime check:** At `2026-08-17 17:26 UTC`, the existing `TRENDYOL_PRODUCT_APPROVAL_RECONCILE` job remained `RETRY_SCHEDULED / PRODUCT_APPROVAL_PENDING` after attempt `18`.
- **Next action:** The durable worker has scheduled the next provider read-back for `2026-08-17 17:30:52 UTC`. No duplicate product-create request, manual state promotion, or provider write was performed.
...
| v10.49 canlı Stage UI smoke | PASS_STAGE | 2026-08-12: Immutable v10.49 deployment sonrasında normal panelde Trendyol Stage bağlantısı `Stage işlemleri / Hazır` ve Stage salt-okunur sync görünürlüğüyle açıldı; teknik capability listesi normal kullanıcı yüzeyinde görünmüyor. API/Worker/Caddy health ve `/health/ready` 200 geçti. |
| İade alıcı API fallback'i | PASS_DEPLOYED / UI_SMOKE_NOT_RUN | 2026-08-12: Trendyol Stage response'unda üst seviye alıcı alanları `Adı Soyadı` placeholder olduğunda aynı order snapshot'ındaki invoice address, o yoksa shipment address ad/soyadı kullanılır. Gerçek üst seviye ad önceliğini korur; değer uydurulmaz. `F3ReturnCustomerNameTests` 3/3 ve web TypeScript `PASS`. Normal panel sync işi `3cb60e4e9e674ed39e1b091020af6ad1` ilk denemede `SUCCEEDED`. v10.50 immutable app/edge digestleri checksumlı `20260812T093713Z` backup sonrası deploy edildi; API/Worker/Caddy/PostgreSQL healthy, iç/dış readiness `200`. Canlı aggregate kontrolü 26/26 iadenin bağlı sipariş, müşteri snapshot ve en az bir adres adı snapshot'ı bulunduğunu doğruladı; PII okunmadı. Girişli iade tablosu görsel smoke oturum yokluğu nedeniyle `NOT_RUN`. |
