...
| `F1-EV-022` AWS Ubuntu host/runtime (`2026-08-02`) | HOST_RUNTIME_PASS / OPERATIONS_PENDING | v3.4 PDF 77 sayfa ve SHA-256 `5A652A...0EC51`; gerçek host Ubuntu 26.04 LTS x86_64, 2 vCPU, 8.153.141.248 byte RAM, 80.530.636.800 byte NVMe aygıt ve 76.878.503.936 byte root filesystem. Docker Engine/CLI `29.7.1`, containerd `2.2.6`, Buildx `0.36.0`, systemd enabled/active ve exact Compose `2.40.2` checksum kapısı `--host-only` ile geçti. Reboot/volume/restore, x5 yük, DNS/TLS ve production image kanıtı bekliyor. |
...
F1 yerel uygulama, public-TLS production edge tanımı, immutable image yayın otomasyonu ve fail-closed Ubuntu Server kurulum scriptleri sonucu `READY_LOCAL_DEPLOY_PREPARED`dır. AWS Ubuntu host profili ve SSH erişimi doğrulanmıştır. Docker/Compose, reboot/volume/RTO, workflow'dan üretilecek registry-pushed immutable app/edge digest, production PFX secret ve off-host hedef kanıtı tamamlanana kadar production çıkışı `BLOCKED_EXTERNAL`dır.
