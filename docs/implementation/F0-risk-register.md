...
| `RISK-PUBLISH-001` | P1 | Ürün create ve onay uzlaştırması dinamik/Stage kanıtı olmadan tamamlanmış sayılabilir | Yanlış canlılık yorumu, görünmeyen satır hatası veya kimlik çatışması | Durable create job + batch poll + approved/unapproved read-back + fail-closed identity link kodlandı; exact runtime, operatör UI ve Stage kanıtı zorunlu | PARTIALLY_CLOSED_DYNAMIC_STAGE_UI_PENDING |
