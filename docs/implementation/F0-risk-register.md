...
| `RISK-PUBLISH-001` | P1 | Ürün create adapterı application job/endpoint/UI akışına bağlı değil | Panelden gerçek yayın yapılamaz | Publication aggregate + job + batch poll + hata ekranı | OPEN_IMPLEMENTATION |
| `RISK-UPsert-001` | P1 | `UpsertAsync` her durumda create endpoint’ine POST ediyor | Güncelleme isteği duplicate/create olabilir | Create ve update komutlarını ayır; archive ayrı capability | OPEN_IMPLEMENTATION |
