...
| Trendyol | Katalog referansı | SUPPORTED | Product V2 kategori ağacı hedefte doğrulandı. `CATEGORIES` snapshot ve leaf kategori mapping'i hedefte hazırdır; resmî `BRANDS` sayfalama/snapshot/mapping hattı `release-2026-08-04-9` CI'da geçti fakat production'a dağıtılıp gerçek Stage verisiyle henüz çalıştırılmadı. Kategoriye bağlı özellik eşitlemesi açılmaz | 2026-08-04 production paneli Stage testi; job `019fc9c1-b405-759a-bf91-adbdec8c42ff`, `ReferenceRead=SUPPORTED`; kategori release 4-5, marka release 9 CI kanıtı | off |
