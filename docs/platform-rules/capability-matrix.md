...
| Trendyol | Katalog referansı | SUPPORTED | Product V2 kategori ağacı hedefte doğrulandı. `CATEGORIES` ve `BRANDS` snapshot/mapping hatlarına ek olarak resmî `CATEGORY_ATTRIBUTES` ile `ATTRIBUTE_VALUES` kaynakları parent kapsamlı snapshot, mapping ve yayınlama kapısına bağlandı. Özellik/değer hattının gerçek Stage çalıştırması production dağıtımı sonrasını bekler | 2026-08-04 production paneli Stage testi; job `019fc9c1-b405-759a-bf91-adbdec8c42ff`, `ReferenceRead=SUPPORTED`; kategori release 4-5, marka release 9, özellik/değer release 11 adayı yerel kanıtı | off |
