    ...
    SafeRemoteDocumentDownloader documents) : IInvoiceProviderPort
{
    ...
    {
        using var request = new HttpRequestMessage(HttpMethod.Post, new Uri(context.BaseAddress, TrendyolEFaturamEndpoints.CustomerSignIn)) { Content = JsonContent.Create(new { email = context.Credential.CustomerEmail, password = context.Credential.CustomerPassword, taxId = context.Credential.CustomerTaxId }) };
        request.Headers.Accept.ParseAdd("application/json");
        ...
        try
        {
            ...
            if (!TrendyolEFaturamCustomerAccess.TryRead(body, out var access)) return AdapterResult<TrendyolEFaturamAccessContext>.Failure(new(AdapterErrorClass.ContractViolation, "EFATURAM_CUSTOMER_SIGNIN_CONTRACT_INVALID", "E-Faturam müşteri oturumu gerekli firma/kullanıcı kapsamını döndürmedi.", null, null, remoteId), rate);
