    ...
    : IConnectionPort, IReferenceDataPort, IProductPort, IProductVisualLookupPort, IInventoryPricePort, IOrderPort, IReturnPort
{
    ...
    public async Task<AdapterResult<AdapterPageResult<RemoteOrder>>> PollAsync(AdapterContext context, OrderPollWindow window, AdapterPageRequest page, CancellationToken cancellationToken)
    {
        ...
        const string gql = "query($first:Int!, $after:String, $query:String) { orders(first:$first, after:$after, query:$query, sortKey:UPDATED_AT, reverse:false) { edges { cursor node { id name createdAt updatedAt cancelledAt email currencyCode displayFinancialStatus displayFulfillmentStatus currentTotalPriceSet { shopMoney { amount currencyCode } } totalDiscountsSet { shopMoney { amount currencyCode } } customer { id displayName email } shippingAddress { address1 address2 city province zip country phone name } billingAddress { address1 address2 city province zip country phone name } lineItems(first:250) { nodes { id name sku quantity currentQuantity originalUnitPriceSet { shopMoney { amount currencyCode } } variant { sku barcode } } } fulfillments(first:50) { id status createdAt trackingInfo { number company url } lineItems(first:250) { nodes { id quantity lineItem { id } } } } refunds(first:100) { nodes { id createdAt totalRefundedSet { shopMoney { amount currencyCode } } } } } } pageInfo { hasNextPage endCursor } } }";
    ...
    public async Task<AdapterResult<RemoteOrder>> GetAsync(AdapterContext context, string externalOrderId, CancellationToken cancellationToken)
    {
        ...
        const string gql = "query($id:ID!) { order(id:$id) { id name createdAt updatedAt cancelledAt email currencyCode displayFinancialStatus displayFulfillmentStatus currentTotalPriceSet { shopMoney { amount currencyCode } } totalDiscountsSet { shopMoney { amount currencyCode } } customer { id displayName email } shippingAddress { address1 address2 city province zip country phone name } billingAddress { address1 address2 city province zip country phone name } lineItems(first:250) { nodes { id name sku quantity currentQuantity originalUnitPriceSet { shopMoney { amount currencyCode } } variant { sku barcode } } } fulfillments(first:50) { id status createdAt trackingInfo { number company url } lineItems(first:250) { nodes { id quantity lineItem { id } } } } refunds(first:100) { nodes { id createdAt totalRefundedSet { shopMoney { amount currencyCode } } } } } }";
    ...
    private static RemoteOrder MapOrder(JsonElement order)
    {
        ...
        foreach (var fulfillment in fulfillments)
        {
            ...
            var allocations = fulfillment.GetProperty("lineItems").GetProperty("nodes").EnumerateArray().Select(item =>
            {
        ...
        var refunds = order.TryGetProperty("refunds", out var refundsElement) && refundsElement.ValueKind == JsonValueKind.Object && refundsElement.TryGetProperty("nodes", out var refundNodes) && refundNodes.ValueKind == JsonValueKind.Array
            ? refundNodes.EnumerateArray().Select(refund => new RemoteOrderRefund(
        ...
        return new(
            ...
            DecimalOrNull(discount, "amount") ?? 0,
            DecimalOrNull(money, "amount") ?? 0,
            JsonSerializer.Serialize(new { id = customer.ValueKind == JsonValueKind.Null ? null : customer.GetProperty("id").GetString(), name = customer.ValueKind == JsonValueKind.Null ? null : customer.GetProperty("displayName").GetString(), email = order.GetProperty("email").ValueKind == JsonValueKind.Null ? null : order.GetProperty("email").GetString() }),
