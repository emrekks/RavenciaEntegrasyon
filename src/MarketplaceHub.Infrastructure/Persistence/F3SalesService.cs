...
public sealed class F3SalesService(AppDbContext db, CursorCodec cursors, IConfiguration configuration, IReturnPort returns, TimeProvider timeProvider) : IF3SalesService
{
    ...
    public async Task<ServiceResult<Guid>> EnqueueShipmentActionAsync(Guid tenantId, Guid packageId, long expectedVersion, ShipmentActionCommand command, string idempotencyKey, string correlationId, CancellationToken cancellationToken)
    {
        ...
        var actions = await CapabilityValues(tenantId, package.ConnectionId, F3Capabilities.ShipmentWrite, "allowedActions", cancellationToken); if (!stage && !actions.Contains(action, StringComparer.Ordinal)) return ServiceResult<Guid>.Fail("CAPABILITY_UNKNOWN", "Bu shipment aksiyonu için Stage/SIT capability kanıtı yok.", 422); if (!stage && !await WritesEnabled(tenantId, package.ConnectionId, cancellationToken)) return ServiceResult<Guid>.Fail("EXTERNAL_WRITES_DISABLED", "Global veya connection dış yazma anahtarı kapalı.", 422);
    ...
    public async Task<ServiceResult<IReadOnlyList<ReturnIssueReason>>> ReturnIssueReasonsAsync(Guid tenantId, Guid id, string correlationId, CancellationToken cancellationToken)
    {
        ...
        if (!await IsStageConnection(tenantId, claim.ConnectionId, cancellationToken) && !await Supported(tenantId, claim.ConnectionId, F3Capabilities.ReturnRead, cancellationToken) && !await Supported(tenantId, claim.ConnectionId, F3Capabilities.ReturnWrite, cancellationToken))
    ...
    public async Task<ServiceResult<Guid>> EnqueueReturnActionAsync(Guid tenantId, Guid userId, Guid claimId, long expectedVersion, ReturnDecisionCommand command, string idempotencyKey, string correlationId, CancellationToken cancellationToken)
    {
        ...
        var actions = await CapabilityValues(tenantId, claim.ConnectionId, F3Capabilities.ReturnWrite, "allowedActions", cancellationToken); if (!stage && !actions.Contains(action, StringComparer.Ordinal)) return ServiceResult<Guid>.Fail("CAPABILITY_UNKNOWN", "Bu return aksiyonu için Stage/SIT capability kanıtı yok.", 422); if (!stage && !await WritesEnabled(tenantId, claim.ConnectionId, cancellationToken)) return ServiceResult<Guid>.Fail("EXTERNAL_WRITES_DISABLED", "Global veya connection dış yazma anahtarı kapalı.", 422);
    ...
    {
        var connection = await db.PlatformConnections.AsNoTracking().SingleOrDefaultAsync(x => x.TenantId == tenantId && x.Id == connectionId && (x.PlatformCode == "TRENDYOL" && x.Status == "ACTIVE"), cancellationToken); if (connection is null) return ServiceResult<Guid>.Fail("ACTIVE_CONNECTION_REQUIRED", "Aktif Trendyol bağlantısı gerekir.", 422); if (!IntegrationRuntimePolicy.AllowsManualRead(connection, await Supported(tenantId, connectionId, capability, cancellationToken))) return ServiceResult<Guid>.Fail("CAPABILITY_UNKNOWN", "Read capability Stage/SIT kanıtı olmadan sync işi oluşturulmaz.", 422); return await Enqueue(tenantId, connectionId, type, $"{type.ToLowerInvariant()}:{connectionId}:{Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(payload)))}", payload, correlationId, cancellationToken);
    }
