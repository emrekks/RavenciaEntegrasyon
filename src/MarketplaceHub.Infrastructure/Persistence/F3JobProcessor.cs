...
public sealed class F3JobProcessor(AppDbContext db, IConnectionPort connections, IOrderPort orders, IReturnPort returns, ShopifyGraphQlClient shopify, TimeProvider timeProvider, HepsiburadaAdapter? hepsiburada = null) : IF3JobProcessor
{
    ...
    private async Task<IOrderPort> OrderPort(Guid tenantId, Guid connectionId, CancellationToken cancellationToken) => await db.PlatformConnections.AsNoTracking().Where(x => x.TenantId == tenantId && x.Id == connectionId).Select(x => x.PlatformCode).SingleAsync(cancellationToken) == ShopifyContract.PlatformCode ? shopify : orders;
