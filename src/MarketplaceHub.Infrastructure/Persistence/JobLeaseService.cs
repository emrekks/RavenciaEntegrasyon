...
public sealed class JobLeaseService(AppDbContext db, TokenHasher hasher, TimeProvider timeProvider) : IJobLeaseService
{
    public async Task<LeasedJob?> TryLeaseAsync(TimeSpan leaseDuration, int? maximumPriority, int? minimumPriority, CancellationToken cancellationToken)
    {
        ...
        {
            (null, null) => await db.IntegrationJobs.FromSqlInterpolated($"SELECT * FROM integration.jobs WHERE \"Status\" IN ('PENDING', 'RETRY_SCHEDULED') AND \"AttemptCount\" < \"MaxAttempts\" AND \"AvailableAt\" <= {now} ORDER BY CASE WHEN \"JobType\" = 'TRENDYOL_ORDER_SYNC' THEN 0 ELSE 1 END ASC, \"Priority\" ASC, \"AvailableAt\" ASC, \"CreatedAt\" ASC FOR UPDATE SKIP LOCKED LIMIT 1").SingleOrDefaultAsync(cancellationToken),
            (null, not null) => await db.IntegrationJobs.FromSqlInterpolated($"SELECT * FROM integration.jobs WHERE \"Status\" IN ('PENDING', 'RETRY_SCHEDULED') AND \"AttemptCount\" < \"MaxAttempts\" AND \"AvailableAt\" <= {now} AND \"Priority\" <= {maximumPriority!.Value} ORDER BY CASE WHEN \"JobType\" = 'TRENDYOL_ORDER_SYNC' THEN 0 ELSE 1 END ASC, \"Priority\" ASC, \"AvailableAt\" ASC, \"CreatedAt\" ASC FOR UPDATE SKIP LOCKED LIMIT 1").SingleOrDefaultAsync(cancellationToken),
            (not null, null) => await db.IntegrationJobs.FromSqlInterpolated($"SELECT * FROM integration.jobs WHERE \"Status\" IN ('PENDING', 'RETRY_SCHEDULED') AND \"AttemptCount\" < \"MaxAttempts\" AND \"AvailableAt\" <= {now} AND \"Priority\" >= {minimumPriority!.Value} ORDER BY CASE WHEN \"JobType\" = 'TRENDYOL_ORDER_SYNC' THEN 0 ELSE 1 END ASC, \"Priority\" ASC, \"AvailableAt\" ASC, \"CreatedAt\" ASC FOR UPDATE SKIP LOCKED LIMIT 1").SingleOrDefaultAsync(cancellationToken),
            _ => await db.IntegrationJobs.FromSqlInterpolated($"SELECT * FROM integration.jobs WHERE \"Status\" IN ('PENDING', 'RETRY_SCHEDULED') AND \"AttemptCount\" < \"MaxAttempts\" AND \"AvailableAt\" <= {now} AND \"Priority\" BETWEEN {minimumPriority!.Value} AND {maximumPriority!.Value} ORDER BY CASE WHEN \"JobType\" = 'TRENDYOL_ORDER_SYNC' THEN 0 ELSE 1 END ASC, \"Priority\" ASC, \"AvailableAt\" ASC, \"CreatedAt\" ASC FOR UPDATE SKIP LOCKED LIMIT 1").SingleOrDefaultAsync(cancellationToken)
        };
