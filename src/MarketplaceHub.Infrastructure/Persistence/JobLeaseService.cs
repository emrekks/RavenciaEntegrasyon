...
{
    public async Task<LeasedJob?> TryLeaseAsync(TimeSpan leaseDuration, int? maximumPriority, int? minimumPriority, CancellationToken cancellationToken, string? onlyJobType = null, Guid? onlyJobId = null)
    {
        ...
        {
            (null, null) => await db.IntegrationJobs.FromSqlInterpolated($"SELECT * FROM integration.jobs WHERE \"Status\" IN ('PENDING', 'RETRY_SCHEDULED') AND \"AttemptCount\" < \"MaxAttempts\" AND \"AvailableAt\" <= {now} AND ({onlyJobType} IS NULL OR \"JobType\" = {onlyJobType}) AND ({onlyJobId} IS NULL OR \"Id\" = {onlyJobId}) ORDER BY CASE WHEN \"JobType\" = 'TRENDYOL_ORDER_SYNC' THEN 0 ELSE 1 END ASC, \"Priority\" ASC, \"AvailableAt\" ASC, \"CreatedAt\" ASC FOR UPDATE SKIP LOCKED LIMIT 1").SingleOrDefaultAsync(cancellationToken),
            (null, not null) => await db.IntegrationJobs.FromSqlInterpolated($"SELECT * FROM integration.jobs WHERE \"Status\" IN ('PENDING', 'RETRY_SCHEDULED') AND \"AttemptCount\" < \"MaxAttempts\" AND \"AvailableAt\" <= {now} AND \"Priority\" <= {maximumPriority!.Value} AND ({onlyJobType} IS NULL OR \"JobType\" = {onlyJobType}) AND ({onlyJobId} IS NULL OR \"Id\" = {onlyJobId}) ORDER BY CASE WHEN \"JobType\" = 'TRENDYOL_ORDER_SYNC' THEN 0 ELSE 1 END ASC, \"Priority\" ASC, \"AvailableAt\" ASC, \"CreatedAt\" ASC FOR UPDATE SKIP LOCKED LIMIT 1").SingleOrDefaultAsync(cancellationToken),
            (not null, null) => await db.IntegrationJobs.FromSqlInterpolated($"SELECT * FROM integration.jobs WHERE \"Status\" IN ('PENDING', 'RETRY_SCHEDULED') AND \"AttemptCount\" < \"MaxAttempts\" AND \"AvailableAt\" <= {now} AND \"Priority\" >= {minimumPriority!.Value} AND ({onlyJobType} IS NULL OR \"JobType\" = {onlyJobType}) AND ({onlyJobId} IS NULL OR \"Id\" = {onlyJobId}) ORDER BY CASE WHEN \"JobType\" = 'TRENDYOL_ORDER_SYNC' THEN 0 ELSE 1 END ASC, \"Priority\" ASC, \"AvailableAt\" ASC, \"CreatedAt\" ASC FOR UPDATE SKIP LOCKED LIMIT 1").SingleOrDefaultAsync(cancellationToken),
            _ => await db.IntegrationJobs.FromSqlInterpolated($"SELECT * FROM integration.jobs WHERE \"Status\" IN ('PENDING', 'RETRY_SCHEDULED') AND \"AttemptCount\" < \"MaxAttempts\" AND \"AvailableAt\" <= {now} AND \"Priority\" BETWEEN {minimumPriority!.Value} AND {maximumPriority!.Value} AND ({onlyJobType} IS NULL OR \"JobType\" = {onlyJobType}) AND ({onlyJobId} IS NULL OR \"Id\" = {onlyJobId}) ORDER BY CASE WHEN \"JobType\" = 'TRENDYOL_ORDER_SYNC' THEN 0 ELSE 1 END ASC, \"Priority\" ASC, \"AvailableAt\" ASC, \"CreatedAt\" ASC FOR UPDATE SKIP LOCKED LIMIT 1").SingleOrDefaultAsync(cancellationToken)
        };
