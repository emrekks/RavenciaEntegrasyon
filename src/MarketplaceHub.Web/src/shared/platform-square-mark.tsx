...
export function PlatformSquareMark({ code, name }: { code: string | null | undefined; name?: string | null }) {
  ...
  const logoSource = platformLogoSource(code)

  return (
    <span className={`platform-square-mark ${platformLogoClass(code)}`} title={label} aria-hidden="true">
      {logoSource ? <img src={logoSource} alt="" /> : <strong>{platformMarks[normalized] ?? (normalized.slice(0, 2) || '?')}</strong>}
    </span>
  )
}
