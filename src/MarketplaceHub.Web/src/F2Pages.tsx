...
export function NewProductPage({ editProductId }: { editProductId?: string } = {}) {
  ...
  function moveVariant(sourceKey: string, targetKey: string) {
    ...
    setVariantRows(rows => {
      ...
      const next = [...rows]; const [moved] = next.splice(sourceIndex, 1); next.splice(targetIndex, 0, moved)
      return next
    })
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const product = productToEdit.data
        ? await hubApi<Product>(`/products/${productToEdit.data.id}`, { method: 'PATCH', headers: { 'If-Match': `"v${productToEdit.data.version}"` }, body: JSON.stringify({ title: form.title, description: form.description, brandId: form.brandId || null, categoryId: form.categoryId || null, attributes: globalAttributes }) })
