...
export function NewProductPage() {
  ...
  return <Page title="Yeni Ürün Ekle" eyebrow="Katalog"><p className="lede page-lede">Kategori özellikleri, varyant kombinasyonları, stok, fiyat ve Trendyol yayın kuyruğu tek ürün çalışma alanında yönetilir.</p><form className="product-creation-workspace" onSubmit={submit}>
    ...
    <div className="product-layout-grid"><div className="product-main-stack">
      ...
      <section className="panel product-step-card cargo-size-card"><div className="editor-section-title"><span>3</span><div><h2>Kargo ölçüleri ve desi</h2><p>Doğrudan desi girin veya ölçülerden otomatik hesaplamayı açın.</p></div></div><label className="desi-calculation-toggle"><input type="checkbox" checked={calculateDesi} onChange={event => setCalculateDesi(event.target.checked)} /><span><strong>Desiyi ölçülerden hesapla</strong><small>En × boy × yükseklik / 3000 formülü kullanılır.</small></span></label>{calculateDesi ? <div className="product-step-grid"><label>Ağırlık (kg)<input value={form.weight} onChange={event => updateField('weight', event.target.value)} type="number" min="0" step="0.01" /></label><label>En (cm)<input value={form.width} onChange={event => updateField('width', event.target.value)} type="number" min="0" step="0.1" /></label><label>Boy (cm)<input value={form.length} onChange={event => updateField('length', event.target.value)} type="number" min="0" step="0.1" /></label><label>Yükseklik (cm)<input value={form.height} onChange={event => updateField('height', event.target.value)} type="number" min="0" step="0.1" /></label><div className="calculated-field"><small>Hesaplanan desi</small><strong>{desi ? desi.toLocaleString('tr-TR', { maximumFractionDigits: 2 }) : '—'}</strong></div></div> : <div className="manual-desi-field"><label>Desi<input value={form.desi} onChange={event => updateField('desi', event.target.value)} type="number" min="0.01" step="0.01" required /></label><small>Varsayılan değer 1’dir.</small></div>}</section>
...
export function ProductDetailPage() {
  const { id = '' } = useParams(); const client = useQueryClient(); const [connectionId, setConnectionId] = useState(''); const [notice, setNotice] = useState('')
  ...
  return <Page title={query.data?.title ?? 'Ürün'} eyebrow="Ürün detayı">{query.isError ? <ErrorBox error={query.error} /> : !query.data ? <p>Yükleniyor…</p> : <>
    {notice && <div role="status" className="notice">{notice}</div>}<div className="detail-grid"><article className="panel product-detail-hero">{query.data.primaryImageUrl ? <img src={query.data.primaryImageUrl} alt={query.data.title} /> : <span className="product-image-placeholder">Görsel yok</span>}<div><Tag>{query.data.status}</Tag><p>Model: {query.data.modelCode ?? '—'}</p><p>Genel stok: <strong>{query.data.totalStock}</strong></p><p>Başlangıç fiyatı: <strong>{money(query.data.startingPrice, query.data.currency)}</strong></p><p>Aktif platformlar: {query.data.activePlatforms?.join(', ') || '—'}</p></div></article><form className="panel form-grid" onSubmit={updateProduct}><h2>Ürünü düzenle</h2><label>Başlık<input name="title" defaultValue={query.data.title} required /></label><label>Açıklama<textarea name="description" defaultValue={query.data.description} /></label><button>Bilgileri kaydet</button></form></div>
    <form className="panel form-grid" onSubmit={image}><h2>Ana görsel</h2><label>Görsel URL<input name="imageUrl" type="url" defaultValue={query.data.primaryImageUrl ?? ''} required /></label><button>Görseli kaydet</button></form>
