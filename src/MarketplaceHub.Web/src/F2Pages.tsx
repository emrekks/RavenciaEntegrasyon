...
function RichTextEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return <div className="rich-text-editor"><div className="rich-text-editor-head"><div><strong>Ürün açıklaması</strong><small>Başlıklar, listeler, bağlantılar ve vurgular ekleyin.</small></div><span>{value.replace(/<[^>]*>/g, '').trim().length} karakter</span></div><div className="rich-text-toolbar" aria-label="Açıklama biçimlendirme araçları"><div className="rich-text-tool-group"><button type="button" title="Kalın" onClick={() => wrap('<strong>', '</strong>')}><b>B</b></button><button type="button" title="İtalik" onClick={() => wrap('<em>', '</em>')}><i>I</i></button><button type="button" title="Üstü çizili" aria-label="Üstü çizili" onClick={() => wrap('<s>', '</s>')}>S</button></div><div className="rich-text-tool-group"><button type="button" onClick={() => wrap('<h2>', '</h2>')}>Başlık</button><button type="button" onClick={() => wrap('<p>', '</p>')}>Paragraf</button><button type="button" onClick={() => wrap('<blockquote>', '</blockquote>')}>Alıntı</button></div><div className="rich-text-tool-group"><button type="button" onClick={() => wrap('<ul><li>', '</li></ul>')}>• Liste</button><button type="button" onClick={() => wrap('<ol><li>', '</li></ol>')}>1. Liste</button><button type="button" onClick={() => wrap('<a href="https://">', '</a>')}>Bağlantı</button></div></div><textarea ref={editor} value={value} onChange={event => onChange(event.target.value)} required aria-label="Açıklama" placeholder="Ürünün öne çıkan özelliklerini anlatın…" /><details><summary>Canlı HTML ön izlemesi</summary><iframe className="rich-text-preview" sandbox="" title="Açıklama HTML ön izlemesi" srcDoc={value} /></details></div>
}
