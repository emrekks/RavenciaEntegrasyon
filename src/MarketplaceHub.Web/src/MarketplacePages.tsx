...
export function OrdersPage() {
  ...
  const matchTab = (status: string, tab: string) => {
    ...
    if (tab === 'RESENT') return ['RESEND', 'RESEND_REQUESTED', 'REPLACEMENT', 'RETURN_IN_TRANSIT', 'RETURNED'].includes(status);
    if (tab === 'ON_HOLD') return ['ON_HOLD', 'MANUAL_REVIEW'].includes(status);
    return false;
  }
  const packageRows = all.flatMap(order => (order.packages ?? []).map(pack => ({ order, pack })))
  const matchesOrderTab = (item: Order, tab: string) => item.packages?.length ? item.packages.some(pack => matchTab(pack.status.toUpperCase(), tab)) : matchTab(item.derivedStatus.toUpperCase(), tab)
  const tabCount = (tab: string) => tab === 'ALL' ? packageRows.length : packageRows.filter(row => matchTab(row.pack.status.toUpperCase(), tab)).length
  ...
  return <section className="content f3 orders-page"><div className="page-heading"><div><p className="eyebrow">Sipariş yönetimi</p><h1>Siparişler</h1><p className="lede">Sipariş, müşteri, ürün, termin, fatura ve kargo bilgilerini tek ekrandan yönetin.</p></div></div>
    ...
    <div className="order-tabs" role="tablist" aria-label="Sipariş durumları">{statuses.map(([value,label]) => <button type="button" role="tab" aria-selected={filters.status === value} className={filters.status === value ? 'active' : ''} key={value} onClick={() => selectStatus(value)}><span>{label}</span><b>{tabCount(value)}</b><small>Paket</small></button>)}</div>
