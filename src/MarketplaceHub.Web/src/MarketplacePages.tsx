...
function ReferenceConnectionCard({ item, onSaved }: { item: Connection; onSaved: () => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`} onClickCapture={event => { const target = event.target; if (target instanceof Element && target.closest('.reference-menu-popover a')) { event.preventDefault(); setMenuOpen(false); setSettingsOpen(true) } }}>
    ...
    <footer className="reference-integration-footer"><button type="button" className={`reference-action ${connected ? 'secondary' : 'primary-action'}`} onClick={() => connected ? setDisconnectOpen(true) : activate.mutate()} disabled={disconnect.isPending || activate.isPending || test.isPending || saving}>{disconnect.isPending ? 'Pasife alınıyor…' : activate.isPending ? 'Aktif ediliyor…' : connected ? 'Pasife al' : 'Aktif et'}</button><button type="button" className="reference-action link-action" onClick={() => test.mutate()} disabled={test.isPending || disconnect.isPending || activate.isPending || saving}>{test.isPending ? 'Kontrol ediliyor…' : 'Bağlantıyı kontrol et'}</button><button type="button" className="reference-action primary-action" onClick={() => saveDetails.mutate()} disabled={saving || disconnect.isPending || activate.isPending || test.isPending}>{saving ? 'Kaydediliyor…' : 'Bilgileri güncelle'}</button></footer>
