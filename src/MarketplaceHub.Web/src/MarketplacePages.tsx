...
function ReferenceConnectionCard({ item, onSaved }: { item: Connection; onSaved: () => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`}>
    ...
    <div className="reference-integration-body"><div className="reference-data-point"><span>{efaturam ? 'VKN / TCKN' : 'Mağaza Kimliği (Seller ID)'}</span><strong>{item.externalStoreId || '—'}</strong></div><div className="reference-data-point"><span>{efaturam ? 'API Secret (Maskeli)' : 'API Key (Maskeli)'}</span><strong className="reference-secret"><b>••••••••••••••••</b><em>{maskedSuffix}</em></strong></div><div className="reference-data-point"><span>Ortam</span><strong>{item.environment === 'PRODUCTION' ? 'Canlı' : item.environment === 'STAGE' ? 'Stage' : item.environment || '—'}</strong></div><div className="reference-data-point"><span>API sürümü</span><strong>{item.apiVersion || '—'}</strong></div><div className="reference-data-point reference-data-point-wide"><span>{efaturam ? 'Son Fatura İletimi' : 'Son Başarılı Senkronizasyon'}</span><strong className={connected ? 'reference-last-success' : ''}>{displayDate ? <><i aria-hidden="true">{connected ? '✓' : '◷'}</i><DateText value={displayDate} /></> : 'Henüz gerçekleşmedi'}</strong></div></div>
...
export function IntegrationsPage() {
  ...
  return <section className="content f3 integrations-reference-page"><div className="integration-reference-heading"><div><h1>Platform Entegrasyonları</h1><p>Pazar yeri ve fatura sağlayıcılarınızla olan veri akışını yönetin.</p></div><div className="integration-heading-actions"><div className="integration-filter-wrap"><button type="button" className="integration-filter-button" aria-expanded={filterOpen} onClick={() => setFilterOpen(value => !value)}><span aria-hidden="true">☷</span> Filtrele</button>{filterOpen && <div className="integration-filter-menu" role="menu"><strong>Görünüm</strong>{(['ALL', 'ACTIVE', 'ATTENTION'] as const).map(value => <button type="button" key={value} className={filter === value ? 'active' : ''} onClick={() => { setFilter(value); setFilterOpen(false) }}>{value === 'ALL' ? 'Tüm platformlar' : value === 'ACTIVE' ? 'Aktif bağlantılar' : 'Dikkat gerekenler'}</button>)}</div>}</div><button type="button" className="integration-add-button" onClick={() => setCreateOpen(true)}><span aria-hidden="true">＋</span> Ekle</button></div></div>
