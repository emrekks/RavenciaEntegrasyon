...
function patchOrderShipment(order: Order, updatedShipment: Shipment): Order {
  ...
  return { ...order, packages, cargoProviderName: isPrimaryPackage ? updatedShipment.cargoProviderName : order.cargoProviderName, cargoTrackingNumber: isPrimaryPackage ? updatedShipment.cargoTrackingNumber : order.cargoTrackingNumber }
}

function CourierChangeModal({ item, onClose, onConfirmed }: { item: Order; onClose: () => void; onConfirmed: (shipment: Shipment) => void }) {
...
export function OrdersPage() {
  ...
  const allOrders = Array.from(allById.values()); const all = allOrders.filter(item => !hiddenCancelledOrderIds.includes(item.id)); const ordersLoading = orderQueries.some(result => result.isPending); const ordersError = orderQueries.find(result => result.isError)?.error; const normalized = filters.search.trim().toLocaleLowerCase('tr-TR')
  ...
  const hiddenPackageCount = (tab: string) => allOrders.filter(item => hiddenCancelledOrderIds.includes(item.id)).reduce((count, item) => count + (item.packages?.length ? item.packages.filter(pack => tab === 'ALL' ? true : tab === 'RESENT' ? pack.isResend : matchTab(pack.status.toUpperCase(), tab)).length : (tab === 'ALL' || matchTab(item.derivedStatus.toUpperCase(), tab) ? 1 : 0)), 0)
  const tabCount = (tab: string) => {
    ...
    if (summary.data && summaryKey) return Math.max(0, summary.data[summaryKey] - hiddenPackageCount(tab))
...
function CategoryMappingWorkspace() {
  ...
  async function exportMappings() {
    ...
    try {
      ...
      const bundle: MappingTransferBundle = { format: 'RAVENCIA_MAPPING_BUNDLE', version: 1, exportedAt: new Date().toISOString(), categories: activeCategories, attributes: localAttributes.data?.items ?? [], categoryMappings: savedMappings, attributeMappings, attributeValueMappings }
      const url = URL.createObjectURL(new Blob([JSON.stringify(bundle, null, 2)], { type: 'application/json;charset=utf-8' }))
      const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'ravencia-eslestirme-yedegi.json'; anchor.click(); URL.revokeObjectURL(url)
...
<section className="mapping-transfer-panel" aria-label="Eşleştirme aktarımı"><div><strong>Eşleştirme yedeği</strong><small>Kategorileri, ürün seçeneklerini, ürün özelliklerini ve eşleşmeleri JSON olarak taşıyın.</small></div><div className="button-row"><button type="button" className="secondary" onClick={() => void exportMappings()} disabled={transferBusy || !connectionId}>{transferBusy ? 'Hazırlanıyor…' : 'Dışa aktar'}</button><label className="mapping-transfer-upload">İçe aktar<input type="file" accept="application/json,.json" onChange={readMappingBundle} disabled={transferBusy} /></label></div></section>
