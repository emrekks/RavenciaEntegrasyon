...
function CategoryMappingWorkspace() {
  const client = useQueryClient()
  const [connectionId, setConnectionId] = useState(''); const [localId, setLocalId] = useState(''); const [externalId, setExternalId] = useState(''); const [notice, setNotice] = useState(''); const [categoryName, setCategoryName] = useState(''); const [categoryLibrarySearch, setCategoryLibrarySearch] = useState('')
  ...
  return <section className="content f3 mapping-page mapping-workspace"><div className="mapping-workspace-heading"><div><p className="eyebrow">Merkezi katalog sistemi</p><h1>Eşleştirme Merkezi</h1></div></div><MappingViewTabs active="category" />{notice && <div role="status" className="notice">{notice}</div>}
    <article className="panel mapping-console">
      <form className="platform-category-create" onSubmit={event => { event.preventDefault(); createCategory.mutate() }}><label>Yeni panel kategorisi<input aria-label="Yeni panel kategorisi adı" maxLength={160} value={categoryName} onChange={event => setCategoryName(event.target.value)} placeholder="Örn. Anne Bluz" required /></label><button disabled={createCategory.isPending}>{createCategory.isPending ? 'Ekleniyor…' : '+ Kategori ekle'}</button></form><details className="category-library"><summary><span>Eklenen panel kategorileri</span><small>{activeCategories.length.toLocaleString('tr-TR')} kayıt · aç / kapat</small></summary><div className="category-library-tools"><input aria-label="Eklenen panel kategorilerinde ara" value={categoryLibrarySearch} onChange={event => setCategoryLibrarySearch(event.target.value)} placeholder="Kategori ara…" /><span>{filteredCategoryLibrary.length.toLocaleString('tr-TR')} sonuç</span></div><div className="category-chip-list" aria-label="Eklenen panel kategorileri">{filteredCategoryLibrary.length ? filteredCategoryLibrary.map(category => <span key={category.id} className={category.id === localId ? 'active' : ''}><button type="button" onClick={() => { setLocalId(category.id); setExternalId(''); setNotice('') }}>{category.name}</button><button type="button" className="category-chip-remove" aria-label={`${category.name} kategorisini kaldır`} onClick={() => archiveCategory.mutate(category)} disabled={archiveCategory.isPending}>×</button></span>) : <small>{activeCategories.length ? 'Aramayla eşleşen kategori yok.' : 'Henüz panel kategorisi eklenmedi.'}</small>}</div></details>
