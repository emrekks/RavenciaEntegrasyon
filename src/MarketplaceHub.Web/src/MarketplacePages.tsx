...
function ReferenceConnectionCard({ item, onSaved }: { item: Connection; onSaved: () => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`}>
    ...
    <div className="reference-integration-body"><label className="reference-data-point"><span>{efaturam ? 'VKN / TCKN' : 'Mağaza Kimliği (Seller ID)'}</span><input className="reference-editable-field" value={externalStoreId} onChange={event => setExternalStoreId(event.target.value)} /></label><label className="reference-data-point"><span>{efaturam ? 'API Secret (Maskeli)' : 'API Key (Maskeli)'}</span><input className="reference-editable-field" type="password" value={efaturam ? '' : apiKey} onChange={event => setApiKey(event.target.value)} autoComplete="off" /></label>{!efaturam && <label className="reference-data-point"><span>API Secret (Maskeli)</span><input className="reference-editable-field" type="password" value={apiSecret} onChange={event => setApiSecret(event.target.value)} autoComplete="new-password" /></label>}<label className="reference-data-point"><span>Ortam</span><select className="reference-editable-field" value={environment} onChange={event => setEnvironment(event.target.value)}><option value="STAGE">Stage</option><option value="PRODUCTION">Canlı</option></select></label><div className="reference-data-point"><span>API sürümü</span><strong>{item.apiVersion || ''}</strong></div></div>
