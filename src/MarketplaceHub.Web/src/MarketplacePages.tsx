...
async function loadOrderPage(filters: OrderFilters, limit: number, after: string | null): Promise<Page<Order>> {
...
export function OrdersPage() {
  ...
  const ordersQuery = useQuery({ queryKey: ['orders', 'page', filters, pageSize, page, orderCursor], queryFn: () => loadOrderPage(filters, pageSize, orderCursor) })
  ...
  const tabCount = (tab: string) => {
    ...
    if (summary.data && summaryKey && typeof summary.data[summaryKey] === 'number') return Math.max(0, summary.data[summaryKey] - hiddenOrderCount(tab))
  ...
  const totalCount = ordersQuery.data?.totalCount ?? 0; const totalPages = totalCount > 0 ? Math.ceil(totalCount / pageSize) : ordersQuery.data?.hasMore ? page + 1 : 1; const safePage = Math.min(page, totalPages); const pageItems = items; const selectablePageItems = pageItems.filter(item => item.derivedStatus.toUpperCase() !== 'CANCELLED'); const allPageSelected = selectablePageItems.length > 0 && selectablePageItems.every(item => selectedIds.includes(item.id))
  useEffect(() => {
