...
type OrderFilters = { search: string; status: string; platform: string; listing: string; cargo: string; invoice: string; invoiceType: string; invoiceRegion: string; sort: OrderSort; dateFrom: string; dateTo: string }
...
const initialOrderFilters: OrderFilters = { search: '', status: 'ALL', platform: 'ALL', listing: 'ALL', cargo: 'ALL', invoice: 'ALL', invoiceType: 'ALL', invoiceRegion: 'ALL', sort: 'DATE_DESC', dateFrom: '', dateTo: '' }
...
export function OrdersPage() {
  ...
  return <section className="content f3 orders-page"><div className="page-heading"><div><p className="eyebrow">Sipariş yönetimi</p><h1>Sipariş Yönetimi</h1><p className="lede">Tüm pazar yeri siparişlerinizi tek merkezden yönetin ve takip edin.</p></div><div className="page-heading-actions orders-reference-heading-actions"><button type="button" className="secondary orders-export-action" disabled={!items.length} onClick={exportOrders}><span className="orders-export-icon" aria-hidden="true" />Dışa Aktar</button><button type="button" className="orders-sync-action" onClick={() => setSingleSyncOpen(true)}><span aria-hidden="true">↻</span> Sipariş Senkronizasyonu</button></div></div>
    ...
    <div className="orders-reference-filter-shell"><div className="order-tabs" role="tablist" aria-label="Sipariş durumları">{statuses.map(([value,label]) => <button type="button" role="tab" aria-selected={filters.status === value} className={filters.status === value ? 'active' : ''} key={value} onClick={() => selectStatus(value)}><span>{label}</span><b>{summary.isLoading ? '…' : tabCount(value)}</b><small>Paket</small></button>)}</div><div className="orders-bulk-toolbar"><div><strong>Toplu işlem merkezi</strong><span>{selectedIds.length ? `${selectedIds.length} sipariş seçildi` : 'İşlem uygulamak için sipariş seçin'}</span></div><div className="bulk-menu-shell"><button type="button" className="bulk-action" disabled={!selectedIds.length} aria-expanded={bulkOpen} onClick={() => setBulkOpen(value => !value)}>Toplu işlemler {selectedIds.length ? `(${selectedIds.length})` : ''}⌄</button>{bulkOpen && <div className="bulk-action-menu" role="menu"><button type="button" role="menuitem" onClick={() => void bulkAction('processing')}><b>01</b><span>İşleme Al<small>Yalnız yeni siparişler</small></span></button><button type="button" role="menuitem" onClick={() => void bulkAction('courier')}><b>02</b><span>Kargo firmasını değiştir<small>Seçili paketler</small></span></button><button type="button" role="menuitem" onClick={() => void bulkAction('invoice')}><b>03</b><span>Toplu fatura kes<small>Önce taslakları kontrol edin</small></span></button><button type="button" role="menuitem" onClick={() => void bulkAction('labels')}><b>04</b><span>Kargo stickerlarını yazdır<small>Takip numarası olanlar</small></span></button></div>}</div></div>
    <section className="order-filter-panel" aria-label="Sipariş filtreleri"><div className="order-filter-primary"><label className="order-search"><span aria-hidden="true">⌕</span><input aria-label="Sipariş ara" value={filterForm.search} onChange={event => updateFilter('search', event.target.value)} placeholder="No girin…" onKeyDown={event => { if (event.key === 'Enter') applyFilters() }} /></label><label>Platform<select value={filterForm.platform} onChange={event => updateFilter('platform', event.target.value)}><option value="ALL">Tüm platformlar</option>{platforms.map(value => <option key={value} value={value}>{value === 'TRENDYOL' ? 'Trendyol' : value}</option>)}</select></label><label>Sipariş durumu<select value={filterForm.status} onChange={event => updateFilter('status', event.target.value)}>{statuses.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="order-sort-filter">Sıralama<select aria-label="Sıralama" value={filterForm.sort} onChange={event => applyFilterValue('sort', event.target.value as OrderSort)}>{sortOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button type="button" className="filter-toggle" onClick={() => setAdvancedFilters(value => !value)} aria-expanded={advancedFilters}>Filtrele</button><button type="button" className="secondary filter-clear" onClick={clearFilters}>Temizle</button><button type="button" className="filter-apply" onClick={applyFilters}>Uygula</button><button type="button" className="secondary single-order-sync-btn" onClick={() => setSingleSyncOpen(true)} title="Trendyol'dan sipariş numarası ile tekil sipariş çek">⚡ Tekil Sipariş Çek</button></div>{advancedFilters && <div className="order-filter-advanced"><label>Listeleme durumu<select value={filterForm.listing} onChange={event => updateFilter('listing', event.target.value)}><option value="ALL">Tüm kayıtlar</option><option value="OPEN">Açık siparişler</option><option value="CLOSED">Kapanan siparişler</option></select></label><label>Sipariş tarihi başlangıç<input type="date" value={filterForm.dateFrom} onChange={event => updateFilter('dateFrom', event.target.value)} /></label><label>Sipariş tarihi bitiş<input type="date" value={filterForm.dateTo} onChange={event => updateFilter('dateTo', event.target.value)} /></label></div>}</section></div>
    {ordersLoading && !all.length ? <Busy text="Yerel sipariş kayıtları yükleniyor…" /> : ordersError && !all.length ? <ErrorBox error={ordersError} /> : !all.length ? <Empty>Aktif ve kanıtlanmış bağlantıdan sipariş eşitlemesi çalıştırıldığında kayıtlar burada görünür.</Empty> : !items.length ? <Empty>Seçili durum ve filtrelerle eşleşen sipariş yok.</Empty> : <><div className="order-reference-table"><div className="order-reference-head"><label className="order-select"><input type="checkbox" checked={allPageSelected} onChange={event => togglePageSelection(event.target.checked)} aria-label="Sayfadaki siparişleri seç" /></label><strong>Sipariş Bilgileri</strong><strong>Alıcı</strong><strong>Bilgiler</strong><strong>Birim Fiyat</strong><div className="order-column-filter"><strong>Kargo</strong><select aria-label="Kargo firmasına göre filtrele" value={filterForm.cargo} onChange={event => applyFilterValue('cargo', event.target.value)}><option value="ALL">Tüm kargolar</option>{cargos.map(value => <option key={value} value={value}>{value}</option>)}</select></div><div className="order-column-filter order-invoice-column-filter"><strong>Fatura</strong><select aria-label="Fatura durumuna göre filtrele" value={filterForm.invoice} onChange={event => applyFilterValue('invoice', event.target.value)}><option value="ALL">Durum: Tümü</option>{invoiceStatuses.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select aria-label="Fatura türüne göre filtrele" value={filterForm.invoiceType} onChange={event => applyFilterValue('invoiceType', event.target.value)}><option value="ALL">Tür: Tümü</option><option value="BIREYSEL">Bireysel</option><option value="KURUMSAL">Kurumsal</option></select><select aria-label="Fatura bölgesine göre filtrele" value={filterForm.invoiceRegion} onChange={event => applyFilterValue('invoiceRegion', event.target.value)}><option value="ALL">Bölge: Tümü</option><option value="TR">Türkiye</option><option value="MICRO_EXPORT">Mikro ihracat</option></select></div><strong>Durum</strong></div>{pageItems.map(item => <OrderReferenceRow item={item} key={item.id} selected={selectedIds.includes(item.id)} onSelect={checked => updateSelection(item.id, checked)} openMenu={menu?.orderId === item.id ? menu.kind : null} onMenuChange={kind => setMenu(kind ? { orderId: item.id, kind } : null)} onInvoiceCreate={() => { setMenu(null); setInvoiceDraftOrder(item) }} onInvoiceDetails={() => { setMenu(null); setInvoiceInfoOrder(item) }} onInvoiceUpload={() => { setMenu(null); setInvoiceUploadOrder(item) }} onCourierChange={() => { setMenu(null); setCourierOrder(item) }} onProcessOrder={() => void processSingleOrder(item)} onPreviewImage={setPreviewImage} />)}</div><div className="order-pagination"><label>Sayfa başına <select aria-label="Sayfa başına sipariş" value={pageSize} onChange={event => { setPageSize(Number(event.target.value)); setPage(1) }}>{[20, 50, 100, 200].map(value => <option key={value} value={value}>{value}</option>)}</select></label><span>{items.length} siparişten {(safePage - 1) * pageSize + 1}–{Math.min(safePage * pageSize, items.length)} arası gösteriliyor</span><div><button type="button" disabled={safePage <= 1} onClick={() => setPage(safePage - 1)}>‹ Önceki</button><b>{safePage} / {totalPages}</b><button type="button" disabled={safePage >= totalPages} onClick={() => setPage(safePage + 1)}>Sonraki ›</button></div></div></>}
