...
function PanelAttributeLibraryBuilder({ role, attributes, onNotice }: { role: 'ATTRIBUTE' | 'OPTION'; attributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const inferredOption = (attribute: LocalAttribute) => /^(beden|renk|web color|yaş grubu|cinsiyet|ölçü|boyut|size|color)$/i.test(attribute.name.trim())
  const records = attributes.filter(attribute => isOption ? attribute.code.startsWith('option-') || inferredOption(attribute) : !attribute.code.startsWith('option-') && !inferredOption(attribute))
