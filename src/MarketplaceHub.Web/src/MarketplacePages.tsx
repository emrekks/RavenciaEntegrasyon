...
function ShippingLabelModal({ item, format, onClose }: { item: Order; format?: ShippingLabelFormat; onClose: () => void }) {
  ...
  return <div className="workspace-modal-backdrop shipping-label-backdrop" role="presentation" onMouseDown={onClose}>
    <section className={`workspace-modal shipping-label-modal format-${format}`} role="dialog" aria-modal="true" aria-labelledby="shipping-label-title" onMouseDown={event => event.stopPropagation()}>
      ...
      <footer className="shipping-label-modal-actions"><span>Etiket bilgileri sipariş ve paket kaydından dolduruldu.</span><button type="button" className="secondary" onClick={onClose}>Vazgeç</button><button type="button" onClick={() => { markShippingLabelPrinted(item.id, format); window.print() }}>Yazdır</button></footer>
    </section>
  </div>
}
...
function OrderReferenceRow({ item, selected, onSelect, openMenu, onMenuChange, onInvoiceCreate, onInvoiceDetails, onInvoiceUpload, onCourierChange, onProcessOrder, onPrintLabel, onHideCancelled, showCancelledActions, onPreviewImage }: { item: Order; selected: boolean; onSelect: (checked: boolean) => void; openMenu: 'invoice' | 'actions' | null; onMenuChange: (value: 'invoice' | 'actions' | null) => void; onInvoiceCreate: () => void; onInvoiceDetails: () => void; onInvoiceUpload: () => void; onCourierChange: () => void; onProcessOrder: () => void; onPrintLabel: (format: ShippingLabelFormat) => void; onHideCancelled?: () => void; showCancelledActions?: boolean; onPreviewImage: (preview: { url: string; title: string }) => void }) {
  ...
  const labelSettings = loadShippingLabelSettings()
  const printedLabels = new Set(loadPrintedShippingLabels())
...
export function OrdersPage() {
  ...
  async function bulkAction(kind: 'processing' | 'courier' | 'invoice' | 'labels') {
    ...
    if (kind === 'processing') {
      ...
      try {
        ...
        setBulkNotice(`${eligible.length} sipariş anlık olarak işleme alındı ve panelde güncellendi.`)
        setSelectedIds([])
        await client.invalidateQueries({ queryKey: ['orders'] })
  ...
  async function processSingleOrder(item: Order) {
    ...
    try {
      const updatedShipment = await hubApi<Shipment>(`/shipments/${pack.id}/instant-process`, {
        method: 'POST',
        headers: { 'Idempotency-Key': `instant-picking:${pack.id}:${Date.now()}`, 'If-Match': `"v${pack.version}"` }
      })
