...
const initialOrderLabelSettings = loadShippingLabelSettings()
const initialPrintedOrderLabels = new Set(loadPrintedShippingLabels())
...
async function loadOrdersByStatus(status: string): Promise<Order[]> {
  ...
  const page = await loadAllPages<Order>(status ? `/orders?status=${encodeURIComponent(status)}` : '/orders')
...
export function OrdersPage() {
  ...
  const ordersQuery = useQuery({ queryKey: ['orders', 'ALL'], queryFn: () => loadOrdersByStatus('') })
  ...
  const allOrders = ordersQuery.data ?? []; const all = allOrders.filter(item => !hiddenCancelledOrderIds.includes(item.id)); const ordersLoading = ordersQuery.isPending; const ordersError = ordersQuery.error; const normalized = filters.search.trim().toLocaleLowerCase('tr-TR')
  ...
  const matchTab = (status: string, tab: string) => {
    ...
    if (tab === 'RETURN_IN_TRANSIT') return status === 'RETURN_IN_TRANSIT';
    if (tab === 'PARTIALLY_CANCELLED') return status === 'PARTIALLY_CANCELLED';
  ...
  const hiddenOrderCount = (tab: string) => allOrders.filter(item => hiddenCancelledOrderIds.includes(item.id) && matchesOrderTab(item, tab)).length
  ...
  const tabCount = (tab: string) => {
    ...
    if (summary.data && summaryKey) return Math.max(0, summary.data[summaryKey] - hiddenOrderCount(tab))
    return allOrders.filter(item => matchesOrderTab(item, tab)).length
  }
  ...
  return <section className="content f3 orders-page"><div className="page-heading"><div><p className="eyebrow">Sipariş yönetimi</p><h1>Sipariş Yönetimi</h1><p className="lede">Tüm pazar yeri siparişlerinizi tek merkezden yönetin ve takip edin.</p></div><div className="page-heading-actions orders-reference-heading-actions"><button type="button" className="secondary orders-export-action" disabled={!items.length} onClick={exportOrders}><span className="orders-export-icon" aria-hidden="true" />Dışa Aktar</button><button type="button" className="orders-sync-action" onClick={() => setSingleSyncOpen(true)}><span aria-hidden="true">↻</span> Sipariş Senkronizasyonu</button></div></div><div className="order-label-filter-quick"><label>Etiket durumu<select aria-label="Etiket yazdırma durumuna göre filtrele" value={filterForm.label} onChange={event => applyFilterValue('label', event.target.value)}><option value="ALL">Tüm etiketler</option><option value="PRINTED">Etiketi yazdırılanlar</option><option value="NOT_PRINTED">Etiketi yazdırılmayanlar</option></select></label></div>
    {bulkNotice && <div key={bulkNoticeVersion} className="notice order-bulk-notice" role="status">{bulkNotice}<button type="button" aria-label="Bildirimi kapat" onClick={() => setBulkNotice('')}>×</button></div>}{hiddenCancelledOrderIds.length > 0 && <div className="notice order-hidden-cancelled-notice" role="status"><span>{hiddenCancelledOrderIds.length} iptal sipariş gizli.</span><button type="button" onClick={restoreHiddenCancelledOrders}>Gizlenenleri geri göster</button></div>}
    <div className="orders-reference-filter-shell"><div className="order-tabs" role="tablist" aria-label="Sipariş durumları">{statuses.map(([value,label]) => <button type="button" role="tab" aria-selected={filters.status === value} className={filters.status === value ? 'active' : ''} key={value} onClick={() => selectStatus(value)}><span>{label}</span><b>{summary.isLoading ? '…' : tabCount(value)}</b><small>Sipariş</small></button>)}</div>
