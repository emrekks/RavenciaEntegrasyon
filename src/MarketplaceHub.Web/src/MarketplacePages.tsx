...
function ReferenceConnectionCard({ item, onSaved }: { item: Connection; onSaved: () => void }) {
  ...
  const deepDelete = useMutation({ mutationFn: () => hubApi(`/connections/${item.id}/deep-delete`, { method: 'POST', headers: { 'Idempotency-Key': idempotency(), 'If-Match': `"v${item.version}"` }, body: JSON.stringify({ confirmation: deleteConfirmation }) }), onSuccess: () => { setDeleteOpen(false); setDeleteConfirmation(''); onSaved() }, onError: reason => setNotice(reason instanceof Error ? reason.message : 'Bağlantı ve bağlı veriler silinemedi.') })
  const displayDate = item.lastSuccessAt ?? item.lastTestedAt
  const maskedSuffix = item.hasCredential ? 'Kayıtlı' : 'Bekleniyor'
  return <article className={`integration-card reference-integration-card ${connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`}>
    <div className="reference-integration-accent" aria-hidden="true" />
    <header className="reference-integration-header">
      <div className="reference-integration-brand"><span className={`reference-provider-logo ${efaturam ? 'efaturam' : 'trendyol'}`} aria-hidden="true">{efaturam ? 'e-' : 'ty'}</span><div><h2>{item.displayName}</h2><span className={`reference-connection-state ${connected ? 'connected' : 'inactive'}`}><i />{connected ? 'Bağlı & Aktif' : 'Bağlantı Pasif'}</span></div></div>
    ...
    <div className="reference-integration-body"><div className="reference-data-point"><span>{efaturam ? 'VKN / TCKN' : 'Mağaza Kimliği (Seller ID)'}</span><strong>{item.externalStoreId || '—'}</strong></div><div className="reference-data-point"><span>{efaturam ? 'API Secret (Maskeli)' : 'API Key (Maskeli)'}</span><strong className="reference-secret"><b>••••••••••••••••</b><em>{maskedSuffix}</em></strong></div><div className="reference-data-point reference-data-point-wide"><span>{efaturam ? 'Son Fatura İletimi' : 'Son Başarılı Senkronizasyon'}</span><strong className={connected ? 'reference-last-success' : ''}>{displayDate ? <><i aria-hidden="true">{connected ? '✓' : '◷'}</i><DateText value={displayDate} /></> : 'Henüz gerçekleşmedi'}</strong></div></div>
    {notice && <div className="reference-card-notice" role="status">{notice}</div>}
    <footer className="reference-integration-footer"><button type="button" className="reference-action secondary" onClick={() => setDisconnectOpen(true)} disabled={!connected || disconnect.isPending || test.isPending}>{disconnect.isPending ? 'İptal ediliyor…' : 'İptal et'}</button><button type="button" className="reference-action link-action" onClick={() => test.mutate()} disabled={test.isPending || disconnect.isPending}>{test.isPending ? 'Kontrol ediliyor…' : <><span aria-hidden="true">⟳</span> Bağlantıyı kontrol et</>}</button><Link className={`reference-action primary-action ${efaturam ? 'hidden-update' : ''}`} to={`/integrations/${item.id}`}>{efaturam ? 'Bilgileri görüntüle' : 'Bilgileri güncelle'}</Link></footer>
...
export function IntegrationsPage() {
  ...
  const visibleConnections = activeConnections.filter(item => filter === 'ALL' || (filter === 'ACTIVE' ? item.status === 'ACTIVE' || item.status === 'VERIFIED' : item.status !== 'ACTIVE' && item.status !== 'VERIFIED' || Boolean(item.lastErrorCode)))
  const filterLabel = filter === 'ACTIVE' ? 'Aktif bağlantılar' : filter === 'ATTENTION' ? 'Dikkat gerekenler' : 'Tüm platformlar'
...
export function BrandMappingPage() {
  const client = useQueryClient(); const [connectionId, setConnectionId] = useState(''); const [localId, setLocalId] = useState(''); const [externalId, setExternalId] = useState(''); const [notice, setNotice] = useState(''); const [brandName, setBrandName] = useState(''); const [brandEditOpen, setBrandEditOpen] = useState(false)
