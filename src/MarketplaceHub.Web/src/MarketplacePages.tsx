...
export function OrdersPage() {
  ...
  const matchTab = (status: string, tab: string) => {
    ...
    if (tab === 'PROCESSING') return ['PROCESSING', 'READY_TO_SHIP', 'ON_HOLD'].includes(status);
    if (tab === 'CANCELLED') return ['CANCELLED', 'PARTIALLY_CANCELLED', 'RETURNED', 'RETURN_IN_TRANSIT'].includes(status);
    if (tab === 'DELIVERED') return ['DELIVERED', 'UNDELIVERED'].includes(status);
    return status === tab;
  }
  ...
  return <section className="content f3 orders-page"><div className="page-heading"><div><p className="eyebrow">Sipariş yönetimi</p><h1>Siparişler</h1><p className="lede">Sipariş, müşteri, ürün, termin, fatura ve kargo bilgilerini tek ekrandan yönetin.</p></div></div>
    ...
    <div className="order-tabs" role="tablist" aria-label="Sipariş durumları">{statuses.map(([value,label]) => <button type="button" role="tab" aria-selected={filters.status === value} className={filters.status === value ? 'active' : ''} key={value} onClick={() => selectStatus(value)}><span>{label}</span><b>{value === 'ALL' ? all.length : all.filter(x => matchTab(x.derivedStatus.toUpperCase(), value)).length}</b></button>)}</div>
