...
type SyncPolicy = { id: string; resourceType: string; intervalSeconds: number; overlapSeconds: number; jitterSeconds: number; enabled: boolean; version: number; lastSuccessAt: string | null; lastModifiedWatermark: string | null }
...
type OrderSummary = { all: number; new: number; processing: number; shipped: number; delivered: number; resent: number; onHold: number }
...
const orderStatusRank: Record<string, number> = { MANUAL_REVIEW: 90, RETURNED: 80, DELIVERED: 70, UNDELIVERED: 65, SHIPPED: 60, READY_TO_SHIP: 50, PROCESSING: 40, ON_HOLD: 30, CANCELLED: 20, NEW: 10 }
...
async function loadOrdersByStatus(status: string): Promise<Order[]> {
  ...
  const page = await loadAllPages<Order>(`/orders?status=${encodeURIComponent(status)}`)
  for (const item of page.items) itemsById.set(item.id, itemsById.has(item.id) ? mergeOrderRows(itemsById.get(item.id)!, item) : item)
  return Array.from(itemsById.values())
}
const orderSyncStatuses = ['NEW', 'PROCESSING', 'READY_TO_SHIP', 'SHIPPED', 'UNDELIVERED', 'DELIVERED', 'ON_HOLD', 'CANCELLED', 'RETURNED', 'MANUAL_REVIEW'] as const
async function loadAllOrders(): Promise<Page<Order>> {
  const statuses = orderSyncStatuses
  // These are independent local reads; run them together so the slowest status
  // determines the load time instead of the sum of all seven requests.
  const groups = await Promise.all(statuses.map(status => loadOrdersByStatus(status)))
  const itemsById = new Map<string, Order>()
  for (const item of groups.flat()) itemsById.set(item.id, itemsById.has(item.id) ? mergeOrderRows(itemsById.get(item.id)!, item) : item)
  return { items: Array.from(itemsById.values()), nextCursor: null, hasMore: false }
}
function ProductImage({ url, alt, onClick }: { url: string; alt: string; onClick: () => void }) {
  const [failed, setFailed] = useState(false)
  useEffect(() => setFailed(false), [url])
  if (failed) return <span className="reference-product-placeholder" aria-label="Ürün görseli yüklenemedi">▧</span>
...
type MappingProduct = { id: string; title: string; categoryId: string | null; options?: Array<{ id: string; label: string; values: Array<{ id: string; label: string }> }> }
...
function CourierChangeModal({ item, onClose }: { item: Order; onClose: () => void }) {
  ...
  const carriers = [
    { label: 'Yurtiçi Kargo', code: 'YKMP' },
    { label: 'Sürat Kargo', code: 'SURATMP' },
    { label: 'DHL eCommerce', code: 'DHLECOMMP' },
    { label: 'PTT Kargo', code: 'PTTMP' },
    { label: 'Kolay Gelsin', code: 'KOLAYGELSINMP' },
    { label: 'Aras Kargo', code: 'ARASMP' },
    { label: 'Horoz Kargo', code: 'HOROZMP' },
    { label: 'CEVA Logistics', code: 'CEVAMP' },
    { label: 'UPS', code: 'UPSMP' },
    { label: 'Trendyol Express', code: 'TEXMP' }
  ]
  ...
  async function handleCarrierChange() {
    ...
    try {
      ...
      await client.invalidateQueries({ queryKey: ['orders'] })
      window.setTimeout(() => void client.invalidateQueries({ queryKey: ['orders'] }), 2500)
      onClose()
    } catch (err) {
...
function SingleOrderSyncModal({ activeConnection, onClose, onSuccess }: { activeConnection: Connection | null; onClose: () => void; onSuccess: (orderNumber: string) => void }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!trimmed) {
      setErrorMsg('Lütfen sipariş numarasını girin.')
      return
    }
    if (!activeConnection) {
      setErrorMsg('Aktif Trendyol bağlantısı bulunamadı. Platformlar sayfasında Trendyol bağlantınızı kontrol edin.')
      return
    }
    setIsSubmitting(true)
    setErrorMsg('')
    setSuccessMsg('')
    try {
      await hubApi(`/connections/${activeConnection.id}/order-sync-jobs`, {
        method: 'POST',
        headers: { 'Idempotency-Key': idempotency() },
        body: JSON.stringify({ externalOrderId: trimmed })
      })
      onSuccess(trimmed)
      onClose()
    } catch (err) {
      setErrorMsg(err instanceof Error ? err.message : 'Sipariş çekme işi başlatılamadı. Sipariş numarasını ve Trendyol bağlantı durumunu kontrol edin.')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  return (
    <div className="workspace-modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="workspace-modal single-order-sync-modal" role="dialog" aria-modal="true" onMouseDown={e => e.stopPropagation()} style={{ maxWidth: '440px' }}>
        <header>
          <div>
            <h2>Tekil Sipariş Çek</h2>
            <p>Trendyol sipariş numarasını girerek sipariş ve kargo bilgilerini anında içeri aktarın.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '0 24px 20px', display: 'grid', gap: '14px' }}>
          ...
          {successMsg && (
            <p role="status" style={{ margin: 0, color: '#22c55e', fontSize: '0.85rem', fontWeight: 600, display: 'flex', gap: '6px', alignItems: 'center' }}>
              ✓ {successMsg}
            </p>
          )}
          <footer style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '8px' }}>
            ...
            <button type="submit" disabled={isSubmitting || !orderNumber.trim() || !!successMsg}>
              {isSubmitting ? 'Çekiliyor…' : successMsg ? '✓ Tamamlandı' : 'Siparişi Getir'}
            </button>
          </footer>
        </form>
      </section>
...
function OrderReferenceRow({ item, selected, onSelect, openMenu, onMenuChange, onInvoiceCreate, onInvoiceDetails, onInvoiceUpload, onCourierChange, onProcessOrder, onPreviewImage }: { item: Order; selected: boolean; onSelect: (checked: boolean) => void; openMenu: 'invoice' | 'actions' | null; onMenuChange: (value: 'invoice' | 'actions' | null) => void; onInvoiceCreate: () => void; onInvoiceDetails: () => void; onInvoiceUpload: () => void; onCourierChange: () => void; onProcessOrder: () => void; onPreviewImage: (preview: { url: string; title: string }) => void }) {
...
function InlineConnectionCard({ item, onSaved }: { item: Connection; onSaved: () => void }) {
  ...
  const disconnect = useMutation({ mutationFn: (version: number) => hubApi<Connection>(`/connections/${item.id}/active`, { method: 'PUT', headers: { 'Idempotency-Key': idempotency(), 'If-Match': `"v${version}"` }, body: JSON.stringify({ active: false }) }), onSuccess: () => { setDisconnectOpen(false); setNotice('Bağlantı iptal edildi. Siparişler yalnızca aktif bağlantıdan okunur.'); onSaved() }, onError: reason => setNotice(reason instanceof Error ? reason.message : 'Bağlantı iptal edilemedi.') })
 ...
 return <form className="integration-card integration-card-form" onSubmit={submit}><div className="integration-card-top"><span className={`platform-mark ${item.platformCode.toLowerCase()}`}>{efaturam ? '▤' : '◉'}</span><div className="integration-card-top-actions"><span className={connected ? 'connection-state connected' : 'connection-state'}><i />{connected ? 'Bağlı' : 'Bağlı değil'}</span><Link className="integration-card-settings" to={`/integrations/${item.id}`} aria-label={`${item.displayName} ayarlarını aç`} title="Bağlantı ayarlarını aç">⚙</Link></div></div><h2>{item.displayName}</h2><p>{efaturam ? 'Fatura sağlayıcısı' : 'Sipariş kaynağı'}</p><div className="integration-fields"><label>Ortam<select value={environment} onChange={event => changeEnvironment(event.target.value)} disabled={update.isPending}><option value="STAGE">Stage</option><option value="PRODUCTION">Canlı</option></select></label><label>{efaturam ? 'Yerel bağlantı kapsamı' : 'Mağaza kimliği'}<input value={item.externalStoreId} readOnly /></label>{efaturam ? <><label>Hesap e-postası<input name="email" type="email" autoComplete="username" placeholder={item.hasCredential ? 'Kayıtlı hesabı değiştirmek için girin' : 'ornek@firma.com'} required /></label><label>Hesap parolası<input name="password" type="password" autoComplete="new-password" placeholder="Parolanızı girin" required /></label></> : <><label>Servis anahtarı<input name="apiKey" type="password" autoComplete="off" placeholder={item.hasCredential ? 'Kayıtlı anahtarı değiştirmek için girin' : 'API key'} required /></label><label>Servis gizli anahtarı<input name="apiSecret" type="password" autoComplete="new-password" placeholder="API secret" required /></label></>}</div>{testResult && <div role="status" className={`integration-test-result ${item.lastErrorCode ? 'failed' : 'success'}`}>{testResult}{item.lastTestedAt && <small> · {new Date(item.lastTestedAt).toLocaleString('tr-TR')}</small>}</div>}{notice && <div role="status" className="integration-card-notice">{notice}</div>}<div className="integration-card-actions"><button type="button" className="secondary" onClick={() => test.mutate()} disabled={test.isPending || save.isPending || disconnect.isPending}>{test.isPending ? 'Kontrol ediliyor…' : 'Bağlantıyı kontrol et'}</button><button type="submit" className="connection-button" disabled={save.isPending || disconnect.isPending}>{save.isPending ? 'Kaydediliyor…' : connected ? 'Bağlandı ✓' : 'Bağlan'}</button></div>{connected && <button type="button" className="connection-cancel-button" onClick={() => setDisconnectOpen(true)} disabled={disconnect.isPending || test.isPending || save.isPending}>Bağlantıyı iptal et</button>}<button type="button" className="connection-delete-button" onClick={() => { setDisconnectOpen(false); setDeleteOpen(true) }} disabled={deepDelete.isPending}>Bağlantıyı ve verilerini sil</button>{disconnectOpen && <div className="integration-disconnect-confirm" role="alertdialog" aria-live="polite"><span>Bağlantı pasife alınacak. Siparişler bu bağlantıdan canlı çekilmeyecek. Daha sonra yeniden bağlayabilirsiniz.</span><div><button type="button" onClick={() => setDisconnectOpen(false)} disabled={disconnect.isPending}>Vazgeç</button><button type="button" onClick={() => disconnect.mutate(item.version)} disabled={disconnect.isPending}>{disconnect.isPending ? 'İptal ediliyor…' : 'Bağlantıyı iptal et'}</button></div></div>}{deleteOpen && <div className="integration-delete-confirm" role="alertdialog"><strong>Derin temizlik</strong><span>Bağlantı otomatik kesilir; bu mağazanın sipariş, iade, fatura, ürün bağlantıları ve eşitleme kayıtları kalıcı silinir.</span><label>Onay için <b>{item.displayName}</b> yazın<input value={deleteConfirmation} onChange={event => setDeleteConfirmation(event.target.value)} /></label><div><button type="button" onClick={() => { setDeleteOpen(false); setDeleteConfirmation('') }}>Vazgeç</button><button type="button" className="destructive" disabled={deleteConfirmation !== item.displayName || deepDelete.isPending} onClick={() => deepDelete.mutate()}>{deepDelete.isPending ? 'Temizleniyor…' : 'Kalıcı olarak sil'}</button></div></div>}</form>
}
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  return <div className={rootClass}><div className="page-heading"><div><Link to="/integrations" className="back">← Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    {orderReadSupported && <div className="panel"><h2>Salt-okunur sipariş eşitleme</h2><p>{stageManual ? 'Aktif Stage bağlantısından siparişleri okuyup yerel kaydı günceller. Bu işlem dış platforma yazmaz.' : 'Doğrulanmış sipariş okuma desteğiyle platform siparişlerini yerel kayda alır. Bu işlem dış platforma yazmaz.'}</p><div className="button-row"><button disabled={run.isPending} onClick={() => run.mutate({ path: `/connections/${id}/order-sync-jobs`, body: { externalOrderId: null } })}>{run.isPending ? 'Kuyruğa alınıyor…' : 'Yeni değişiklikleri eşitle'}</button><button className="secondary" disabled={run.isPending} onClick={() => run.mutate({ path: `/connections/${id}/order-sync-jobs`, body: { externalOrderId: null, full: true } })}>{run.isPending ? 'Kuyruğa alınıyor…' : 'Erişilebilir tüm siparişleri tara'}</button></div><small>Normal eşitleme son başarılı watermark’tan güvenlik örtüşmesiyle devam eder. Trendyol Stream’in 14 günlük pencere sınırı aşılmışsa erişilebilir geçmiş güvenli parçalara bölünür.</small><form className="inline-form" onSubmit={refreshSingleOrder}><label>Sipariş numarasıyla yenile<input name="orderNumber" inputMode="numeric" pattern="[0-9]+" placeholder="Örn. 1238693012" required /></label><button className="secondary" disabled={run.isPending}>{run.isPending ? 'Kuyruğa alınıyor…' : 'Yalnız bu siparişi yenile'}</button></form></div>}
    {item.platformCode === 'TRENDYOL' && readConnectionReady && <div className="panel sync-policy-panel"><h2>Otomatik veritabanı eşitleme</h2><p>Sipariş, iade, fatura ve ürün ekranları Trendyol’a canlı sorgu göndermez; yerel veritabanındaki kayıtları okur. Aşağıdaki işler yalnız arka planda Trendyol değişikliklerini veritabanına işler.</p>{syncPolicies.isLoading ? <Busy text="Eşitleme ayarları yükleniyor…" /> : syncPolicies.isError ? <ErrorBox error={syncPolicies.error} /> : <div className="sync-policy-list">{syncPolicies.data?.filter(policy => ['ORDERS', 'RETURNS', 'PRODUCTS'].includes(policy.resourceType)).map(policy => <form className="sync-policy-row" key={policy.id} onSubmit={event => updateSyncPolicy(event, policy)}><div className="sync-policy-identity"><strong>{policy.resourceType === 'ORDERS' ? 'Siparişler ve paket durumları' : policy.resourceType === 'RETURNS' ? 'İade talepleri' : 'Ürün kataloğu'}</strong><small>Son başarılı çalışma: <DateText value={policy.lastSuccessAt} /></small>{policy.resourceType === 'ORDERS' && <small>Değişiklik watermark: <DateText value={policy.lastModifiedWatermark} /></small>}</div><label>Kontrol aralığı<select name="intervalSeconds" defaultValue={policy.intervalSeconds}>{![60, 120, 300, 600, 900].includes(policy.intervalSeconds) && <option value={policy.intervalSeconds}>{Math.round(policy.intervalSeconds / 60)} dakika</option>}{[[60, '1 dakika'], [120, '2 dakika'], [300, '5 dakika'], [600, '10 dakika'], [900, '15 dakika']].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>{policy.resourceType === 'ORDERS' ? <label>Güvenlik örtüşmesi<select name="overlapSeconds" defaultValue={policy.overlapSeconds}>{![60, 120, 300, 600, 900].includes(policy.overlapSeconds) && <option value={policy.overlapSeconds}>{Math.round(policy.overlapSeconds / 60)} dakika</option>}{[[60, '1 dakika'], [120, '2 dakika'], [300, '5 dakika'], [600, '10 dakika'], [900, '15 dakika']].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label> : <input type="hidden" name="overlapSeconds" value={policy.overlapSeconds} />}<label className="sync-policy-toggle"><input type="checkbox" name="enabled" defaultChecked={policy.enabled} />Aktif</label><button disabled={saveSyncPolicy.isPending}>{saveSyncPolicy.isPending ? 'Kaydediliyor…' : 'Kaydet'}</button></form>)}</div>}</div>}
    {productReadSupported && <div className="panel"><h2>Salt-okunur ürün eşitleme</h2><p>Bu Trendyol mağazasındaki onaylı ürünleri, varyantları, seçenekleri ve görsel adreslerini yerel kataloğa alır. Trendyol’a dış yazma yapılmaz.</p><button disabled={run.isPending} onClick={() => run.mutate({ path: `/connections/${id}/product-sync-jobs` })}>{run.isPending ? 'Kuyruğa alınıyor…' : 'Ürünleri panele çek'}</button><small>İlk tarama tüm erişilebilir ürünleri sayfa sayfa alır; sonraki taramalar değişiklik watermark’ından güvenlik örtüşmesiyle devam eder.</small></div>}
...
export function OrdersPage() {
  const client = useQueryClient()
  const [searchParams, setSearchParams] = useSearchParams(); const requestedSearch = searchParams.get('search') ?? ''; const initialFilters = { ...initialOrderFilters, search: requestedSearch }; const [filterForm, setFilterForm] = useState<OrderFilters>(initialFilters); const [filters, setFilters] = useState<OrderFilters>(initialFilters); const [advancedFilters, setAdvancedFilters] = useState(false); const [pageSize, setPageSize] = useState(50); const [page, setPage] = useState(1); const [selectedIds, setSelectedIds] = useState<string[]>([]); const [menu, setMenu] = useState<{ orderId: string; kind: 'invoice' | 'actions' } | null>(null); const [bulkOpen, setBulkOpen] = useState(false); const [bulkNotice, setBulkNotice] = useState(''); const [invoiceInfoOrder, setInvoiceInfoOrder] = useState<Order | null>(null); const [invoiceViewerOrder, setInvoiceViewerOrder] = useState<Order | null>(null); const [invoiceDraftOrder, setInvoiceDraftOrder] = useState<Order | null>(null); const [invoiceUploadOrder, setInvoiceUploadOrder] = useState<Order | null>(null); const [courierOrder, setCourierOrder] = useState<Order | null>(null); const [singleSyncOpen, setSingleSyncOpen] = useState(false); const [previewImage, setPreviewImage] = useState<{ url: string; title: string } | null>(null)


  const orderQueries = useQueries({ queries: orderSyncStatuses.map(status => ({ queryKey: ['orders', status], queryFn: () => loadOrdersByStatus(status) })) })
  const summary = useQuery({ queryKey: ['orders', 'summary'], queryFn: () => hubApi<OrderSummary>('/orders/summary') })
  ...
  const statuses = [
    ...
    ['ON_HOLD', 'Askıdaki Siparişler'],
    ['MANUAL_REVIEW', 'Kontrol Gereken']
  ] as const

  const allById = new Map<string, Order>()
  for (const item of orderQueries.flatMap(result => result.data ?? [])) allById.set(item.id, allById.has(item.id) ? mergeOrderRows(allById.get(item.id)!, item) : item)
  const all = Array.from(allById.values()); const ordersLoading = orderQueries.some(result => result.isPending); const ordersError = orderQueries.find(result => result.isError)?.error; const normalized = filters.search.trim().toLocaleLowerCase('tr-TR')
  ...
  const matchTab = (status: string, tab: string) => {
    ...
    if (tab === 'PROCESSING') return ['PROCESSING', 'READY_TO_SHIP'].includes(status);
    if (tab === 'SHIPPED') return ['SHIPPED', 'UNDELIVERED'].includes(status);
    if (tab === 'DELIVERED') return status === 'DELIVERED';
    if (tab === 'CANCELLED') return status === 'CANCELLED';
    if (tab === 'RETURNED') return status === 'RETURNED';
    if (tab === 'RESENT') return false;
    if (tab === 'ON_HOLD') return status === 'ON_HOLD';
    if (tab === 'MANUAL_REVIEW') return status === 'MANUAL_REVIEW';
    return false;
  }
  const packageRows = all.flatMap(order => (order.packages ?? []).map(pack => ({ order, pack })))
  const matchesOrderTab = (item: Order, tab: string) => item.packages?.length ? item.packages.some(pack => tab === 'RESENT' ? pack.isResend : matchTab(pack.status.toUpperCase(), tab)) : matchTab(item.derivedStatus.toUpperCase(), tab)
  const tabCount = (tab: string) => {
    const summaryKey = ({ ALL: 'all', NEW: 'new', PROCESSING: 'processing', SHIPPED: 'shipped', DELIVERED: 'delivered', RESENT: 'resent', ON_HOLD: 'onHold' } as const)[tab as 'ALL' | 'NEW' | 'PROCESSING' | 'SHIPPED' | 'DELIVERED' | 'RESENT' | 'ON_HOLD']
    if (summary.data && summaryKey) return summary.data[summaryKey]
    return tab === 'ALL' ? packageRows.length : packageRows.filter(row => tab === 'RESENT' ? row.pack.isResend : matchTab(row.pack.status.toUpperCase(), tab)).length
  }
  ...
  async function processSingleOrder(item: Order) {
    ...
    if (!pack) {
      if (!item.connectionId) {
        setBulkNotice('Bu sipariş için aktif bağlantı veya kargo paketi bulunamadı.')
        return
      }
      try {
        setBulkNotice(`Sipariş #${item.orderNumber} paket bilgisi yenileniyor ve işleme alınıyor…`)
        await hubApi(`/connections/${item.connectionId}/order-sync-jobs`, {
          method: 'POST',
          headers: { 'Idempotency-Key': `sync-pick:${item.orderNumber}:${Date.now()}` },
          body: JSON.stringify({ externalOrderId: item.orderNumber })
        })
        await client.invalidateQueries({ queryKey: ['orders'] })
        window.setTimeout(() => void client.invalidateQueries({ queryKey: ['orders'] }), 3500)
      } catch (error) {
        setBulkNotice(error instanceof Error ? error.message : 'Sipariş paket bilgisi yenilenemedi.')
      }
      return
    }
    try {
      await hubApi(`/shipments/${pack.id}/actions`, {
        method: 'POST',
        headers: {
          'Idempotency-Key': `picking:${pack.id}:${Date.now()}`,
          'If-Match': `"v${pack.version}"`
        },
        body: JSON.stringify({ action: 'PICKING', payloadJson: '{}' })
      })
      setBulkNotice(`Sipariş #${item.orderNumber} için gerçek Trendyol Picking isteği kuyruğa alındı; uzak durum yeniden okunarak doğrulanacak.`)
      await client.invalidateQueries({ queryKey: ['orders'] })
      window.setTimeout(() => void client.invalidateQueries({ queryKey: ['orders'] }), 2500)
    } catch (error) {
      setBulkNotice(error instanceof Error ? error.message : 'Sipariş işleme alınamadı.')
    }
  }
...
function ReturnReferenceRow({ item, order }: { item: ReturnClaim; order: Order | null }) {
  ...
  return <><Link to={`/returns/${item.id}`} className="return-reference-row" role="row">
    ...
    <span className="order-reference-products return-order-products">{lines.length ? lines.map(line => { const imageUrl = line.imageUrl ?? item.primaryImageUrl; return <article key={line.id}><span className="reference-product-media">{imageUrl ? <button type="button" className="product-image-button" onClick={event => { event.preventDefault(); event.stopPropagation(); setPreviewImage({ url: imageUrl, title: line.title }) }} aria-label={`${line.title} görselini büyüt`}><img src={imageUrl} alt={`${line.title} ürün görseli`} loading="lazy" decoding="async" referrerPolicy="no-referrer" /></button> : <span className="reference-product-placeholder">↩</span>}<b className="quantity-bubble">{line.orderedQuantity}</b></span><div><strong>{line.title}</strong><small>Stok Kodu: {line.sku}</small>{optionRows(line.optionSignature).map(option => <small key={`${option.label}:${option.value}`}>{option.label}: {option.value}</small>)}<small>Barkod: {line.barcode ?? '—'}</small><small>Model Kodu: {line.modelCode ?? '—'}</small></div></article> }) : <div className="reference-no-product"><strong>{item.productCount} ürün</strong><small>Barkod: {item.primaryBarcode ?? '—'}</small></div>}</span>
    ...
    <span className="order-reference-cargo"><strong>{order?.packages?.[0]?.cargoProviderName ?? order?.cargoProviderName ?? item.cargoProviderName ?? 'Kargo bilgisi yok'}</strong><b>{trackingNumber ?? '—'}</b>{canTrack && <button type="button" className="return-cargo-track" onClick={event => { event.preventDefault(); event.stopPropagation(); navigate(`/shipments/${order!.packages![0].id}`) }}>Kargo takip →</button>}</span>
    ...
    <span><Badge value={item.status} />{item.actionDueAt && <small>{remainingText(item.actionDueAt)}</small>}<button type="button" className="return-detail-trigger" onClick={event => { event.preventDefault(); event.stopPropagation(); setDetailOpen(true) }}>Detaylı bilgi gör →</button></span>
  </Link>{previewImage && <div className="workspace-modal-backdrop product-image-backdrop" role="presentation" onMouseDown={() => setPreviewImage(null)}><section className="workspace-modal product-image-modal" role="dialog" aria-modal="true" aria-label={`${previewImage.title} büyük ürün görseli`} onMouseDown={event => event.stopPropagation()}><header><h2>{previewImage.title}</h2><button type="button" className="modal-close" onClick={() => setPreviewImage(null)} aria-label="Pencereyi kapat">×</button></header><img src={previewImage.url} alt={`${previewImage.title} büyük ürün görseli`} /></section></div>}{detailOpen && <div className="workspace-modal-backdrop return-detail-backdrop" role="presentation" onMouseDown={() => setDetailOpen(false)}><section className="workspace-modal return-detail-modal" role="dialog" aria-modal="true" aria-label={`${item.orderNumber} iade detayı`} onMouseDown={event => event.stopPropagation()}><header><div><p className="eyebrow">İade detayı</p><h2>Sipariş #{item.orderNumber}</h2><p>{order?.customerName ?? item.customerName}</p></div><button type="button" className="modal-close" onClick={() => setDetailOpen(false)} aria-label="Pencereyi kapat">×</button></header><div className="return-detail-summary"><div><small>Durum</small><Badge value={item.status} /></div><div><small>İade sebebi</small><strong>{item.reasonText ?? 'Belirtilmedi'}</strong></div><div><small>Kargo</small><strong>{trackingNumber ?? 'Takip numarası yok'}</strong></div><div><small>Faturalandırılmış tutar</small><strong>{money(invoicedTotal)}</strong></div></div><h3>Ürünler</h3><div className="return-detail-lines">{lines.map(line => <div key={line.id}><span>{line.imageUrl ? <img src={line.imageUrl} alt="" /> : <span className="reference-product-placeholder">↩</span>}<strong>{line.title}</strong></span><small>{line.quantity ?? line.orderedQuantity} adet · {line.sku}</small></div>)}</div><footer><button type="button" className="secondary" onClick={() => setDetailOpen(false)}>Kapat</button><Link className="button-link" to={`/returns/${item.id}`} onClick={() => setDetailOpen(false)}>İade kaydını aç</Link></footer></section></div>}</>
}
...
function CategoryMappingWorkspace() {
  ...
  const categoryScope = mapping.data?.externalId ?? externalId
  const attributeReferences = useQuery({ queryKey: ['reference-attributes', connectionId, categoryScope, 'summary'], queryFn: () => hubApi<ReferenceData>(`/reference-data/categories/${encodeURIComponent(categoryScope)}/attributes?connectionId=${encodeURIComponent(connectionId)}`), enabled: !!connectionId && !!categoryScope, retry: false })
  const attributeMappings = useQuery({ queryKey: ['attribute-mappings', connectionId, categoryScope], queryFn: () => hubApi<CatalogMapping[]>(`/mappings/attributes?connectionId=${encodeURIComponent(connectionId)}&scopeExternalId=${encodeURIComponent(categoryScope)}`), enabled: !!connectionId && !!categoryScope, retry: false })
  ...
  const filteredCategoryLibrary = activeCategories.filter(item => !categoryLibrarySearch.trim() || item.name.toLocaleLowerCase('tr-TR').includes(categoryLibrarySearch.trim().toLocaleLowerCase('tr-TR')))
  ...
  const selectedLocal = localLeaves.find(item => item.id === localId)
  ...
  const visibleExternal = [...(selectedExternal ? [selectedExternal] : []), ...matchingExternal.filter(item => item.externalId !== selectedExternal?.externalId)].slice(0, 4)
  const requirementItems = requirements.data ?? []
  const optionRequirements = requirementItems.filter(item => item.role === 'OPTION').slice(0, 2)
  const productRequirements = requirementItems.filter(item => item.role === 'ATTRIBUTE')
  const attributeMappingByLocal = new Map((attributeMappings.data ?? []).map(item => [item.localId, item]))
  const attributeReferenceById = new Map((attributeReferences.data?.items ?? []).map(item => [item.externalId, item]))
  ...
  function openAdvanced() { setAdvancedOpen(true); window.setTimeout(() => document.getElementById('mapping-advanced-tools')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 40) }
  function exportMappings() { const rows = [['Lokal Kategori', 'Platform', 'Platform Kategorisi', 'Durum'], ...savedRows.map(row => [row.local?.path ?? row.item.localId, 'Trendyol', cleanTrendyolCategoryPath(row.external?.path ?? row.item.externalId), row.item.status])]; const csv = `\uFEFF${rows.map(row => row.map(value => `"${String(value).replaceAll('"', '""')}"`).join(';')).join('\n')}`; const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'ravencia-kategori-eslestirmeleri.csv'; anchor.click(); URL.revokeObjectURL(url) }
  return <section className="content f3 mapping-page mapping-workspace stitch-reference-mapping"><header className="mapping-reference-heading"><h1>Eşleştirme Ayarları</h1><p>Lokal kategorilerinizi pazar yeri kategorileriyle eşleştirin.</p></header><MappingViewTabs active="category" />{notice && <div role="status" className="notice">{notice}</div>}
    <article className="mapping-reference-card mapping-category-builder"><h2><span aria-hidden="true">⌘</span>1. Yeni Kategori Eşleştirme</h2><div className="mapping-reference-grid"><section className="mapping-reference-local"><label>Lokal Kategori<select aria-label="Lokal kategori" value={localId} onChange={event => { setLocalId(event.target.value); setExternalId(''); setExternalSearch(''); setNotice('') }} disabled={!connectionId}><option value="">Lokal kategori seçin...</option>{localLeaves.map(item => <option key={item.id} value={item.id}>{item.path}</option>)}</select></label><div className="mapping-local-info"><span aria-hidden="true">ⓘ</span><div><strong>Seçilen Lokal Kategori Bilgisi</strong><p>{selectedLocal ? <>{selectedLocal.path} kategorisi hedef pazaryeri kategorisiyle eşleştirilecek. Bu kategoriye bağlı ürünler yayın sırasında seçilen karşılığı kullanır.</> : 'Eşleştirme kapsamını görmek için bir lokal kategori seçin.'}</p></div></div><details className="mapping-local-manager"><summary>Lokal kategorileri yönet</summary><form className="platform-category-create" onSubmit={event => { event.preventDefault(); createCategory.mutate() }}><label>Yeni lokal kategori<input aria-label="Yeni panel kategorisi adı" maxLength={160} value={categoryName} onChange={event => setCategoryName(event.target.value)} placeholder="Örn. Anne Bluz" required /></label><button disabled={createCategory.isPending}>{createCategory.isPending ? 'Ekleniyor…' : '+ Kategori ekle'}</button></form><div className="category-library-tools"><input aria-label="Eklenen panel kategorilerinde ara" value={categoryLibrarySearch} onChange={event => setCategoryLibrarySearch(event.target.value)} placeholder="Kategori ara…" /><span>{filteredCategoryLibrary.length.toLocaleString('tr-TR')} sonuç</span></div><div className="category-chip-list">{filteredCategoryLibrary.slice(0, 100).map(category => <span key={category.id} className={category.id === localId ? 'active' : ''}><button type="button" onClick={() => { setLocalId(category.id); setExternalId('') }}>{category.name}</button><button type="button" className="category-chip-remove" aria-label={`${category.name} kategorisini kaldır`} onClick={() => archiveCategory.mutate(category)} disabled={archiveCategory.isPending}>×</button></span>)}</div></details></section><section className="mapping-reference-target"><label>Hedef Pazar Yeri &amp; Kategori</label><div className="mapping-target-controls"><select aria-label="Hedef pazaryeri bağlantısı" value={connectionId} onChange={event => { setConnectionId(event.target.value); setLocalId(''); setExternalId(''); setNotice('') }}><option value="">Trendyol bağlantısı</option>{trendyolConnections.map(item => <option value={item.id} key={item.id}>Trendyol{trendyolConnections.length > 1 ? ` · ${item.displayName}` : ''}</option>)}</select><div className="mapping-target-search"><span aria-hidden="true">⌕</span><input value={externalSearch} onChange={event => setExternalSearch(event.target.value)} placeholder={references.isLoading ? 'Kategoriler yükleniyor...' : 'Platform kategorisi ara...'} disabled={!localId || references.isError} /></div></div><div className="mapping-category-results" role="listbox" aria-label="Platform kategorileri">{visibleExternal.length ? visibleExternal.map(item => <button type="button" role="option" aria-selected={item.externalId === externalId} key={item.externalId} onClick={() => setExternalId(item.externalId)}><span>{item.externalId === externalId ? '◉' : '↳'}</span>{cleanTrendyolCategoryPath(item.path)}</button>) : <span>{references.isError ? 'Kategori listesi alınamadı. Bağlantı ekranından kategorileri eşitleyin.' : localId ? 'Aramayla eşleşen kategori bulunamadı.' : 'Önce lokal kategori seçin.'}</span>}</div></section></div><footer><span>{references.data ? `${externalLeaves.length.toLocaleString('tr-TR')} yaprak kategori` : 'Kategori snapshot’ı bekleniyor'}{mapping.data ? ` · Kayıt v${mapping.data.version}` : ''}</span><button type="button" disabled={!localId || !externalId || save.isPending || mapping.isLoading} onClick={() => save.mutate()}><span aria-hidden="true">↗</span>{save.isPending ? 'Kaydediliyor…' : mapping.data ? 'Eşleştirmeyi Güncelle' : 'Eşleştirmeyi Kaydet'}</button></footer></article>
    <article className="mapping-reference-card mapping-option-summary"><h2><span aria-hidden="true">☷</span>2. Ürün Seçenekleri Eşitleme</h2><div className="mapping-summary-list">{optionRequirements.length ? optionRequirements.map(requirement => { const mapped = attributeMappingByLocal.get(requirement.attributeId); const remote = mapped ? attributeReferenceById.get(mapped.externalId) : null; return <div className="mapping-option-row" key={requirement.attributeId}><div><strong>{requirement.attribute.name}{requirement.attribute.code ? ` (${requirement.attribute.code})` : ''}</strong><small>Lokal varyant tipi</small></div><span aria-hidden="true">→</span><div><strong>{remote?.name ?? 'Trendyol karşılığı bekleniyor'}</strong><small>Trendyol varyant tipi</small></div><button type="button" onClick={openAdvanced}>Değerleri Eşle ({requirement.attribute.values.filter(value => value.isActive).length})</button></div> }) : <div className="mapping-empty-row"><span>Seçili kategori için ürün seçeneği tanımlanmamış.</span><button type="button" onClick={openAdvanced}>Seçenek tanımla</button></div>}</div></article>
    <article className="mapping-reference-card mapping-attribute-summary"><h2><span aria-hidden="true">▤</span>3. Kategori Özellik Eşitleme</h2><div className="mapping-attribute-selects">{productRequirements.slice(0, 2).map(requirement => { const mapped = attributeMappingByLocal.get(requirement.attributeId); const remote = mapped ? attributeReferenceById.get(mapped.externalId) : null; return <label key={requirement.attributeId}>{requirement.attribute.name}{requirement.isRequired ? ' (Zorunlu)' : ''}<select value={mapped?.externalId ?? ''} onChange={openAdvanced}><option value="">Eşleme bekleniyor</option>{mapped && <option value={mapped.externalId}>{remote?.name ?? mapped.externalId}</option>}<option value="__manage">Eşlemeyi düzenle...</option></select></label> })}{!productRequirements.length && <div className="mapping-empty-row"><span>Seçili kategori için ürün özelliği tanımlanmamış.</span><button type="button" onClick={openAdvanced}>Özellik tanımla</button></div>}</div>{productRequirements.length > 2 && <button type="button" className="mapping-show-all" onClick={openAdvanced}>Tüm {productRequirements.length} özelliği yönet</button>}</article>
    <article className="mapping-reference-card mapping-saved-card"><header><h2>4. Kayıtlı Kategori Eşleştirmeleri</h2><div>{showSavedFilter && <input aria-label="Kayıtlı eşleştirmelerde ara" value={savedSearch} onChange={event => setSavedSearch(event.target.value)} placeholder="Eşleştirme ara..." />}<button type="button" aria-label="Kayıtlı eşleştirmeleri filtrele" onClick={() => setShowSavedFilter(value => !value)}>≡</button><button type="button" aria-label="Kayıtlı eşleştirmeleri indir" onClick={exportMappings}>⇩</button></div></header><div className="mapping-saved-table"><table><thead><tr><th>Lokal Kategori</th><th>Platform</th><th>Platform Kategorisi</th><th>Durum</th><th>İşlemler</th></tr></thead><tbody>{savedRows.length ? savedRows.map(({ item, local, external }) => <tr key={item.id}><td>{local?.path ?? item.localId}</td><td><span className="trendyol-badge">Trendyol</span></td><td>{cleanTrendyolCategoryPath(external?.path ?? item.externalId)}</td><td><span className={`mapping-table-status ${item.status === 'VERIFIED' || item.status === 'ACTIVE' ? 'active' : 'error'}`}><i />{item.status === 'VERIFIED' || item.status === 'ACTIVE' ? 'Aktif' : item.status}</span></td><td><div className="mapping-row-actions"><button type="button" aria-label="Eşleştirmeyi düzenle" onClick={() => { setLocalId(item.localId); setExternalId(item.externalId); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>✎</button><button type="button" aria-label="Eşleştirmeyi sil" disabled={removeMapping.isPending} onClick={() => { if (window.confirm('Bu kategori eşleştirmesi kaldırılsın mı?')) removeMapping.mutate(item) }}>♜</button></div></td></tr>) : <tr><td colSpan={5}>{categoryMappings.isLoading ? 'Kayıtlı eşleştirmeler yükleniyor…' : 'Kayıtlı kategori eşleştirmesi bulunamadı.'}</td></tr>}</tbody></table></div><footer><span>Toplam {savedMappings.length.toLocaleString('tr-TR')} eşleştirme</span><div><button type="button" disabled>‹</button><button type="button" className="active">1</button><button type="button" disabled>›</button></div></footer></article>
    <details id="mapping-advanced-tools" className="mapping-advanced-tools" open={advancedOpen} onToggle={event => setAdvancedOpen(event.currentTarget.open)}><summary>Gelişmiş seçenek ve özellik yönetimi</summary>{advancedOpen && <div><CategoryRequirementBuilder categoryId={localId} categoryVersion={currentCategory.data?.version ?? null} attributes={localAttributes.data?.items.filter(item => item.isActive) ?? []} requirements={requirementItems} onNotice={setNotice} onSaved={async () => { await client.invalidateQueries({ queryKey: ['category-requirements', localId, 'mapping'] }); await client.invalidateQueries({ queryKey: ['category-requirements', localId, 'attribute-workspace'] }); await client.invalidateQueries({ queryKey: ['category', localId, 'mapping'] }); await client.invalidateQueries({ queryKey: ['attributes', 'mapping-builder'] }) }} /><CategoryAttributeWorkspace connectionId={connectionId} localCategoryId={localId} localCategories={localLeaves} localAttributes={localAttributes.data?.items.filter(item => item.isActive) ?? []} onNotice={setNotice} /></div>}</details>
  </section>
}
...
function CompactAttributeValues({ attributeId, values, onRemove }: { attributeId: string; values: LocalAttribute['values']; onRemove: (attributeId: string, valueId: string) => Promise<void> }) {
  ...
  const filtered = activeValues.filter(value => !search.trim() || value.value.toLocaleLowerCase('tr-TR').includes(search.trim().toLocaleLowerCase('tr-TR')))
  const visible = filtered.slice(0, 60)
  return <details className="attribute-values-collapsible" onClick={event => event.stopPropagation()}><summary>{activeValues.length.toLocaleString('tr-TR')} değeri göster</summary><div className="attribute-values-tools"><input aria-label="Değerlerde ara" value={search} onChange={event => setSearch(event.target.value)} placeholder="Değer ara…" /><small>{filtered.length > visible.length ? `İlk ${visible.length} / ${filtered.length} gösteriliyor` : `${filtered.length} sonuç`}</small></div><div className="attribute-value-list">{visible.map(value => <span key={value.id}>{value.value}<button type="button" aria-label={`${value.value} değerini sil`} onClick={event => { event.stopPropagation(); void onRemove(attributeId, value.id) }}>×</button></span>)}</div></details>
}
...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const productRemoteItems = remoteItems.filter(item => !optionExternalIds.has(item.externalId))
  ...
  const optionProducts = useQuery({ queryKey: ['mapping-option-products', attributeCategoryId], queryFn: async () => {
    const result = await loadAllPages<MappingProduct>('/products')
    return { items: result.items.filter(item => item.categoryId === attributeCategoryId).slice(0, 15), nextCursor: null, hasMore: false } satisfies Page<MappingProduct>
  }, enabled: !!attributeCategoryId && optionAttributes.length > 0, staleTime: 30_000 })
  const categoryProducts = (optionProducts.data?.items ?? []).filter(item => item.categoryId === attributeCategoryId).slice(0, 15)
  const productsWithOptions = categoryProducts.filter(item => (item.options?.length ?? 0) > 0).length
  const productSync = useMutation({ mutationFn: () => hubApi(`/connections/${connectionId}/product-sync-jobs`, { method: 'POST', headers: { 'Idempotency-Key': idempotency() }, body: '{}' }), onSuccess: async () => { onNotice('Ürünler salt-okunur seçenek eşitleme kuyruğuna alındı; Trendyol’a yazma yapılmaz.'); await new Promise(resolve => window.setTimeout(resolve, 2500)); await client.invalidateQueries({ queryKey: ['mapping-option-products', attributeCategoryId] }) }, onError: reason => onNotice(reason instanceof Error ? reason.message : 'Ürün seçenekleri eşitlenemedi.') })
...
function OptionMappingCard({ connectionId, categoryScope, snapshotId, localAttribute, remoteAttributes, existingMapping, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttribute: LocalAttribute; remoteAttributes: ReferenceItem[]; existingMapping: CatalogMapping | null; onNotice: (value: string) => void }) {
  ...
  return <section className={`attribute-mapping-card option-mapping-card ${mapped ? '' : 'unmapped'}`}><div className="attribute-mapping-card-head"><span className="attr-source">OPT</span><div><strong>Panel: {localAttribute.name}</strong><small>{mapped ? `Trendyol: ${selectedRemote?.name ?? existingMapping?.externalId} · eşlendi` : 'Varyant üreten panel seçeneği'}</small></div></div><div className="mapping-fields compact"><label>Trendyol seçenek/özellik karşılığı<select aria-label={`${localAttribute.name} Trendyol seçenek karşılığı`} value={externalId} onChange={event => setExternalId(event.target.value)} disabled={mapped}><option value="">Karşılık seçin</option>{remoteAttributes.map(item => <option key={item.externalId} value={item.externalId}>{item.name}{item.isRequired ? ' · zorunlu' : ''}</option>)}</select></label>{mapped ? <button type="button" className="secondary" onClick={() => void remove()}>Eşlemeyi kaldır</button> : <button type="button" onClick={() => void save()} disabled={!externalId}>Kaydet</button>}</div>{mapped && localAttribute.values.length > 0 && <><button type="button" className="value-mapping-toggle" onClick={() => setShowValues(value => !value)}>{showValues ? 'Seçenek değerlerini gizle' : `Seçenek değerlerini eşle (${localAttribute.values.length})`}</button>{showValues && selectedRemote && <AttributeValueMappingEditor connectionId={connectionId} categoryScope={categoryScope} attribute={localAttribute} externalAttributeId={selectedRemote.externalId} />}</>}</section>
}

function ProductOptionCoverage({ categoryProducts, optionAttributes, productsWithOptions, isLoading }: { categoryProducts: MappingProduct[]; optionAttributes: LocalAttribute[]; productsWithOptions: number; isLoading: boolean }) {
  const normalize = (value: string) => value.trim().toLocaleUpperCase('tr-TR')
  const axisCoverage = optionAttributes.map(attribute => {
    const count = categoryProducts.filter(product => product.options?.some(option => normalize(option.label) === normalize(attribute.name) && option.values.some(value => value.label.trim()))).length
    return { label: attribute.name, count }
  })
  return <section className="option-coverage panel"><div className="option-coverage-head"><div><h3>Ürün seçenekleri eşleme önizlemesi</h3><p>Salt-okunur ürün senkronizasyonu sonrası yerel ürünlerde oluşan seçenekleri gösterir. Kaynakta bulunmayan eksenler otomatik değerle doldurulmaz.</p></div><strong>{productsWithOptions}/{categoryProducts.length} seçenekli ürün</strong></div>{isLoading ? <Busy text="Ürün seçenekleri okunuyor…" /> : categoryProducts.length ? <><div className="option-coverage-list">{axisCoverage.map(axis => <div key={axis.label}><strong>{axis.label}</strong><span>{axis.count}/{categoryProducts.length} üründe seçenek değeri{axis.count === 0 ? ' · kaynak değeri yok' : ''}</span></div>)}</div><div className="option-coverage-list">{categoryProducts.map(product => { const productOptions = product.options ?? []; const missing = optionAttributes.filter(attribute => !productOptions.some(option => normalize(option.label) === normalize(attribute.name) && option.values.some(value => value.label.trim()))).map(attribute => attribute.name); return <div key={product.id}><strong>{product.title}</strong><span>{productOptions.length ? productOptions.map(option => `${option.label}: ${option.values.map(value => value.label).join(', ')}`).join(' · ') : 'Seçenek bekleniyor'}{missing.length ? ` · Eksik kaynak ekseni: ${missing.join(', ')}` : ''}</span></div> })}</div></> : <Empty>Seçili kategoride henüz ürün bulunamadı.</Empty>}</section>
}
