...
function CategoryMappingWorkspace() {
  ...
  const optionRequirements = requirementItems.filter(item => item.role === 'OPTION')
  ...
  function openAdvanced() { if (!localId) { setNotice('Önce bir kategori eşleştirmesi seçin.'); return }; setAdvancedOpen(true) }
...
<article id="category-value-mapping" className="mapping-reference-card mapping-attribute-summary"><div className="mapping-section-heading"><h2><span aria-hidden="true">▤</span>3. Kategori Özellik ve Değer Eşitleme</h2><button type="button" disabled={!localId} onClick={() => setInlineMappingOpen(value => !value)}>{inlineMappingOpen ? 'Eşleme alanını kapat' : 'Değer eşlemeyi aç'}</button></div><div className="mapping-attribute-preview-list">{productRequirements.slice(0, 4).map(requirement => { const mapped = attributeMappingByLocal.get(requirement.attributeId); const remote = mapped ? attributeReferenceById.get(mapped.externalId) : null; return <button type="button" key={requirement.attributeId} onClick={() => setInlineMappingOpen(true)}><span><strong>{requirement.attribute.name}</strong><small>{requirement.attribute.values.length ? `${requirement.attribute.values.length} yerel değer` : 'Serbest değer'}</small></span><span className={mapped ? 'mapped' : ''}>{remote?.name ?? 'Eşleme bekleniyor'}<small>{mapped ? 'Alan ve değerleri düzenle' : 'Trendyol karşılığını seç'}</small></span><i aria-hidden="true">›</i></button> })}{!productRequirements.length && <div className="mapping-empty-row"><span>{localId ? 'Bu kategori için ürün özelliği tanımlanmamış.' : 'Değer eşlemek için önce bir kategori seçin.'}</span><button type="button" onClick={openAdvanced}>{localId ? 'Özellik tanımla' : 'Kategori seçin'}</button></div>}</div>{productRequirements.length > 4 && <button type="button" className="mapping-show-all" onClick={() => setInlineMappingOpen(true)}>Tüm {productRequirements.length} özelliğin değerlerini eşle</button>}{inlineMappingOpen && <div className="inline-attribute-workspace"><CategoryAttributeWorkspace connectionId={connectionId} localCategoryId={localId} localCategories={localLeaves} localAttributes={localAttributes.data?.items.filter(item => item.isActive) ?? []} onNotice={setNotice} /></div>}</article>
...
<details id="mapping-advanced-tools" className="mapping-advanced-tools" open={advancedOpen} onToggle={event => setAdvancedOpen(event.currentTarget.open)}><summary><span>Kategori başlıkları ve seçenekleri</span><small>Kapat ×</small></summary>{advancedOpen && <div><section className="mapping-popup-content"><CategoryRequirementBuilder categoryId={localId} categoryVersion={currentCategory.data?.version ?? null} attributes={localAttributes.data?.items.filter(item => item.isActive) ?? []} requirements={requirementItems} onNotice={setNotice} onSaved={async () => { await client.invalidateQueries({ queryKey: ['category-requirements', localId, 'mapping'] }); await client.invalidateQueries({ queryKey: ['category-requirements', localId, 'attribute-workspace'] }); await client.invalidateQueries({ queryKey: ['category', localId, 'mapping'] }); await client.invalidateQueries({ queryKey: ['attributes', 'mapping-builder'] }) }} /></section></div>}</details>
  </section>
}
