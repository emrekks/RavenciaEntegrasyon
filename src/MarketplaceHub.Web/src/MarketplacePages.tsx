...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  return <div className={rootClass}><div className="page-heading"><div><Link to="/integrations" className="back">← Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    {item.platformCode === 'TRENDYOL' && readConnectionReady && <div className="panel sync-policy-panel"><h2>Otomatik veritabanı eşitleme</h2><p>Sipariş ve iade ekranları Trendyol’a canlı sorgu göndermez; kalıcı arka plan işleri platform değişikliklerini yerel veritabanına işler. Ürün aktarımı yalnız “Ürünleri panele çek” aksiyonuyla başlatılır.</p>{syncPolicies.isLoading ? <Busy text="Eşitleme ayarları yükleniyor…" /> : syncPolicies.isError ? <ErrorBox error={syncPolicies.error} /> : <div className="sync-policy-list">{syncPolicies.data?.filter(policy => ['ORDERS', 'RETURNS'].includes(policy.resourceType)).map(policy => {
      const intervals: Array<[number, string]> = [[30, '30 saniye'], [60, '1 dakika'], [120, '2 dakika'], [180, '3 dakika'], [300, '5 dakika'], [600, '10 dakika'], [900, '15 dakika']]
      const overlaps: Array<[number, string]> = [[120, '2 dakika'], [300, '5 dakika'], [600, '10 dakika'], [900, '15 dakika']]
      return <form className="sync-policy-row" key={policy.id} onSubmit={event => updateSyncPolicy(event, policy)}><div className="sync-policy-identity"><strong>{policy.resourceType === 'ORDERS' ? 'Siparişler ve paket durumları' : 'İade talepleri'}</strong><small>Son tamamlanan çalışma: <DateText value={policy.lastSuccessAt} /></small><small>Değişiklik watermark: <DateText value={policy.lastModifiedWatermark} /></small></div><label>Kontrol aralığı<select name="intervalSeconds" defaultValue={policy.intervalSeconds}>{!intervals.some(([value]) => value === policy.intervalSeconds) && <option value={policy.intervalSeconds}>{policy.intervalSeconds} saniye</option>}{intervals.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Güvenlik örtüşmesi<select name="overlapSeconds" defaultValue={policy.overlapSeconds}>{!overlaps.some(([value]) => value === policy.overlapSeconds) && <option value={policy.overlapSeconds}>{Math.round(policy.overlapSeconds / 60)} dakika</option>}{overlaps.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="sync-policy-toggle"><input type="checkbox" name="enabled" defaultChecked={policy.enabled} />Aktif</label><button disabled={saveSyncPolicy.isPending}>{saveSyncPolicy.isPending ? 'Kaydediliyor…' : 'Kaydet'}</button></form>
    })}</div>}</div>}
