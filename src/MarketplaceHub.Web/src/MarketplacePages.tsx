...
{exportOpen && <div className="workspace-modal-backdrop mapping-transfer-backdrop" role="presentation" onMouseDown={() => { if (!transferBusy) setExportOpen(false) }}><section className="workspace-modal mapping-transfer-modal" role="dialog" aria-modal="true" aria-labelledby="mapping-export-title" onMouseDown={event => event.stopPropagation()}><header><div><p className="eyebrow">EŞLEŞTİRME YEDEĞİ</p><h2 id="mapping-export-title">Dışa aktarılacak alanları seçin</h2><p>Seçtiğiniz bilgiler JSON yedeğine eklenir.</p></div><button type="button" className="modal-close" onClick={() => setExportOpen(false)} disabled={transferBusy} aria-label="Dışa aktarma penceresini kapat">×</button></header><div className="mapping-transfer-options"><label><input type="checkbox" checked={exportSelection.categories} onChange={event => setExportSelection(current => ({ ...current, categories: event.target.checked }))} /><span><strong>Panel kategorileri</strong><small>Paneldeki aktif kategori kayıtlarını aktarır.</small></span></label><label><input type="checkbox" checked={exportSelection.options} onChange={event => setExportSelection(current => ({ ...current, options: event.target.checked }))} /><span><strong>Ürün seçenekleri</strong><small>Renk, beden gibi option değerlerini aktarır.</small></span></label><label><input type="checkbox" checked={exportSelection.attributes} onChange={event => setExportSelection(current => ({ ...current, attributes: event.target.checked }))} /><span><strong>Kategori özellikleri ve değerleri</strong><small>Ürün özellikleri ile kayıtlı değerlerini aktarır.</small></span></label><label><input type="checkbox" checked={exportSelection.mappings} onChange={event => setExportSelection(current => ({ ...current, mappings: event.target.checked }))} /><span><strong>Kategori eşleştirmeleri</strong><small>Kategori, özellik ve değer eşleştirmelerini aktarır.</small></span></label><label className="mapping-transfer-all-option"><input type="checkbox" checked={Object.values(exportSelection).every(Boolean)} onChange={event => setExportSelection({ categories: event.target.checked, options: event.target.checked, attributes: event.target.checked, mappings: event.target.checked })} /><span><strong>Tümü</strong><small>Yukarıdaki tüm alanları tek seferde seçer.</small></span></label></div><footer><button type="button" className="secondary" onClick={() => setExportOpen(false)} disabled={transferBusy}>Vazgeç</button><button type="button" onClick={() => void exportSelectedMappings()} disabled={transferBusy || !Object.values(exportSelection).some(Boolean)}>{transferBusy ? 'Hazırlanıyor…' : 'Seçilenleri dışa aktar'}</button></footer></section></div>}
