...
function ProductOptionCoverage({ categoryProducts, productsWithOptions, isLoading }: { categoryProducts: MappingProduct[]; productsWithOptions: number; isLoading: boolean }) {
  return <section className="option-coverage panel"><div className="option-coverage-head"><div><h3>Ürün seçenekleri eşleme önizlemesi</h3><p>Salt-okunur ürün senkronizasyonu sonrası yerel ürünlerde oluşan seçenekleri gösterir.</p></div><strong>{productsWithOptions}/{categoryProducts.length} seçenekli ürün</strong></div>{isLoading ? <Busy text="Ürün seçenekleri okunuyor…" /> : categoryProducts.length ? <div className="option-coverage-list">{categoryProducts.map(product => <div key={product.id}><strong>{product.title}</strong><span>{product.options?.length ? product.options.map(option => `${option.label}: ${option.values.map(value => value.label).join(', ')}`).join(' · ') : 'Seçenek bekleniyor'}</span></div>)}</div> : <Empty>Seçili kategoride henüz ürün bulunamadı.</Empty>}</section>
}
