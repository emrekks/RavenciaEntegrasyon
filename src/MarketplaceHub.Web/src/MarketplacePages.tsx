...
function CategoryMappingWorkspace() {
  ...
  return <section className="content f3 mapping-page mapping-workspace"><div className="mapping-workspace-heading"><div><p className="eyebrow">Merkezi katalog sistemi</p><h1>Eşleştirme Merkezi</h1></div></div><MappingViewTabs active="category" />{notice && <div role="status" className="notice">{notice}</div>}
    ...
    <CategoryRequirementBuilder categoryId={localId} categoryVersion={currentCategory.data?.version ?? null} attributes={localAttributes.data?.items.filter(item => item.isActive) ?? []} requirements={requirements.data ?? []} onNotice={setNotice} onSaved={async () => { await client.invalidateQueries({ queryKey: ['category-requirements', localId, 'mapping'] }); await client.invalidateQueries({ queryKey: ['category-requirements', localId, 'attribute-workspace'] }); await client.invalidateQueries({ queryKey: ['category', localId, 'mapping'] }); await client.invalidateQueries({ queryKey: ['attributes', 'mapping-builder'] }) }} />
...
function CategoryRequirementBuilder({ categoryId, categoryVersion, attributes, requirements, onNotice, onSaved }: { categoryId: string; categoryVersion: number | null; attributes: LocalAttribute[]; requirements: CategoryRequirementView[]; onNotice: (value: string) => void; onSaved: () => Promise<void> }) {
  ...
  async function changeRole(item: CategoryRequirementView, role: 'ATTRIBUTE' | 'OPTION') {
    try {
      await persist(current.map(currentItem => currentItem.attributeId === item.attributeId ? { ...currentItem, role } : currentItem))
      setFeedback(`${item.attribute.name} ${role === 'OPTION' ? 'Seçenek Eşitleme' : 'Ürün özelliği'} olarak kaydedildi.`)
  ...
  async function addAttribute(mode: 'create' | 'values') {
    try {
      ...
      if (categoryId) await persist([...current, { attributeId: createdAttribute.id, isRequired: true, allowsCustomValue: !createdAttribute.values.length, role: 'ATTRIBUTE' }])
