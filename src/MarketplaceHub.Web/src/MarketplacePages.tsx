...
function ReferenceConnectionCard({ item, onSaved }: { item: Connection; onSaved: () => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`}>
    ...
    <div className="reference-integration-body"><div className="reference-data-point"><span>{efaturam ? 'VKN / TCKN' : 'Mağaza Kimliği (Seller ID)'}</span><strong>{item.externalStoreId || '—'}</strong></div><div className="reference-data-point"><span>{efaturam ? 'API Secret (Maskeli)' : 'API Key (Maskeli)'}</span><strong className="reference-secret"><b>••••••••••••••••</b><em>{maskedSuffix}</em></strong></div>{!efaturam && <div className="reference-data-point"><span>API Secret (Maskeli)</span><strong className="reference-secret"><b>••••••••••••••••</b><em>{maskedSuffix}</em></strong></div>}<div className="reference-data-point"><span>Ortam</span><strong>{item.environment === 'PRODUCTION' ? 'Canlı' : item.environment === 'STAGE' ? 'Stage' : item.environment || '—'}</strong></div><div className="reference-data-point"><span>API sürümü</span><strong>{item.apiVersion || '—'}</strong></div></div>
    {notice && <div className="reference-card-notice" role="status">{notice}</div>}
    <footer className="reference-integration-footer"><button type="button" className="reference-action secondary" onClick={() => setDisconnectOpen(true)} disabled={!connected || disconnect.isPending || test.isPending}>{disconnect.isPending ? 'İptal ediliyor…' : 'İptal et'}</button><button type="button" className="reference-action link-action" onClick={() => test.mutate()} disabled={test.isPending || disconnect.isPending}>{test.isPending ? 'Kontrol ediliyor…' : 'Bağlantıyı kontrol et'}</button>{!efaturam && <Link className="reference-action primary-action" to={`/integrations/${item.id}`}>Bilgileri güncelle</Link>}</footer>
