...
function CategoryMappingWorkspace() {
  ...
  return <section className="content f3 mapping-page mapping-workspace stitch-reference-mapping"><header className="mapping-reference-heading"><h1>Eşleştirme Ayarları</h1><p>Lokal kategorilerinizi pazar yeri kategorileriyle eşleştirin.</p></header><MappingViewTabs active="category" />{notice && <div role="status" className="notice">{notice}</div>}
    ...
    <details id="mapping-advanced-tools" className="mapping-advanced-tools" open={advancedOpen} onToggle={event => setAdvancedOpen(event.currentTarget.open)}><summary><span>Kategori ayarları</span><small>Kapat ×</small></summary>{advancedOpen && <div><nav className="mapping-popup-nav" aria-label="Kategori ayarları bölümleri"><button type="button" className={advancedTab === 'definitions' ? 'active' : ''} onClick={() => setAdvancedTab('definitions')}><strong>Başlık ve değerler</strong><small>Ürün özellikleri ile seçenekleri düzenle</small></button><button type="button" className={advancedTab === 'marketplace' ? 'active' : ''} onClick={() => setAdvancedTab('marketplace')}><strong>Trendyol eşleme</strong><small>Alan ve değer karşılıklarını bağla</small></button></nav><section className="mapping-popup-content">{advancedTab === 'definitions' ? <CategoryRequirementBuilder categoryId={localId} categoryVersion={currentCategory.data?.version ?? null} attributes={localAttributes.data?.items.filter(item => item.isActive) ?? []} requirements={requirementItems} onNotice={setNotice} onSaved={async () => { await client.invalidateQueries({ queryKey: ['category-requirements', localId, 'mapping'] }); await client.invalidateQueries({ queryKey: ['category-requirements', localId, 'attribute-workspace'] }); await client.invalidateQueries({ queryKey: ['category', localId, 'mapping'] }); await client.invalidateQueries({ queryKey: ['attributes', 'mapping-builder'] }) }} /> : <CategoryAttributeWorkspace connectionId={connectionId} localCategoryId={localId} localCategories={localLeaves} localAttributes={localAttributes.data?.items.filter(item => item.isActive) ?? []} onNotice={setNotice} />}</section></div>}</details>
  </section>
}
