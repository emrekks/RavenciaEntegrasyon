...
async function loadOrdersByStatus(status: string): Promise<Order[]> {
  const itemsById = new Map<string, Order>(); const seenCursors = new Set<string>(); let after: string | null = null
  while (true) {
    const params = new URLSearchParams({ limit: '200', status }); if (after) params.set('after', after)
    const page = await hubApi<Page<Order>>(`/orders?${params}`); for (const item of page.items) itemsById.set(item.id, itemsById.has(item.id) ? mergeOrderRows(itemsById.get(item.id)!, item) : item)
    if (!page.hasMore) return Array.from(itemsById.values())
...
const orderSyncStatuses = ['NEW', 'PROCESSING', 'READY_TO_SHIP', 'SHIPPED', 'UNDELIVERED', 'DELIVERED', 'ON_HOLD'] as const
...
async function loadReturns(limit: number): Promise<Page<ReturnClaim>> {
  // Keep the workspace bounded even when the marketplace contains older history.
  const page = await hubApi<Page<ReturnClaim>>(`/returns?limit=${limit}&latest=true`)
  return { items: page.items.slice(0, limit), nextCursor: null, hasMore: false }
}
async function loadAllReturns(): Promise<Page<ReturnClaim>> {
  return loadReturns(200)
}
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  return <div className={rootClass}><div className="page-heading"><div><Link to="/integrations" className="back">← Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    {orderReadSupported && <div className="panel"><h2>Salt-okunur sipariş eşitleme</h2><p>{stageManual ? 'Aktif Stage bağlantısından siparişleri okuyup yerel kaydı günceller. Bu işlem dış platforma yazmaz.' : 'Doğrulanmış sipariş okuma desteğiyle platform siparişlerini yerel kayda alır. Bu işlem dış platforma yazmaz.'}</p><div className="button-row"><button disabled={run.isPending} onClick={() => run.mutate({ path: `/connections/${id}/order-sync-jobs`, body: { externalOrderId: null } })}>{run.isPending ? 'Kuyruğa alınıyor…' : 'Son 14 günü eşitle'}</button><button className="secondary" disabled={run.isPending} onClick={() => run.mutate({ path: `/connections/${id}/order-sync-jobs`, body: { externalOrderId: null, full: true } })}>{run.isPending ? 'Kuyruğa alınıyor…' : 'Erişilebilir tüm siparişleri tara'}</button></div><small>Trendyol Stream sözleşmesi tek çağrıda en fazla 14 gün ve erişilebilir geçmişte en fazla 3 ay sunar; tarama bu sınırlar içinde tüm storefront’ları dolaşır.</small><form className="inline-form" onSubmit={refreshSingleOrder}><label>Sipariş numarasıyla yenile<input name="orderNumber" inputMode="numeric" pattern="[0-9]+" placeholder="Örn. 1238693012" required /></label><button className="secondary" disabled={run.isPending}>{run.isPending ? 'Kuyruğa alınıyor…' : 'Yalnız bu siparişi yenile'}</button></form></div>}
    {productReadSupported && <div className="panel"><h2>Salt-okunur Stage ürün eşitleme</h2><p>Trendyol STAGE seller 2738’teki onaylı ürünleri, varyantları, seçenekleri ve görsel adreslerini panele alır. Trendyol’a dış yazma yapılmaz; kayıtlar yalnız yerel kataloğa yazılır.</p><button disabled={run.isPending} onClick={() => run.mutate({ path: `/connections/${id}/product-sync-jobs` })}>{run.isPending ? 'Kuyruğa alınıyor…' : 'Stage ürünlerini panele çek'}</button><small>İlk tarama tam erişilebilir ürün listesini alır; sonraki taramalar 5 dakikalık aralıkla değişen varyantları günceller.</small></div>}
...
export function OrdersPage() {
  ...
  async function processSingleOrder(item: Order) {
    ...
    try {
      ...
      setBulkNotice(`Sipariş #${item.orderNumber} Trendyol üzerinde işleme alındı (Picking).`)
      await client.invalidateQueries({ queryKey: ['orders'] })
    } catch (error) {
...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const optionProducts = useQuery({ queryKey: ['mapping-option-products', attributeCategoryId], queryFn: async () => {
    const items: MappingProduct[] = []; let after = ''
    for (let page = 0; page < 25; page++) {
      const query = new URLSearchParams({ limit: '200' }); if (after) query.set('after', after)
      const result = await hubApi<Page<MappingProduct>>(`/products?${query.toString()}`)
      items.push(...result.items.filter(item => item.categoryId === attributeCategoryId))
      if (!result.hasMore || items.length >= 15 || !result.nextCursor) break
      after = result.nextCursor
    }
    return { items, nextCursor: null, hasMore: false } satisfies Page<MappingProduct>
