...
function ProductImage({ url, fallbackUrl, alt, onClick }: { url: string | null; fallbackUrl?: string | null; alt: string; onClick: () => void }) {
  ...
  return <button type="button" className="product-image-button" onClick={onClick} aria-label={`${alt} görselini büyüt`}><img src={source} alt={alt} loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={() => source !== fallbackUrl && fallbackUrl ? setSource(fallbackUrl) : setFailed(true)} /></button>
}
