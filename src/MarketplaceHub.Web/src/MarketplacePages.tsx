...
type OrderFilters = { search: string; status: string; platform: string; listing: string; cargo: string; invoice: string; invoiceType: string; invoiceRegion: string; country: string; sort: OrderSort; dateFrom: string; dateTo: string }
...
const initialOrderFilters: OrderFilters = { search: '', status: 'ALL', platform: 'ALL', listing: 'ALL', cargo: 'ALL', invoice: 'ALL', invoiceType: 'ALL', invoiceRegion: 'ALL', country: 'ALL', sort: 'DATE_DESC', dateFrom: '', dateTo: '' }
...
function OrderReferenceRow({ item, selected, onSelect, openMenu, onMenuChange, onInvoiceCreate, onInvoiceDetails, onInvoiceUpload, onCourierChange, onProcessOrder, onPreviewImage }: { item: Order; selected: boolean; onSelect: (checked: boolean) => void; openMenu: 'invoice' | 'actions' | null; onMenuChange: (value: 'invoice' | 'actions' | null) => void; onInvoiceCreate: () => void; onInvoiceDetails: () => void; onInvoiceUpload: () => void; onCourierChange: () => void; onProcessOrder: () => void; onPreviewImage: (preview: { url: string; title: string }) => void }) {
  ...
  return <article className={`order-reference-row ${item.derivedStatus.toLowerCase()} ${item.isMicroExport ? 'micro-export' : ''} ${openMenu ? 'menu-open' : ''}`}>
    <div className="order-reference-grid">
      ...
      <div className="order-reference-actions">{item.derivedStatus === 'SHIPPED' && shipment?.cargoTrackingNumber ? <><Link className="cargo-track-action" to={`/shipments/${shipment.id}`}>Kargo Takip</Link><span className="cargo-action-unavailable">İşlem Yapılamaz</span></> : shipment?.cargoTrackingNumber ? <><Link className="cargo-track-action" to={`/shipments/${shipment.id}`}>Kargo takip linki</Link><Link to={`/shipments/${shipment.id}`}>Kargo Etiketini A4 Yazdır</Link><Link to={`/shipments/${shipment.id}`} title="Trendyol ortak etiket çıktısı ZPL olarak desteklendiğinde açılır">Kargo Etiketini Sticker Yazdır</Link></> : <>{shipment ? <Link className="cargo-track-pending" to={`/shipments/${shipment.id}`} title="Takip numarası oluştuğunda bu paket ekranından izlenebilir">Kargo takip linki</Link> : <Link className="cargo-track-pending" to={`/orders?search=${encodeURIComponent(item.orderNumber)}`}>Kargo takip linki</Link>}<small>Takip numarası bekleniyor</small></>}<div className="row-menu"><button type="button" className="order-action-menu row-menu-trigger" onClick={event => toggleMenu('actions', event)} aria-expanded={openMenu === 'actions'}><span>İşlemler</span><b aria-hidden="true">⌄</b></button>{openMenu === 'actions' && <div className={`row-popover action-popover opens-${menuPlacement}`} role="menu">{item.derivedStatus.toUpperCase() === 'NEW' ? <button type="button" role="menuitem" onClick={onProcessOrder}>İşleme Al</button> : <button type="button" role="menuitem" disabled>İşleme Al</button>}<button type="button" role="menuitem" onClick={onCourierChange}>Başka Kargo Firması İle Gönder</button><button type="button" role="menuitem" disabled>İptal Et</button></div>}</div></div>
    </div>
  </article>
...
/* Legacy inline connection editor superseded by ReferenceConnectionCard.
...
export function OrdersPage() {
  ...
  const getCargoName = (item: Order) => item.packages?.[0]?.cargoProviderName ?? item.cargoProviderName ?? ''
  ...
  const getAddressCountry = (json: string) => {
    try {
      const address = JSON.parse(json) as Record<string, unknown>
      const country = [address.countryCode, address.country, address.countryName].find(value => typeof value === 'string' && value.trim())
      return typeof country === 'string' ? country.trim().toUpperCase() : 'TR'
    } catch {
      return 'TR'
    }
  }
  const getOrderCountry = (item: Order) => getAddressCountry(item.shipmentAddressJson || item.invoiceAddressJson)
  const countries = Array.from(new Set(all.map(getOrderCountry))).sort()
  const countryLabel = (country: string) => country === 'TR' || country === 'TURKEY' || country === 'TÜRKİYE' ? 'Türkiye' : country
  ...
  const items = all.filter(item => {
    ...
    return matchesOrderTab(item, filters.status) && (filters.platform === 'ALL' || item.platformCode === filters.platform) && (filters.listing === 'ALL' || (filters.listing === 'OPEN' ? !isClosed : isClosed)) && (filters.cargo === 'ALL' || getCargoName(item) === filters.cargo) && (filters.invoice === 'ALL' || item.invoiceStatus === filters.invoice) && (filters.invoiceType === 'ALL' || getInvoiceType(item) === filters.invoiceType) && (filters.invoiceRegion === 'ALL' || getInvoiceRegion(item) === filters.invoiceRegion) && (filters.country === 'ALL' || getOrderCountry(item) === filters.country) && (!from || ordered >= from) && (!to || ordered <= to) && (!normalized || searchValues.some(value => value.toLocaleLowerCase('tr-TR').includes(normalized)))
  }).sort((left, right) => {
    if (filters.sort === 'DATE_ASC' || filters.sort === 'DATE_DESC') {
      const delta = new Date(left.orderedAt).getTime() - new Date(right.orderedAt).getTime()
      return filters.sort === 'DATE_ASC' ? delta : -delta
    }
    const leftDue = left.shipmentDueAt ? new Date(left.shipmentDueAt).getTime() : Number.POSITIVE_INFINITY
    const rightDue = right.shipmentDueAt ? new Date(right.shipmentDueAt).getTime() : Number.POSITIVE_INFINITY
    if (!Number.isFinite(leftDue) && !Number.isFinite(rightDue)) return 0
    if (!Number.isFinite(leftDue)) return 1
    if (!Number.isFinite(rightDue)) return -1
    const delta = leftDue - rightDue
    return filters.sort === 'DUE_ASC' ? delta : -delta
  })
  ...
  useEffect(() => {
    const closeColumnFilter = (event: PointerEvent) => {
      if (!(event.target instanceof Element) || !event.target.closest('.order-column-filter')) setColumnFilterOpen(null)
    }
