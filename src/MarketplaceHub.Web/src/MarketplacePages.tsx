...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const currentPage = section === 'attributes' ? 1 : 2
  const pageTitle = section === 'attributes' ? 'Ürün özellikleri' : 'Seçenek eşleştirme'
  return <article className="panel mapping-step"><div className="editor-section-title"><span>4</span><div><h2>Kategori özellikleri</h2><p>Trendyol kategori alanlarını iki adımda eşleyin: ürün özellikleri ve seçenekler.</p></div></div><label className="matched-category-select">Eşleşmiş panel kategorisi<select value={attributeCategoryId} onChange={event => { setAttributeCategoryId(event.target.value); setSection('attributes') }}>{matchedCategories.map(({ category }) => <option key={category.id} value={category.id}>{category.path}</option>)}</select></label>{references.isLoading || mappings.isLoading || categoryRequirements.isLoading ? <Busy text="Kategori özellikleri ve eşlemeler yükleniyor…" /> : references.isError || mappings.isError || categoryRequirements.isError ? <div className="unknown"><strong>Güncel kategori özellik verisi alınamadı</strong><p>Seçili Trendyol kategorisinin özellik listesini yeniden eşitleyin.</p><button type="button" disabled={sync.isPending} onClick={() => sync.mutate()}>{sync.isPending ? 'Kuyruğa alınıyor…' : 'Kategori özelliklerini eşitle'}</button></div> : <><div className={`mapping-progress ${mappedRequired === requiredItems.length ? 'complete' : ''}`}><strong>{mappedRequired}/{requiredItems.length} ürün özelliği zorunlusu eşlendi</strong><span>{remoteItems.length} toplam alan · {productMappings.length} ürün özelliği · {optionMappings.length}/{optionAttributes.length} seçenek eşlemesi · Trendyol kategori no: {categoryScope}</span></div><nav className="mapping-page-stepper" aria-label="Kategori özellik eşleme adımları"><button type="button" className={`mapping-page-step ${section === 'attributes' ? 'active' : ''}`} aria-current={section === 'attributes' ? 'step' : undefined} onClick={() => setSection('attributes')}><span>1</span><span><strong>Ürün özellikleri</strong><small>Trendyol alanlarını eşle</small></span></button><span className="mapping-page-stepper-line" aria-hidden="true" /><button type="button" className={`mapping-page-step ${section === 'options' ? 'active' : ''}`} aria-current={section === 'options' ? 'step' : undefined} onClick={() => setSection('options')}><span>2</span><span><strong>Seçenek eşleştirme</strong><small>Renk ve beden karşılıkları</small></span></button></nav><div className="mapping-section-toolbar mapping-page-toolbar"><div className="mapping-page-toolbar-title"><span>Adım {currentPage} / 2</span><strong>{pageTitle}</strong><small>{section === 'attributes' ? 'Zorunlu ve isteğe bağlı Trendyol alanlarını eşleyin.' : 'Panel seçeneklerinin Trendyol karşılıklarını ve değerlerini yönetin.'}</small></div><div className="button-row"><button type="button" className="secondary" disabled={sync.isPending} onClick={() => sync.mutate()}>{sync.isPending ? 'Alanlar yenileniyor…' : 'Kategori alanlarını yenile'}</button>{section === 'options' && optionAttributes.length > 0 && <button type="button" disabled={productSync.isPending} onClick={() => productSync.mutate()}>{productSync.isPending ? 'Ürünler eşitleniyor…' : 'Ürün seçeneklerini salt-okunur eşitle'}</button>}</div></div><div className="mapping-page-content" role="tabpanel" aria-label={pageTitle}>{section === 'attributes' ? <div className="embedded-attribute-grid">{productRemoteItems.map(item => <CategoryAttributeCard key={item.externalId} connectionId={connectionId} categoryScope={categoryScope} snapshotId={references.data!.snapshotId} localAttributes={productAttributes} remoteAttribute={item} existingMapping={productMappingByExternal.get(item.externalId) ?? null} usedLocalIds={usedProductLocalIds} onNotice={onNotice} />)}</div> : <>{optionAttributes.length ? <div className="embedded-attribute-grid option-mapping-grid">{optionAttributes.map(attribute => <OptionMappingCard key={attribute.id} connectionId={connectionId} categoryScope={categoryScope} snapshotId={references.data!.snapshotId} localAttribute={attribute} remoteAttributes={remoteItems.filter(item => !productMappingByExternal.has(item.externalId))} existingMapping={mappingByLocal.get(attribute.id) ?? null} onNotice={onNotice} />)}</div> : <div className="unknown"><strong>Seçenek eşleme tanımlı değil</strong><p>3. adımda en fazla iki seçenek başlığı oluşturun veya ürün özelliklerinden taşıyın.</p></div>}{optionAttributes.length > 0 && <ProductOptionCoverage categoryProducts={categoryProducts} optionAttributes={optionAttributes} productsWithOptions={productsWithOptions} isLoading={optionProducts.isLoading} />}</>}</div><footer className="mapping-page-navigation"><button type="button" className="secondary" disabled={section === 'attributes'} onClick={() => setSection('attributes')}>← Önceki</button><span><strong>Sayfa {currentPage}</strong><small> / 2</small></span><button type="button" disabled={section === 'options'} onClick={() => setSection('options')}>Sonraki →</button></footer></>}</article>
}
