...
test('stores direct-account E-Faturam credentials without exposing fiscal settings', async () => {
  ...
  expect(await screen.findByRole('heading', { name: 'E-Faturam hesap yetkisi' })).toBeInTheDocument()
  expect(screen.getByText('Yenileme gerekli')).toBeInTheDocument()
  expect(screen.getByText('E-Faturam hesap e-postası ve parolasıyla şifreli kaydı yenileyin.')).toBeInTheDocument()
  ...
  fireEvent.change(screen.getByLabelText('Hesap e-postası'), { target: { value: 'account@example.test' } })
  fireEvent.change(screen.getByLabelText('Hesap parolası'), { target: { value: 'account-password' } })
