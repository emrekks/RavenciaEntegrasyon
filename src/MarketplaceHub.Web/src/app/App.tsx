...
function Security() {
  const client = useQueryClient()
  const [settingsTab, setSettingsTab] = useState<'security' | 'database' | 'shipping' | 'appearance'>(() => {
    const tab = new URLSearchParams(window.location.search).get('tab')
    return tab === 'database' || tab === 'shipping' || tab === 'appearance' ? tab : 'security'
  })
  useEffect(() => {
