...
import { PageHeader, Tabs, type TabItem } from '../shared/ui'
import { AttributesPage, AttributeMappingPage, BrandsPage, CategoriesPage, ImportDetailPage, ImportsPage, InventoryPage, NewProductPage, ProductDetailPage, ProductsPage, IntegrationDetailPage, IntegrationsPage, MappingPage, OrdersPage, ReturnsPage, ShipmentDetailPage, ShipmentsPage, BillingSettingsPage, JobsPage } from './route-components'
...
function Dashboard({ me }: { me: Me }) {
  ...
  const revenueAxisStep = dashboardNiceAxisStep(maxRevenue)
  const revenueAxisMax = revenueAxisStep * 10
  const revenueCurrency = revenueSeries.find(item => item.amount > 0)?.currency || revenueSeries[0]?.currency || 'TRY'
  const revenueAxisTicks = Array.from({ length: 21 }, (_, index) => {
    const ratio = index / 20
    return { ratio, amount: revenueAxisMax * ratio, major: index % 2 === 0 }
  })
...
type SettingsTab = 'security' | 'database' | 'shipping' | 'appearance'

const settingsTabItems: TabItem[] = [
  { value: 'security', label: 'Güvenlik ve oturumlar' },
  { value: 'database', label: 'Veritabanı temizliği' },
  { value: 'shipping', label: 'Kargo ayarları' },
  { value: 'appearance', label: 'Görünüm' }
]

function SettingsPageHeader({ active, onChange, description = 'Güvenlik ve yerel operasyon verilerini tek ekrandan yönetin.' }: { active: SettingsTab; onChange: (tab: SettingsTab) => void; description?: string }) {
  return <><PageHeader className="page-heading" eyebrow="Ayarlar" title="Sistem ayarları" description={description} /><Tabs className="settings-tabs" items={settingsTabItems} value={active} onChange={value => onChange(value as SettingsTab)} ariaLabel="Sistem ayarları" /></>
}
...
function AppearanceSettingsPage() {
  ...
  return <section className="content security-page">
    <SettingsPageHeader active="appearance" onChange={tab => navigate(tab === 'security' ? '/settings' : `/settings?tab=${tab}`)} />
