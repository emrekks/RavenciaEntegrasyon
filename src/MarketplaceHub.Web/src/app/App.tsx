...
function Shell({ me }: { me: Me }) {
  ...
  const navigationGroups: Array<{ label: string; items: ReactNode[] }> = [
    ...
    { label: 'Operasyon', items: [item('/inventory', 'stock', 'Stok yönetimi'), item('/returns', 'returns', 'İadeler'), item('/invoices', 'invoiceDue', 'Faturalar')] },
  ...
  return <div className={`app-shell stitch-shell ${menuCollapsed ? 'sidebar-collapsed' : ''} ${sidebarPinned ? 'sidebar-pinned' : ''}`}>
    <aside aria-label="Ravencia ana menüsü">
      <div className="sidebar-brand-row"><div className="stitch-brand-mark" aria-hidden="true">R</div><div className="brand wordmark"><strong>Ravencia <span className="brand-edition">NEXT</span></strong><small>MarketplaceHub</small></div><button type="button" className={`sidebar-pin-toggle ${sidebarPinned ? 'is-pinned' : ''}`} aria-label={sidebarPinned ? 'Menü sabitlemesini kaldır' : 'Menüyü aç ve sabitle'} aria-pressed={sidebarPinned} title={sidebarPinned ? 'Menü sabitlendi' : 'Menüyü aç ve sabitle'} onClick={toggleSidebarPinned}><UiIcon name="pin" size={18} /></button></div>
      <Link className="workspace-switch" to="/settings" aria-label="Çalışma alanı ayarlarını aç"><span className="workspace-switch-icon"><UiIcon name="layout" size={18} /></span><span className="workspace-switch-copy"><strong>Ravencia</strong><small>Çalışma alanı</small></span><UiIcon className="workspace-switch-chevron" name="chevronDown" size={16} /></Link>
      <nav aria-label="Ana menü">{navigation}</nav>
      <div className="sidebar-side-bottom"><div className="settings-nav">{item('/settings', 'settings', 'Sistem Ayarları', true)}<Link className="sidebar-help-link" to="/settings" aria-label="Yardım merkezi" title="Yardım merkezi"><UiIcon className="nav-icon" name="help" size={22} /><span className="nav-label">Yardım merkezi</span></Link></div><div className="sidebar-plan-card"><div className="sidebar-plan-heading"><strong>Bir sonraki adıma hazır mısınız?</strong><UiIcon name="sparkle" size={16} /></div><p>Tüm kanallarınız için daha fazla kontrol.</p><Link className="rv-button rv-button-secondary rv-button-sm" to="/settings/billing">Planları keşfet <UiIcon name="arrowRight" size={16} /></Link></div><div className="sidebar-profile"><span className="sidebar-avatar">{initials}</span><span className="sidebar-profile-copy"><strong>{displayName}</strong><small>Çalışma alanı sahibi</small></span><button type="button" className="sidebar-profile-logout" aria-label="Oturumdan çık" title="Oturumdan çık" onClick={() => void logout()}><UiIcon name="logout" size={18} /></button></div></div>
    </aside>
    <main>
