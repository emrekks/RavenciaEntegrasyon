...
function ProductColorRows({ product, selected, onSelect, onQuickEdit }: { product: Product; selected: boolean; onSelect: () => void; onQuickEdit: (mode: QuickEditMode) => void }) {
  ...
  return <article className="product-catalog-item color-variant-item">
      <div className="product-catalog-row">
        ...
        <div className="product-list-actions"><button type="button" className="product-price-action" onClick={() => onQuickEdit('both')}>Hızlı fiyat / stok güncelle</button><Link className="product-edit-link" to={`/products/${product.id}`}>Düzenle</Link></div>
      </div>
    </article>
}
...
export function ProductDetailPage() {
  ...
  async function updateProduct(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!query.data) return; const data = new FormData(event.currentTarget); try { await hubApi(`/products/${id}`, { method: 'PATCH', headers: { 'If-Match': `"v${query.data.version}"` }, body: JSON.stringify({ title: data.get('title'), status: data.get('status'), description, brandId: query.data.brandId, categoryId: query.data.categoryId }) }); setNotice('Ürün bilgileri güncellendi.'); await refresh() } catch (error) { setNotice(error instanceof Error ? error.message : 'Ürün güncellenemedi.') } }
  ...
  return <Page title={query.data?.title ?? 'Ürün'} eyebrow="Ürün detayı">{query.isError ? <ErrorBox error={query.error} /> : !query.data ? <p>Yükleniyor…</p> : <>
    {notice && <div role="status" className="notice">{notice}</div>}<div className="detail-grid product-edit-overview"><article className="panel product-detail-hero">{query.data.primaryImageUrl ? <img src={query.data.primaryImageUrl} alt={query.data.title} /> : <span className="product-image-placeholder">Görsel yok</span>}<div><Tag>{query.data.status === 'ACTIVE' ? 'SATI�TA' : 'KAPALI'}</Tag><p>Model: {query.data.modelCode ?? '—'}</p><p>Stok: <strong>{query.data.totalStock}</strong></p><p>Başlangıç fiyatı: <strong>{money(query.data.startingPrice, query.data.currency)}</strong></p><p>Aktif platformlar: {query.data.activePlatforms?.join(', ') || '—'}</p></div></article><form className="panel product-step-card product-edit-form" onSubmit={updateProduct}><div className="editor-section-title"><span>1</span><div><h2>Temel ürün bilgileri</h2><p>Oluşturma ekranındaki alan yapısıyla ürünü düzenleyin.</p></div></div><label>Ürün adı<input name="title" defaultValue={query.data.title} required /></label><label>Satış durumu<select name="status" defaultValue={query.data.status}><option value="ACTIVE">Satışa Açık</option><option value="ARCHIVED">Satışa Kapalı</option><option value="DRAFT">Taslak</option></select></label><label>Açıklama<RichTextEditor value={description} onChange={setDescription} /></label><button>Bilgileri kaydet</button></form></div>
