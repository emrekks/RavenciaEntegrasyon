...
export function NewProductPage({ editProductId }: { editProductId?: string } = {}) {
  ...
  const publishConnections = (connections.data?.items ?? []).filter(item => item.status === 'ACTIVE')
  const platformCards = [
