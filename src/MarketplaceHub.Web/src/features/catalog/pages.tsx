...
export function ProductsPage() {
  ...
  function openProductImport() {
    ...
    setProductImportMode('EXISTING_ONLY')
  ...
  async function importProductsFromPlatforms() {
    ...
    try {
      ...
      const query = new URLSearchParams({ full: String(productImportMethod === 'BULK' && full), newOnly: String(productImportMethod === 'BULK' && newOnly), existingOnly: String(productImportMethod === 'BULK' && existingOnly), mappingOnly: String(productImportMethod === 'BULK' && mappingOnly), includeArchived: String(productImportIncludeArchived), includeDrafts: String(productImportMethod === 'BULK' && productImportIncludeArchived && selectedImportHasShopify) })
      ...
      showProductToast(`${productImportConnectionIds.length} bağlantı için ${modeLabel} çekimi kuyruğa alındı${productImportIncludeArchived ? selectedImportHasShopify ? ' · arşiv ve taslak ürünler dahil' : ' · arşiv ürünleri dahil' : ''}.`, 'success')
...
function BulkVariantPlatformPricingModal({ row, rows, platforms, selectedPlatform, saving, onClose, onSelectRow, onSelectPlatform, onSave }: { row: VariantDraft; rows: VariantDraft[]; platforms: VariantPlatformStatus[]; selectedPlatform: VariantPlatformStatus; saving: boolean; onClose: () => void; onSelectRow: (row: VariantDraft) => void; onSelectPlatform: (platform: VariantPlatformStatus) => void; onSave: (platform: VariantPlatformStatus, draft: ChannelPricingDraft) => void }) {
  ...
  const initialPlatformDrafts = () => Object.fromEntries(platforms.map(platform => [platformKey(platform), { listPrice: String(platform.listPrice ?? row.listPrice ?? ''), salePrice: String(platform.salePrice ?? row.salePrice ?? '') }])) as Record<string, ChannelPricingDraft>
  const [platformDrafts, setPlatformDrafts] = useState<Record<string, ChannelPricingDraft>>(initialPlatformDrafts)
  useEffect(() => { setPlatformDrafts(initialPlatformDrafts()) }, [row.key, platforms])
  const updatePlatformDraft = (platform: VariantPlatformStatus, field: keyof ChannelPricingDraft, value: string) => setPlatformDrafts(current => ({ ...current, [platformKey(platform)]: { ...(current[platformKey(platform)] ?? { listPrice: '', salePrice: '' }), [field]: value } }))
  ...
  return <div className="workspace-modal-backdrop variant-platform-pricing-backdrop" role="presentation" onMouseDown={() => !saving && onClose()}><section className="workspace-modal variant-platform-pricing-modal" role="dialog" aria-modal="true" aria-labelledby="variant-platform-pricing-title" onMouseDown={event => event.stopPropagation()}><header><div><p className="eyebrow">PLATFORM FİYATLARI</p><h2 id="variant-platform-pricing-title">Varyant kanal fiyatları</h2><p>{row.optionSignature} · {row.barcode || row.sku}</p></div><button type="button" className="modal-close" onClick={onClose} disabled={saving} aria-label="Fiyat penceresini kapat"><UiIcon name="close" /></button></header><div className="variant-platform-pricing-body">
    <section className="variant-platform-pricing-bulk-grid" aria-label="Platformlara göre toplu fiyat güncelleme"><div className="variant-platform-pricing-bulk-heading"><strong>Platform fiyatlarını toplu düzenle</strong><span>{rows.length} varyanta uygulanır · Panel ana fiyatı değişmez</span></div><div className="variant-platform-pricing-platform-columns">{platforms.map(platform => { const active = platform.connectionId === selectedPlatform.connectionId; const key = platformKey(platform); const platformDraft = platformDrafts[key] ?? { listPrice: '', salePrice: '' }; return <article className={`variant-platform-pricing-platform-column${active ? ' is-selected' : ''}`} key={key}><header><span className={`publish-platform-mark ${platform.platformCode.toLocaleLowerCase('tr-TR')}`}><img className={`publish-platform-logo ${platformLogoClass(platform.platformCode)}`} src={platformLogoSource(platform.platformCode) ?? '/platforms/trendyol.png'} alt="" /></span><div><strong>{marketplacePlatformName(platform)}</strong><small>{platform.isLinked ? 'Bağlı' : 'Bağlı değil'}</small></div><span className={`variant-platform-pricing-state ${platform.isLinked ? 'is-linked' : 'is-unlinked'}`} /></header><label>Liste fiyatı<input type="number" min="0" step="0.01" value={platformDraft.listPrice} onChange={event => updatePlatformDraft(platform, 'listPrice', event.target.value)} /></label><label>Satış fiyatı<input type="number" min="0" step="0.01" value={platformDraft.salePrice} onChange={event => updatePlatformDraft(platform, 'salePrice', event.target.value)} /></label><button type="button" onClick={() => { onSelectPlatform(platform); onSave(platform, platformDraft) }} disabled={saving || !platform.connectionId}>{saving ? 'Kaydediliyor…' : 'Toplu güncelle'}</button></article> })}</div></section>
    <section className="variant-platform-pricing-variant-list" aria-label="Renk ve varyant listesi"><div className="variant-platform-pricing-variant-list-head"><strong>Varyantları seç</strong><span>{rows.length} varyant · tüm platform fiyatları</span></div><div className="quick-edit-color-list variant-platform-pricing-color-list">{colorOptions.map(color => { const items = colorGroups[color]; const isOpen = openColorGroups.includes(color); return <section className={`quick-edit-color${isOpen ? ' is-open' : ''}`} key={color}><button type="button" className="quick-edit-color-toggle" aria-expanded={isOpen} onClick={() => toggleColorGroup(color)}><span><strong>{color}</strong><small>Renk varyantlarını göster</small></span><b>{items.length}</b><UiIcon name="chevronDown" /></button>{isOpen && <div className="quick-edit-color-variants" role="region" aria-label={`${color} varyantları`}>{renderVariantRows(items)}</div>}</section>})}</div></section>
  </div><footer><button type="button" className="secondary" onClick={onClose} disabled={saving}>Vazgeç</button></footer></section></div>
}
...
export function NewProductPage({ editProductId }: { editProductId?: string } = {}) {
  ...
  return <Page className={`product-add-page${editProductId ? ' product-edit-page' : ''}`} title={editProductId ? "Ürün Düzenle" : "Yeni Ürün Ekle"} eyebrow="Katalog">
    {variantPlatformPricing && selectedVariantPlatformRow && <BulkVariantPlatformPricingModal row={selectedVariantPlatformRow} rows={variantRows} platforms={selectedVariantPlatformRow.platformStatuses ?? []} selectedPlatform={variantPlatformPricing.platform} saving={variantPlatformPricingSaving} onClose={() => setVariantPlatformPricing(null)} onSelectRow={nextRow => { const nextPlatform = nextRow.platformStatuses?.find(item => item.platformCode === variantPlatformPricing.platform.platformCode) ?? nextRow.platformStatuses?.[0]; if (nextPlatform) openVariantPlatformPricing(nextRow, nextPlatform) }} onSelectPlatform={platform => openVariantPlatformPricing(selectedVariantPlatformRow, platform)} onSave={(platform, draft) => void saveVariantPlatformPricingBulk(platform, draft)} />}
