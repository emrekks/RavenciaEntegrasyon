...
export function ProductsPage() {
  ...
  const activeProductSyncJobs = useMemo(() => (productSyncJobsQuery.data ?? []).filter(job => ['TRENDYOL_PRODUCT_SYNC', 'SHOPIFY_PRODUCT_SYNC'].includes(job.jobType) && !['SUCCEEDED', 'CANCELLED', 'DEAD'].includes(job.status) && (!productImportConnectionIds.length || (job.connectionId && productImportConnectionIds.includes(job.connectionId)))), [productImportConnectionIds, productSyncJobsQuery.data])
