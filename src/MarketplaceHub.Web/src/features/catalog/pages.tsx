...
function BulkVariantPlatformPricingModal({ row, rows, platforms, saving, onClose, onSave, onSaveRow }: { row: VariantDraft; rows: VariantDraft[]; platforms: VariantPlatformStatus[]; saving: boolean; onClose: () => void; onSave: (platform: VariantPlatformStatus, draft: ChannelPricingDraft) => void; onSaveRow: (row: VariantDraft, platform: VariantPlatformStatus, draft: ChannelPricingDraft) => void }) {
  ...
  useEffect(() => { setPlatformDrafts(initialPlatformDrafts()); setMatrixDrafts(initialMatrixDrafts()) }, [row.key, rows, platforms])
  ...
  const renderMatrixHeader = (platform: VariantPlatformStatus, field: keyof ChannelPricingDraft, label: string) => {
    const key = platformKey(platform)
    const draft = platformDrafts[key] ?? { listPrice: '', salePrice: '' }
    return <div className="variant-platform-pricing-matrix-cell variant-platform-pricing-matrix-head" key={`${key}:${field}`}><div className="variant-platform-pricing-matrix-platform"><span className={`publish-platform-mark ${platform.platformCode.toLocaleLowerCase('tr-TR')}`}><img className={`publish-platform-logo ${platformLogoClass(platform.platformCode)}`} src={platformLogoSource(platform.platformCode) ?? '/platforms/trendyol.png'} alt="" /></span><strong>{marketplacePlatformName(platform)}</strong></div><span>{label}</span><input aria-label={`${marketplacePlatformName(platform)} ${label} toplu değeri`} type="number" min="0" step="0.01" value={draft[field]} onChange={event => updatePlatformDraft(platform, field, event.target.value)} /><button type="button" className="variant-platform-pricing-matrix-bulk-button" onClick={() => onSave(platform, draft)} disabled={saving || !platform.connectionId}>{saving ? 'Kaydediliyor…' : 'Toplu güncelle'}</button></div>
  }
  return <div className="workspace-modal-backdrop variant-platform-pricing-backdrop" role="presentation" onMouseDown={() => !saving && onClose()}><section className="workspace-modal variant-platform-pricing-modal" role="dialog" aria-modal="true" aria-labelledby="variant-platform-pricing-title" onMouseDown={event => event.stopPropagation()}><header><div><p className="eyebrow">PLATFORM FİYATLARI</p><h2 id="variant-platform-pricing-title">Varyant kanal fiyatları</h2><p>{row.optionSignature} · {row.barcode || row.sku}</p></div><button type="button" className="modal-close" onClick={onClose} disabled={saving} aria-label="Fiyat penceresini kapat"><UiIcon name="close" /></button></header><div className="variant-platform-pricing-body"><section className="variant-platform-pricing-matrix" aria-label="Platformlara göre varyant fiyat matrisi"><div className="variant-platform-pricing-bulk-heading"><strong>Platform fiyatlarını toplu düzenle</strong><span>{rows.length} varyanta uygulanır · Panel ana fiyatı değişmez</span></div><div className="variant-platform-pricing-matrix-scroll"><div className="variant-platform-pricing-matrix-grid" style={{ gridTemplateColumns: `minmax(220px, 1.35fr) repeat(${platforms.length * 2}, minmax(145px, 1fr))` }}><div className="variant-platform-pricing-matrix-cell variant-platform-pricing-matrix-corner"><strong>Varyant</strong><small>Model / barkod</small></div>{platforms.flatMap(platform => [renderMatrixHeader(platform, 'listPrice', 'Liste fiyatı'), renderMatrixHeader(platform, 'salePrice', 'Satış fiyatı')])}{sortedRows.flatMap(item => { return [<div className="variant-platform-pricing-matrix-cell variant-platform-pricing-matrix-row-label" key={`${item.key}:label`}><strong>{item.optionSignature || item.sku}</strong><small>{item.barcode || item.sku}</small></div>, ...platforms.flatMap(platform => { const draft = matrixDraft(item, platform); return [<div className="variant-platform-pricing-matrix-cell" key={`${item.key}:${platformKey(platform)}:list`}><input aria-label={`${item.optionSignature || item.sku} ${marketplacePlatformName(platform)} liste fiyatı`} type="number" min="0" step="0.01" value={draft.listPrice} disabled={saving || !platform.connectionId} onChange={event => updateMatrixDraft(item, platform, 'listPrice', event.target.value)} onBlur={() => onSaveRow(item, platform, matrixDraft(item, platform))} /></div>, <div className="variant-platform-pricing-matrix-cell" key={`${item.key}:${platformKey(platform)}:sale`}><input aria-label={`${item.optionSignature || item.sku} ${marketplacePlatformName(platform)} satış fiyatı`} type="number" min="0" step="0.01" value={draft.salePrice} disabled={saving || !platform.connectionId} onChange={event => updateMatrixDraft(item, platform, 'salePrice', event.target.value)} onBlur={() => onSaveRow(item, platform, matrixDraft(item, platform))} /></div>] })] })}</div></div></section></div><footer><button type="button" className="secondary" onClick={onClose} disabled={saving}>Vazgeç</button></footer></section></div>
}
...
export function NewProductPage({ editProductId }: { editProductId?: string } = {}) {
  ...
  return <Page className={`product-add-page${editProductId ? ' product-edit-page' : ''}`} title={editProductId ? "Ürün Düzenle" : "Yeni Ürün Ekle"} eyebrow="Katalog">
    {variantPlatformPricing && selectedVariantPlatformRow && <BulkVariantPlatformPricingModal row={selectedVariantPlatformRow} rows={variantRows} platforms={selectedVariantPlatformRow.platformStatuses ?? []} saving={variantPlatformPricingSaving} onClose={() => setVariantPlatformPricing(null)} onSave={(platform, draft) => void saveVariantPlatformPricingBulk(platform, draft)} onSaveRow={(targetRow, platform, draft) => void saveVariantPlatformPricingRow(targetRow, platform, draft)} />}
        ...
        {variantRows.length > 0 && <>
          <div className={`variant-filter-panel${variantFilterOpen ? ' is-open' : ''}`}>
            <div className="variant-filter-panel-head">
              <button type="button" className="variant-filter-panel-toggle" aria-expanded={variantFilterOpen} onClick={() => setVariantFilterOpen(current => !current)}>
              ...
              <div className="variant-filter-panel-summary"><b>{hasVariantFilters ? `${matchingVariantCount} varyant seçildi` : 'Tüm varyantlar seçili'}</b>{hasVariantFilters && <button type="button" className="variant-filter-clear" onClick={clearVariantFilters}>Filtreleri temizle</button>}</div>
            </div>
