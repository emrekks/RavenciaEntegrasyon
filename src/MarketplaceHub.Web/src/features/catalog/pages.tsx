...
export function ProductsPage() {
  ...
  async function importProductsFromPlatforms() {
    ...
    if (productImportMethod === 'SINGLE' && !productImportLookup.trim()) { showProductToast('Tekil çekim için Trendyol ürün linki veya model kodu girin.', 'error'); return }
    ...
    try {
      ...
      const query = new URLSearchParams({ full: String(productImportMethod === 'BULK' && full), newOnly: String(productImportMethod === 'BULK' && newOnly), existingOnly: String(productImportMethod === 'BULK' && existingOnly), includeArchived: String(productImportIncludeArchived) })
      ...
      showProductToast(`${productImportConnectionIds.length} bağlantı için ${modeLabel} çekimi kuyruğa alındı${productImportIncludeArchived ? ' · arşiv ürünleri dahil' : ''}.`, 'success')
  ...
  return <Page className="products-page" title="Ürünler" eyebrow="Katalog" action={<div className="products-page-actions page-heading-actions"><button type="button" className="button-link product-import-trigger" onClick={openProductImport}><UiIcon name="download" /> Platformdan Ürün Çek</button><Link className="button-link product-create-trigger" to="/products/new"><UiIcon name="plus" /> Yeni Ürün Ekle</Link></div>}>
   ...
   {quickEdit && <ProductQuickEditModal products={selectedProducts} connections={connections} mode={quickEdit.mode} onChanged={refresh} onResult={showProductToast} onClose={() => setQuickEdit(null)} />}
    ...
    {productImportOpen && <div className="workspace-modal-backdrop product-import-backdrop" role="presentation" onMouseDown={() => !productImporting && setProductImportOpen(false)}><section className="workspace-modal product-import-modal" role="dialog" aria-modal="true" aria-labelledby="product-import-title" onMouseDown={event => event.stopPropagation()}><header><div><p className="eyebrow">PLATFORM KATALOĞU</p><h2 id="product-import-title">Ürünleri platformdan çek</h2><p>Seçtiğiniz aktif Trendyol bağlantılarındaki ürünleri yerel kataloğa salt-okunur olarak alın.</p></div><button type="button" className="modal-close" onClick={() => setProductImportOpen(false)} disabled={productImporting} aria-label="Pencereyi kapat"><UiIcon name="close" /></button></header><div className="product-import-body"><fieldset><legend>Platform bağlantıları</legend>{connections.length ? <div className="product-import-connections">{connections.map(connection => { const selected = productImportConnectionIds.includes(connection.id); return <label key={connection.id} className={`product-import-connection${selected ? ' selected' : ''}`}><input type="checkbox" checked={selected} onChange={() => setProductImportConnectionIds(ids => selected ? ids.filter(id => id !== connection.id) : productImportMethod === 'SINGLE' ? [connection.id] : [...ids, connection.id])} /><span><strong>{connection.displayName}</strong><small>{connection.externalStoreId} · {statusLabel(connection.status)}</small></span></label> })}</div> : <p className="product-import-empty">Ürün çekmeye uygun aktif veya doğrulanmış Trendyol bağlantısı bulunamadı.</p>}</fieldset><fieldset><legend>Çekim türü</legend><label className={`product-import-mode${productImportMethod === 'BULK' ? ' selected' : ''}`}><input type="radio" name="product-import-method" value="BULK" checked={productImportMethod === 'BULK'} onChange={() => setProductImportMethod('BULK')} /><span><strong>Toplu çekim</strong><small>Seçtiğiniz bağlantıların kataloğunu toplu olarak tarar.</small></span></label><label className={`product-import-mode${productImportMethod === 'SINGLE' ? ' selected' : ''}`}><input type="radio" name="product-import-method" value="SINGLE" checked={productImportMethod === 'SINGLE'} onChange={() => { setProductImportMethod('SINGLE'); setProductImportConnectionIds(ids => ids.slice(0, 1)) }} /><span><strong>Tekil çekim</strong><small>Tek bir ürün linki veya model kodu ile yalnızca istediğiniz ürünü çeker.</small></span></label></fieldset>{productImportMethod === 'SINGLE' ? <fieldset><legend>Tekil ürün</legend><label className="product-import-mode selected"><span><strong>Trendyol ürün linki veya model kodu</strong><input className="product-import-lookup-input" value={productImportLookup} onChange={event => setProductImportLookup(event.target.value)} placeholder="https://www.trendyol.com/...-p-12345 veya MZ019" /></span></label></fieldset> : <fieldset><legend>Ürün Çekim Seçenekleri</legend><label className={`product-import-mode${productImportMode === 'FULL' ? ' selected' : ''}`}><input type="radio" name="product-import-mode" value="FULL" checked={productImportMode === 'FULL'} onChange={() => setProductImportMode('FULL')} /><span><strong>Tam tarama</strong><small>Trendyol kataloğundaki tüm ürünleri baştan çeker ve mevcut eşleşmeleri günceller.</small></span></label><label className={`product-import-mode${productImportMode === 'NEW_ONLY' ? ' selected' : ''}`}><input type="radio" name="product-import-mode" value="NEW_ONLY" checked={productImportMode === 'NEW_ONLY'} onChange={() => setProductImportMode('NEW_ONLY')} /><span><strong>Ekli olmayan ürünleri çek</strong><small>Yeni model kodlarını ve mevcut model kodlarına eklenen yeni varyantları panele alır; ekli ürünleri güncellemez.</small></span></label><label className={`product-import-mode${productImportMode === 'EXISTING_ONLY' ? ' selected' : ''}`}><input type="radio" name="product-import-mode" value="EXISTING_ONLY" checked={productImportMode === 'EXISTING_ONLY'} onChange={() => setProductImportMode('EXISTING_ONLY')} /><span><strong>Ekli olanları güncelle</strong><small>Panelde zaten bulunan model kodlarını günceller; yeni model kodlarını eklemez.</small></span></label></fieldset>}<fieldset><legend>Seçenekler</legend><label className={`product-import-mode${productImportIncludeArchived ? ' selected' : ''}`}><input type="checkbox" checked={productImportIncludeArchived} onChange={event => setProductImportIncludeArchived(event.target.checked)} /><span><strong>Arşiv ürünlerini de al</strong><small>{productImportIncludeArchived ? 'Açık: aktif ve arşivlenmiş varyantlar birlikte getirilir.' : 'Kapalı: yalnızca aktif varyantlar panele aktarılır.'}</small></span></label></fieldset>{activeProductSyncJobs.length > 0 && <section className="product-import-progress" aria-live="polite"><div className="product-import-progress-heading"><strong>Devam eden aktarmalar</strong><small>{activeProductSyncJobs.length} işlem</small></div>{activeProductSyncJobs.map(job => { const received = Math.max(0, job.progressReceived); const total = job.progressTotal != null && job.progressTotal >= received ? job.progressTotal : null; const percent = total != null && total > 0 ? Math.min(100, Math.max(0, Math.floor(received * 100 / total))) : null; const fallbackLabel = job.status === 'PENDING' ? 'Kuyrukta bekliyor' : 'Aktarım çalışıyor'; const progressLabel = job.progressLabel && job.progressTotal != null && job.progressReceived > job.progressTotal ? job.progressLabel.replace(/^[^·]+·\s*/, `${received.toLocaleString('tr-TR')} · `) : (job.progressLabel ?? fallbackLabel); return <article key={job.id}><div className="product-import-progress-top"><span>{progressLabel}</span><strong>{percent == null ? '—' : `%${percent}`}</strong></div><div className={`product-import-progress-track${percent == null ? ' indeterminate' : ''}`}><i style={percent == null ? undefined : { width: `${percent}%` }} /></div><div className="product-import-progress-bottom"><small className="product-import-progress-counters"><span>Alınan {received.toLocaleString('tr-TR')}</span><span>İşlenen {job.progressProcessed.toLocaleString('tr-TR')}</span><span>Atlanan {job.progressSkipped.toLocaleString('tr-TR')}</span><span>Hatalı {job.progressFailed.toLocaleString('tr-TR')}</span></small><button type="button" className="secondary" disabled={cancelProductSync.isPending} onClick={() => cancelProductSync.mutate(job.id)}>Durdur</button></div></article> })}</section>}</div><footer><button type="button" className="secondary" onClick={() => setProductImportOpen(false)} disabled={productImporting}>Vazgeç</button><button type="button" onClick={() => void importProductsFromPlatforms()} disabled={productImporting || !productImportConnectionIds.length}>{productImporting ? 'Kuyruğa alınıyor…' : 'Ürünleri panele çek'}</button></footer></section></div>}
