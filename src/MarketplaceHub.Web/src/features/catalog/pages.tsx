...
type ProductImportMode = 'NEW_ONLY' | 'INCREMENTAL' | 'FULL'
...
export function ProductsPage() {
  ...
  function openProductImport() {
    ...
    setProductImportIncludeArchived(false)
  ...
  async function importProductsFromPlatforms() {
    ...
    try {
      ...
      const query = new URLSearchParams({ full: String(full), newOnly: String(newOnly), includeArchived: String(productImportIncludeArchived) })
      ...
      const modeLabel = productImportMode === 'NEW_ONLY' ? 'yalnızca yeni ürün' : full ? 'tam katalog güncelleme' : 'yeni ve değişen ürün'
      showProductToast(`${productImportConnectionIds.length} platform için ${modeLabel} taraması kuyruğa alındı${productImportIncludeArchived ? ' · arşiv ürünleri dahil' : ''}. İlerlemeyi bu pencereden izleyebilir veya durdurabilirsiniz.`, 'success')
