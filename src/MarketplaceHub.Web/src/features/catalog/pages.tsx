...
type ProductImportMode = 'FULL' | 'NEW_ONLY' | 'EXISTING_ONLY'
...
function ProductQuickEditModal({ products, connections, mode = 'both', onChanged, onClose, onResult }: { products: Product[]; connections: MarketplaceConnection[]; mode?: QuickEditMode; onChanged: () => Promise<unknown>; onClose: () => void; onResult?: (message: string, kind: 'success' | 'error') => void }) {
  ...
  const [openColorGroups, setOpenColorGroups] = useState<string[]>(colorOptions.length ? [colorOptions[0]] : [])
    ...
    <section className="workspace-modal product-quick-edit-modal" role="dialog" aria-modal="true" aria-label={title} onMouseDown={event => event.stopPropagation()}>
      ...
      <form onSubmit={apply}>
        <details className="quick-edit-step" open>
          <summary><span><b>1</b> Varyantları seç</span><small>{selectionDraft.length ? `${selectionDraft.length} varyant işaretlendi` : 'Önce renk, sonra beden seçin'}</small><UiIcon name="chevronDown" /></summary>
            ...
            <div className="quick-edit-color-list" aria-label="Renk varyant listesi">
              {colorOptions.map(color => {
                ...
                return <section className={`quick-edit-color${isOpen ? ' is-open' : ''}`} key={color}>
                  ...
                  {isOpen && <div className="quick-edit-color-variants" role="region" aria-label={`${colorLabels[color] || color} varyantları`}><div className="quick-edit-color-variants-head"><strong>{colorLabels[color] || color} varyantları</strong><span>A–Z sıralı · seçtiklerinize aşağıdaki adımda değişiklik uygulanır.</span></div>{renderVariantList(items)}</div>}
                </section>
              })}
            </div>
          </div>
        ...
        <details className="quick-edit-step quick-edit-pricing-step" open>
          <summary><span><b>2</b> {mode === 'stock' ? 'Stok değerini düzenle' : mode === 'price' ? 'Fiyatı düzenle' : 'Fiyat ve stok değerini düzenle'}</span><small>{activeSelection.length ? `${activeSelection.length} seçili varyanta uygulanacak` : 'Varyant seçimi bekleniyor'}</small><UiIcon name="chevronDown" /></summary>
...
export function ProductsPage() {
  ...
  async function importProductsFromPlatforms() {
    ...
    try {
      ...
      const query = new URLSearchParams({ full: String(productImportMethod === 'BULK' && full), newOnly: String(productImportMethod === 'BULK' && newOnly), existingOnly: String(productImportMethod === 'BULK' && existingOnly), includeArchived: String(productImportIncludeArchived), includeDrafts: String(productImportMethod === 'BULK' && productImportIncludeArchived && selectedImportHasShopify) })
      ...
      const modeLabel = productImportMethod === 'SINGLE' ? 'tekil ürün' : full ? 'tam katalog' : productImportMode === 'NEW_ONLY' ? 'ekli olmayan ürün' : 'ekli ürün güncelleme'
...
function BulkVariantPlatformPricingModal({ row, rows, platforms, selectedPlatform, draft, saving, onClose, onSelectRow, onSelectPlatform, onDraftChange, onSave }: { row: VariantDraft; rows: VariantDraft[]; platforms: VariantPlatformStatus[]; selectedPlatform: VariantPlatformStatus; draft: ChannelPricingDraft; saving: boolean; onClose: () => void; onSelectRow: (row: VariantDraft) => void; onSelectPlatform: (platform: VariantPlatformStatus) => void; onDraftChange: (field: keyof ChannelPricingDraft, value: string) => void; onSave: () => void }) {
  return <div className="workspace-modal-backdrop variant-platform-pricing-backdrop" role="presentation" onMouseDown={() => !saving && onClose()}><section className="workspace-modal variant-platform-pricing-modal" role="dialog" aria-modal="true" aria-labelledby="variant-platform-pricing-title" onMouseDown={event => event.stopPropagation()}><header><div><p className="eyebrow">PLATFORM FİYATLARI</p><h2 id="variant-platform-pricing-title">Varyant kanal fiyatları</h2><p>{row.optionSignature} · {row.barcode || row.sku}</p></div><button type="button" className="modal-close" onClick={onClose} disabled={saving} aria-label="Fiyat penceresini kapat"><UiIcon name="close" /></button></header><div className="variant-platform-pricing-body">
    <section className="variant-platform-pricing-editor variant-platform-pricing-bulk-editor" aria-label="Toplu platform fiyat düzenleme"><div><strong>{marketplacePlatformName(selectedPlatform)} fiyatını toplu düzenle</strong><small>{rows.length} varyanta uygulanır. Panel ana fiyatı değişmeden yalnızca bu kanal tekliflerini günceller.</small></div><div className="variant-platform-pricing-fields"><label>Liste fiyatı<input autoFocus type="number" min="0" step="0.01" value={draft.listPrice} onChange={event => onDraftChange('listPrice', event.target.value)} /></label><label>Satış fiyatı<input type="number" min="0" step="0.01" value={draft.salePrice} onChange={event => onDraftChange('salePrice', event.target.value)} /></label></div><button type="button" onClick={onSave} disabled={saving || !selectedPlatform.connectionId}>{saving ? 'Kaydediliyor…' : `Tüm ${rows.length} varyanta uygula`}</button></section>
    <div className="variant-platform-pricing-list">{platforms.map(platform => { const active = platform.connectionId === selectedPlatform.connectionId; return <article className={`variant-platform-pricing-card${active ? ' is-selected' : ''}`} key={`${platform.platformCode}:${platform.connectionId ?? platform.platform}`}><div className="variant-platform-pricing-card-head"><span className={`publish-platform-mark ${platform.platformCode.toLocaleLowerCase('tr-TR')}`}><img className={`publish-platform-logo ${platformLogoClass(platform.platformCode)}`} src={platformLogoSource(platform.platformCode) ?? '/platforms/trendyol.png'} alt="" /></span><div><strong>{marketplacePlatformName(platform)}</strong><small>{platform.isLinked ? 'Bağlı' : 'Bağlı değil'}</small></div><span className={`variant-platform-pricing-state ${platform.isLinked ? 'is-linked' : 'is-unlinked'}`} /></div><dl><div><dt>Liste</dt><dd>{marketplacePriceLabel(platform.listPrice, platform.currency)}</dd></div><div><dt>Satış</dt><dd>{marketplacePriceLabel(platform.salePrice, platform.currency)}</dd></div></dl><button type="button" className="secondary" onClick={() => onSelectPlatform(platform)} disabled={saving}>{active ? 'Düzenleniyor' : 'Düzenle'}</button></article> })}</div>
    <section className="variant-platform-pricing-variant-list" aria-label="Tüm varyantlar"><div className="variant-platform-pricing-variant-list-head"><strong>Tüm varyantlar</strong><span>{rows.length} varyant · tüm platform fiyatları</span></div><div className="variant-platform-pricing-variant-items" role="listbox" aria-label="Ürün varyantları">{rows.map(item => { const priceSummary = (item.platformStatuses ?? []).map(platform => `${marketplacePlatformName(platform)}: Liste ${marketplacePriceLabel(platform.listPrice, platform.currency)} · Satış ${marketplacePriceLabel(platform.salePrice, platform.currency)}`).join(' · ') || 'Fiyat tanımlı değil'; return <button type="button" role="option" aria-selected={item.key === row.key} className={`variant-platform-pricing-variant-item${item.key === row.key ? ' is-selected' : ''}`} key={item.key} onClick={() => onSelectRow(item)} disabled={saving} aria-label={`${item.optionSignature || item.sku} · ${priceSummary}`}><span><strong>{item.optionSignature || item.sku}</strong><small>{item.barcode || item.sku}</small><em>{priceSummary}</em></span><i>{item.platformStatuses?.length ?? 0}</i></button> })}</div></section>
...
export function NewProductPage({ editProductId }: { editProductId?: string } = {}) {
  ...
  async function saveVariantPlatformPricingBulk() {
    if (!variantPlatformPricing) return
    const selectedPlatform = variantPlatformPricing.platform
  ...
  return <Page className={`product-add-page${editProductId ? ' product-edit-page' : ''}`} title={editProductId ? "Ürün Düzenle" : "Yeni Ürün Ekle"} eyebrow="Katalog">
    {variantPlatformPricing && selectedVariantPlatformRow && <BulkVariantPlatformPricingModal row={selectedVariantPlatformRow} rows={variantRows} platforms={selectedVariantPlatformRow.platformStatuses ?? []} selectedPlatform={variantPlatformPricing.platform} draft={variantPlatformPricingDraft} saving={variantPlatformPricingSaving} onClose={() => setVariantPlatformPricing(null)} onSelectRow={nextRow => { const nextPlatform = nextRow.platformStatuses?.find(item => item.platformCode === variantPlatformPricing.platform.platformCode) ?? nextRow.platformStatuses?.[0]; if (nextPlatform) openVariantPlatformPricing(nextRow, nextPlatform) }} onSelectPlatform={platform => openVariantPlatformPricing(selectedVariantPlatformRow, platform)} onDraftChange={(field, value) => setVariantPlatformPricingDraft(current => ({ ...current, [field]: value }))} onSave={() => void saveVariantPlatformPricingBulk()} />}
