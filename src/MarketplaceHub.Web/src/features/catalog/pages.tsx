...
export function ProductsPage() {
  ...
  function openProductImport() {
    ...
    setProductImportIncludeDrafts(false)
  ...
  async function importProductsFromPlatforms() {
    ...
    if (productImportMethod === 'SINGLE' && !productImportLookup.trim()) { showProductToast('Tekil çekim için ürün linki veya model kodu girin.', 'error'); return }
    ...
    try {
      ...
      const query = new URLSearchParams({ full: String(productImportMethod === 'BULK' && full), newOnly: String(productImportMethod === 'BULK' && newOnly), existingOnly: String(productImportMethod === 'BULK' && existingOnly), includeArchived: String(productImportIncludeArchived), includeDrafts: String(productImportMethod === 'BULK' && productImportIncludeDrafts) })
      ...
      showProductToast(`${productImportConnectionIds.length} bağlantı için ${modeLabel} çekimi kuyruğa alındı${productImportIncludeArchived ? ' · arşiv ürünleri dahil' : ''}${productImportIncludeDrafts ? ' · taslak ürünler dahil' : ''}.`, 'success')
  ...
  return <Page className="products-page" title="Ürünler" eyebrow="Katalog" action={<div className="products-page-actions page-heading-actions"><button type="button" className="button-link product-import-trigger" onClick={openProductImport}><UiIcon name="download" /> Platformdan Ürün Çek</button><Link className="button-link product-create-trigger" to="/products/new"><UiIcon name="plus" /> Yeni Ürün Ekle</Link></div>}>
   ...
   {quickEdit && <ProductQuickEditModal products={selectedProducts} connections={publicationConnections} mode={quickEdit.mode} onChanged={refresh} onResult={showProductToast} onClose={() => setQuickEdit(null)} />}
