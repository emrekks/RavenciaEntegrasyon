          ...
          <div className="invoice-reference-actions">
            {item.platformCode === 'SHOPIFY' ? <>
              <div className="invoice-shopify-split" data-shopify-invoice-menu>
                {item.invoiceId && !item.canCreateInvoice ? <InvoiceStatusBadge status={invoiceWorkspaceStatus(item).value} tone={invoiceWorkspaceStatus(item).tone} /> : <span className="shopify-invoice-create-wrap" title={!item.invoiceCreationEnabled ? invoiceCreationDisabledHelp : undefined}><button type="button" className="shopify-invoice-create-trigger" aria-busy={create.isPending && create.variables?.packageId === item.packageId} disabled={create.isPending || !item.canCreateInvoice || !item.invoiceCreationEnabled} onClick={() => create.mutate(item)}>{create.isPending && create.variables?.packageId === item.packageId && <span className="invoice-action-spinner" aria-hidden="true" />}<span className="invoice-action-label">{create.isPending && create.variables?.packageId === item.packageId ? 'Hazırlanıyor…' : 'Fatura Oluştur'}</span></button></span>}
