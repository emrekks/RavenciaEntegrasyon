...
export function ReturnsPage() {
  ...
  return <section className="content f3 returns-page reference-returns-page">
    ...
    <div className="returns-reference-filter-shell"><div className="return-reference-tabs" role="tablist" aria-label="İade durumları">{tabs.map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={status === value} className={status === value ? 'active' : ''} onClick={() => selectStatus(value)}><strong>{label}</strong><small>{countsQuery.isLoading && status !== 'ALL' ? '…' : `${value === 'ALL' ? countItems.length : countItems.filter(item => returnGroup(item.status) === value).length} Paket`}</small></button>)}</div>
