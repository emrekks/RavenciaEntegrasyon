...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const webColorRemote = remoteItems.find(item => isWebColorAttributeName(item.name))
    ?? (panelColorAttribute && (remoteItems.find(item => isPanelColorAttributeName(item.name) && (item.isRequired === true || requirementOptionIds.has(panelColorAttribute.id))) ?? null))
  ...
  const optionCount = optionAttributes.length + (specialColorOption ? 1 : 0)
...
export function WebColorMappingCard({ connectionId, categoryScope, snapshotId, localAttribute, localAttributes, categoryOptionIds, remoteAttribute, existingMapping, existingMappings, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttribute: LocalAttribute; localAttributes: LocalAttribute[]; categoryOptionIds: Set<string>; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; existingMappings: CatalogMapping[]; onNotice: (value: string) => void }) {
  ...
  useEffect(() => { setLocalId(existingMapping?.localId ?? localAttribute.id); setShowValues(false) }, [existingMapping?.localId, localAttribute.id])
  ...
  const currentMapping = existingMappings.find(item => item.localId === localId) ?? (existingMapping?.localId === localId ? existingMapping : null)
...
function WebColorValueMappingEditor({ connectionId, categoryScope, attribute, externalAttributeId }: { connectionId: string; categoryScope: string; attribute: LocalAttribute; externalAttributeId: string }) {
  ...
  return <div className="web-color-value-editor"><div className="value-mapping-heading"><div><h3>Web Color değer eşleştirmesi</h3><p>Bir Web Color değeri seçin; karşısındaki panel renklerini işaretleyin. {localValues.length} panel rengi · {remoteValues.length} Web Color değeri</p></div><button type="button" disabled={saving} onClick={() => void saveAll()}>{saving ? 'Kaydediliyor…' : 'Tüm eşlemeleri kaydet'}</button></div>{notice && <p role="status" className="notice">{notice}</p>}<div className="web-color-assignment-picker"><label>Web Color değeri<select aria-label="Web Color değeri" value={selectedRemoteId} onChange={event => setSelectedRemoteId(event.target.value)}><option value="">Web Color seçin</option>{remoteValues.map(remote => <option key={remote.externalId} value={remote.externalId}>{remote.name}</option>)}</select></label><div className="web-color-assignment-current"><span>Seçili hedef</span><div className="web-color-assignment-current-box"><strong>{remoteNameById.get(selectedRemoteId) ?? 'Web Color seçin'}</strong></div></div></div><div className="web-color-panel-value-picker"><div className="web-color-panel-value-picker-head"><strong>Karşılık gelen panel renkleri</strong><span>{selectedLocalIds.size} seçildi</span></div><div className="web-color-panel-value-grid">{localValues.map(localValue => { const assignedTo = selections[localValue.id]; const selected = selectedLocalIds.has(localValue.id); return <label key={localValue.id} className={`web-color-panel-value${selected ? ' selected' : ''}`}><input type="checkbox" checked={selected} disabled={!selectedRemoteId} onChange={() => toggleLocalValue(localValue.id)} /><span>{localValue.value}</span>{assignedTo && !selected && <small>{remoteNameById.get(assignedTo) ?? 'Web Color'}</small>}</label> })}</div></div>{assignedGroups.length > 0 && <div className="web-color-assignment-groups"><strong>Kayıtlı Web Color grupları</strong><div>{assignedGroups.map(group => <p key={group.remote.externalId}><span>{group.remote.name}</span><small>{group.locals.map(value => value.value).join(' · ')}</small></p>)}</div></div>}</div>
}
