...
function Badge({ value }: { value: string }) { const normalized = value.trim().toUpperCase(); return <span className={`badge ${statusTone(value)} status-${normalized.toLowerCase()}`} title={value}><i aria-hidden="true" />{statusLabel(value)}</span> }
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  return <div className={rootClass}><div className={`page-heading ${inline ? 'integration-inline-page-heading' : ''}`}><div>{inline ? <p className="lede integration-inline-meta">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p> : <><Link to="/integrations" className="back"><UiIcon name="arrowLeft" /> Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></>}</div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    {item.platformCode === 'TRENDYOL' && readConnectionReady && <section className="panel integration-detail-section sync-policy-panel" aria-labelledby="sync-policy-title"><div className="integration-detail-section-heading"><div><p className="eyebrow">OTOMATİK AKIŞ</p><h2 id="sync-policy-title">Veri eşitleme ayarları</h2><p>Sipariş ve iade verileri arka planda yerel veritabanına alınır. Akışları kategorilerine göre inceleyin; çalışma sıklığını ve güvenlik aralığını aşağıdaki kartlardan yönetin.</p></div><span className="integration-detail-section-badge">ARKA PLANDA</span></div>{syncPolicies.isLoading ? <Busy text="Eşitleme ayarları yükleniyor…" /> : syncPolicies.isError ? <ErrorBox error={syncPolicies.error} /> : <div className="sync-policy-list">{syncPolicyGroups.map(group => <div className="sync-policy-group" key={group.key}><div className="sync-policy-group-heading"><h3>{group.label}</h3><span>{group.policies.length} akış</span></div><div className="sync-policy-group-grid">{group.policies.map(policy => {
      ...
      return <form className="sync-policy-row" key={policy.id} onSubmit={event => updateSyncPolicy(event, policy)}><div className="sync-policy-identity"><div className="sync-policy-title"><strong>{policyLabel}</strong><Badge value={policy.healthStatus ?? 'OFFLINE'} /></div><div className="sync-policy-meta"><small>Son tamamlanan: <DateText value={policy.lastSuccessAt} /></small><small>Son deneme: <DateText value={policy.lastAttemptAt ?? null} /></small><small>{policy.lastRequestCount ?? 0} istek · {policy.lastReceivedCount ?? 0} kayıt · {policy.lastFailedCount ?? 0} hata · {policy.lastRetryCount ?? 0} retry · {policy.lastRateLimitCount ?? 0} rate-limit{policy.consecutiveFailureCount ? ` · ${policy.consecutiveFailureCount} ardışık hata` : ''}</small></div>{policy.recoveryGapStatus && policy.recoveryGapStatus !== 'OK' && <small className={`sync-policy-gap sync-policy-gap-${policy.recoveryGapStatus.toLowerCase()}`}>Kurtarma aralığı: {policy.recoveryGapStatus === 'UNKNOWN' ? 'henüz watermark yok' : `${Math.round(policy.recoveryGapDays ?? 0)} gün`}</small>}</div><label><span>Kontrol aralığı</span><select name="intervalSeconds" defaultValue={policy.intervalSeconds}>{!intervals.some(([value]) => value === policy.intervalSeconds) && <option value={policy.intervalSeconds}>{policy.intervalSeconds} saniye</option>}{intervals.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span>Güvenlik örtüşmesi</span><select name="overlapSeconds" defaultValue={policy.overlapSeconds}>{!overlaps.some(([value]) => value === policy.overlapSeconds) && <option value={policy.overlapSeconds}>{Math.round(policy.overlapSeconds / 60)} dakika</option>}{overlaps.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="sync-policy-toggle"><input type="checkbox" name="enabled" defaultChecked={policy.enabled} /><span>Bu akış aktif</span></label><button className="sync-policy-save" disabled={saveSyncPolicy.isPending}>{saveSyncPolicy.isPending ? 'Kaydediliyor…' : 'Ayarı kaydet'}</button></form>
     })}</div></div>)}</div>}</section>}
