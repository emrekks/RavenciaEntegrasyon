...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  return <div className={rootClass}><div className={`page-heading ${inline ? 'integration-inline-page-heading' : ''}`}><div>{inline ? <p className="lede integration-inline-meta">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p> : <><Link to="/integrations" className="back"><UiIcon name="arrowLeft" /> Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></>}</div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    <section className="panel connection-settings-panel"><div className="connection-settings-section"><h2>Mağaza bilgileri</h2><p>Mağaza kimliği, API anahtarları ve bağlantı bilgilerini buradan güncelleyin.</p><form onSubmit={event => { event.preventDefault(); saveConnectionDetails.mutate() }}><label>{item.platformCode === 'TRENDYOL_EFATURAM' ? 'VKN / TCKN' : item.platformCode === 'SHOPIFY' ? 'Shopify mağaza adı' : 'Mağaza kimliği (Seller ID)'}<input value={externalStoreId} onChange={event => setExternalStoreId(event.target.value)} autoComplete="off" required /></label>{item.platformCode === 'SHOPIFY' && <label>Model kodu metafield’i<input value={shopifyModelCodeMetafield} onChange={event => setShopifyModelCodeMetafield(event.target.value)} pattern="[A-Za-z0-9_-]{1,80}\.[A-Za-z0-9_-]{1,80}" required /><small>Ürün eşleştirmesinde kullanılacak alan. Varsayılan: <code>ravencia.model_code</code>. SKU’dan model kodu tahmin edilmez.</small></label>}<button disabled={saveConnectionDetails.isPending}>{saveConnectionDetails.isPending ? 'Kaydediliyor…' : 'Mağaza bilgisini kaydet'}</button></form></div><div className="connection-settings-divider" /><form className="connection-settings-section" onSubmit={credential}><h2>API bilgileri</h2>{item.platformCode === 'TRENDYOL_EFATURAM' ? <><label>Hesap e-postası<input name="email" type="email" autoComplete="username" required /></label><label>Hesap parolası<input name="password" type="password" autoComplete="new-password" required /></label><p>Bu alanlar yalnız şifreli credential kaydında tutulur ve tekrar gösterilmez.</p></> : <><label>{item.platformCode === 'SHOPIFY' ? 'Shopify client ID' : 'API key'}<input name="apiKey" type="password" autoComplete="off" required /></label><label>{item.platformCode === 'SHOPIFY' ? 'Shopify client secret' : 'API secret'}<input name="apiSecret" type="password" autoComplete="new-password" required /></label>{item.platformCode === 'SHOPIFY' && <p>Client secret şifreli saklanır; token Shopify mağazasıyla aynı organizasyon için otomatik yenilenir.</p>}</>}<button disabled={run.isPending}>{run.isPending ? 'Kaydediliyor…' : 'Şifreli kaydet'}</button><button type="button" className="secondary" disabled={!item.hasCredential || run.isPending} onClick={() => run.mutate({ path: `/connections/${id}/test-jobs` })}>Bağlantıyı test et</button></form></section>
