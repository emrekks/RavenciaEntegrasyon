...
function ReturnDecisionActions({ item, onNotice }: { item: ReturnDetail; onNotice?: (message: string) => void }) {
  const client = useQueryClient(); const [action, setAction] = useState<'APPROVE' | 'REJECT'>('APPROVE'); const [reasonId, setReasonId] = useState(''); const [evidenceAssetIds, setEvidenceAssetIds] = useState<string[]>([])
  ...
  const decision = useMutation({
    mutationFn: (payload: { action: string; reasonCode: string | null; explanation: string | null }) => hubApi(`/returns/${item.id}/actions`, { method: 'POST', headers: { 'Idempotency-Key': idempotency(), 'If-Match': `"v${item.version}"` }, body: JSON.stringify({ ...payload, evidenceAssetIds }) }),
    onSuccess: async () => {
