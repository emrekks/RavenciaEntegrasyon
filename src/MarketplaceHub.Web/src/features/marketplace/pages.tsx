...
function ReturnDecisionActions({ item, onNotice, selectedLineIds: controlledSelectedLineIds, onSelectedLineIdsChange }: ReturnDecisionActionsProps) {
  ...
  return <section className={`return-inline-action return-decision-action${isControlled ? ' has-product-selection' : ''}`}><div><h3>İade işlemi</h3><p>İade paketi kontrol edildikten sonra kabul veya ret kararını buradan gönderin.</p><p className="return-decision-help">Birden fazla ürün varsa ürün kartlarına tıklayarak hangi ürünlerin işleme alınacağını seçin. Karar gönderilmeden önce seçiminizi özet popup’ında onaylayabilirsiniz.</p></div>{isControlled && <div className="return-decision-selection-note"><strong>{selectedLineIds.length} / {lines.length} ürün seçildi</strong><span>Ürün seçimini yukarıdaki kartlardan değiştirebilirsiniz.</span></div>}<form className="form-grid" onSubmit={requestDecision}>{lines.length > 1 && <fieldset className="return-line-selection"><legend>İşlem yapılacak ürünler</legend><p>Onaylanacak veya reddedilecek ürünleri seçin.</p><label className="return-line-selection-toggle"><input type="checkbox" checked={allLinesSelected} onChange={event => updateSelectedLineIds(event.target.checked ? lines.map(line => line.id) : [])} /><span>Tüm ürünleri seç</span></label><div className="return-line-selection-list">{lines.map(line => <label key={line.id}><input type="checkbox" checked={selectedLineIds.includes(line.id)} onChange={event => updateSelectedLineIds(event.target.checked ? [...selectedLineIds, line.id] : selectedLineIds.filter(id => id !== line.id))} /><span><strong>{line.title}</strong><small>{line.quantity.toLocaleString('tr-TR')} adet · {line.unitPrice.toLocaleString('tr-TR', { style: 'currency', currency: item.currency })}</small></span></label>)}</div><small className="return-decision-hint">{selectedLineIds.length} / {lines.length} ürün seçildi.</small></fieldset>}{lines.length === 1 && <div className="return-single-line-summary"><strong>İşlem yapılacak ürün</strong><span>{lines[0].title} · {lines[0].quantity.toLocaleString('tr-TR')} adet</span></div>}<label>Karar<select value={action} onChange={event => { setAction(event.target.value as 'APPROVE' | 'REJECT'); setReasonId(''); setEvidenceAssetIds([]) }}>{decisionActions.map(value => <option key={value} value={value}>{statusLabel(value)}</option>)}</select></label>{action === 'REJECT' && <><label>Trendyol ret nedeni<select value={reasonId} onChange={event => setReasonId(event.target.value)} required disabled={reasons.isLoading || reasons.isError}><option value="">{reasons.isLoading ? 'Ret nedenleri yükleniyor…' : reasons.isError ? 'Ret nedenleri alınamadı' : 'Seçin'}</option>{reasons.data?.map(reason => <option key={reason.id} value={reason.id}>{reason.name}</option>)}</select></label>{reasons.isError && <small className="return-decision-error">Trendyol ret nedenleri alınamadı. İade detayını yenileyip tekrar deneyin.</small>}{selectedReason && <small className="return-decision-hint">{selectedReason.evidenceRequired ? 'Bu ret nedeni için kanıt dosyası zorunludur.' : 'Bu ret nedeni için açıklama zorunludur.'}</small>}<label>Trendyol açıklaması<textarea name="explanation" maxLength={500} placeholder="Ret kararının nedenini Trendyol’a iletin…" required /></label><label>Kanıt (PDF/JPEG/PNG, en fazla 10 MiB)<input name="file" type="file" accept="application/pdf,image/jpeg,image/png" onChange={event => { const file = event.currentTarget.files?.[0]; if (file) void uploadFile(file) }} /></label><small>{evidenceAssetIds.length} kanıt hazır{selectedReason?.evidenceRequired ? ' · bu neden için zorunlu' : ''}</small></>}<button type="submit" disabled={decision.isPending || !lines.length || (action === 'REJECT' && (reasons.isLoading || reasons.isError || !reasonId || (selectedReason?.evidenceRequired && !evidenceAssetIds.length)))}>{action === 'REJECT' ? 'Ret kararını gönder' : 'İadeyi onayla'}</button></form>{confirmation && <Modal open title={confirmation.action === 'REJECT' ? 'Ret kararını onayla' : 'İade kararını onayla'} description="Aşağıdaki ürünler için işlem kuyruğa alınacak. Devam etmeden önce özeti kontrol edin." onClose={() => { if (!decision.isPending) setConfirmation(null) }} className="return-decision-confirmation" footer={<><button type="button" className="secondary" disabled={decision.isPending} onClick={() => setConfirmation(null)}>Vazgeç</button><button type="button" disabled={decision.isPending} onClick={() => decision.mutate(confirmation)}>{decision.isPending ? 'Gönderiliyor…' : 'Kararı onayla'}</button></>}><div className="return-decision-summary"><div className={`return-decision-summary-badge ${confirmation.action === 'REJECT' ? 'reject' : 'approve'}`}>{confirmation.action === 'REJECT' ? 'Reddet' : 'Onayla'}</div><h3>Seçilen ürünler</h3><div className="return-decision-summary-list">{selectedLines.map(line => <div key={line.id}><span>{line.title}</span><strong>{line.quantity.toLocaleString('tr-TR')} adet · {line.unitPrice.toLocaleString('tr-TR', { style: 'currency', currency: item.currency })}</strong></div>)}</div>{confirmation.action === 'REJECT' && <dl className="return-decision-summary-facts"><div><dt>Ret nedeni</dt><dd>{confirmation.reasonLabel ?? '—'}</dd></div><div><dt>Açıklama</dt><dd>{confirmation.explanation || '—'}</dd></div></dl>}<p className="return-decision-summary-total">{selectedLines.length} ürün satırı işleme alınacak.</p></div></Modal>}</section>
}
