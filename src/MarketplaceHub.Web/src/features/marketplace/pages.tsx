...
function ReturnInlineActions({ item, selectedLineIds, onSelectedLineIdsChange }: { item: ReturnDetail; selectedLineIds?: string[]; onSelectedLineIdsChange?: (ids: string[]) => void }) {
  const [notice, setNotice] = useState('')
  return <div className="return-inline-actions">{notice && <div className="notice" role="status">{notice}</div>}<ReturnReceiptButton item={item} onNotice={setNotice} /><ReturnDecisionActions item={item} onNotice={setNotice} selectedLineIds={selectedLineIds} onSelectedLineIdsChange={onSelectedLineIdsChange} /></div>
}
