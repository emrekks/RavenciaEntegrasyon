...
function InvoiceUploadModal({ item, provider, onClose, onFeedback }: { item: Order; provider: Connection | null; onClose: () => void; onFeedback: IntegrationFeedbackHandler }) {
  ...
  const upload = useMutation({
    ...
    onSuccess: async () => { const message = 'Fatura dosyası güvenli arşive yüklendi.'; onFeedback(message, 'success'); await client.invalidateQueries({ queryKey: ['orders'] }); onClose() },
  ...
  const manualStatus = useMutation({
    mutationFn: async (status: 'PENDING' | 'UPLOADED') => {
      const order = detail.data
      if (!order || order.platformCode !== 'SHOPIFY') throw new Error('Manuel fatura durumu yalnız Shopify siparişlerinde kullanılabilir.')
    ...
    onSuccess: async (_, status) => { const message = status === 'UPLOADED' ? 'Fatura durumu “Faturası yüklendi” olarak güncellendi.' : 'Fatura durumu “Fatura bekliyor” olarak güncellendi.'; setMessage(message); onFeedback(message, 'success'); await client.invalidateQueries({ queryKey: ['orders'] }) },
    onError: error => { const nextMessage = error instanceof Error ? error.message : 'Fatura durumu güncellenemedi.'; setMessage(nextMessage); onFeedback(nextMessage, 'error') }
  })
  function choose(selected: File | undefined) { if (selected) { setFile(selected); setMessage('') } }
  return <div className="workspace-modal-backdrop" role="presentation" onMouseDown={onClose}><section className="workspace-modal invoice-upload-modal" role="dialog" aria-modal="true" aria-labelledby="invoice-upload-title" onMouseDown={event => event.stopPropagation()}><header><h2 id="invoice-upload-title">Fatura Yükle</h2><button type="button" className="modal-close" onClick={onClose} aria-label="Pencereyi kapat"><UiIcon name="close" /></button></header>{detail.isError ? <ErrorBox error={detail.error} /> : <><label className={`invoice-dropzone${dragging ? ' dragging' : ''}`} onDragEnter={event => { event.preventDefault(); setDragging(true) }} onDragOver={event => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); choose(event.dataTransfer.files[0]) }}><input type="file" accept=".pdf,.jpeg,.jpg,.png,application/pdf,image/jpeg,image/png" onChange={event => choose(event.target.files?.[0])} /><span className="invoice-upload-icon" aria-hidden="true"><UiIcon name="upload" /></span><strong>{file ? file.name : 'Fatura Dosyası Yükle'}</strong><small>{file ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : 'Dosyanızı seçin ya da bu alana sürükleyin.'}</small><b>Dosya Seç</b></label>{message && <p className="error" role="alert">{message}</p>}{item.platformCode === 'SHOPIFY' && <div className="invoice-manual-status-actions" role="group" aria-label="Shopify manuel fatura durumu"><span>Durumu elle ayarla:</span><button type="button" className="secondary" disabled={detail.isLoading || manualStatus.isPending} onClick={() => manualStatus.mutate('PENDING')}>Fatura bekliyor</button><button type="button" className="secondary" disabled={detail.isLoading || manualStatus.isPending} onClick={() => manualStatus.mutate('UPLOADED')}>Faturası yüklendi</button></div>}<footer><button type="button" className="secondary" onClick={onClose}>Vazgeç</button><button type="button" disabled={!file || detail.isLoading || upload.isPending} onClick={() => upload.mutate()}>{upload.isPending ? 'Yükleniyor…' : 'Faturayı Yükle'}</button></footer></>}</section></div>
}
...
function ShopifyInvoiceStatusModal({ item, onClose, onFeedback }: { item: Order; onClose: () => void; onFeedback: IntegrationFeedbackHandler }) {
  ...
  return <div className="workspace-modal-backdrop" role="presentation" onMouseDown={onClose}><section className="workspace-modal shopify-invoice-status-modal" role="dialog" aria-modal="true" aria-labelledby="shopify-invoice-status-title" onMouseDown={event => event.stopPropagation()}><header><div><p className="eyebrow">SHOPIFY</p><h2 id="shopify-invoice-status-title">Fatura durumunu değiştir</h2><p>{displayOrderNumber(item.orderNumber)} · Bu değişiklik yalnızca paneldeki fatura durumunu günceller.</p></div><button type="button" className="modal-close" onClick={onClose} aria-label="Pencereyi kapat"><UiIcon name="close" /></button></header>{detail.isLoading ? <Busy text="Sipariş ve fatura bilgileri yükleniyor…" /> : detail.isError || !detail.data ? <ErrorBox error={detail.error} /> : <div className="shopify-invoice-status-form"><label><span>Yeni fatura durumu</span><select value={status} onChange={event => setStatus(event.target.value as 'PENDING' | 'UPLOADED')} disabled={update.isPending}><option value="PENDING">Fatura bekliyor</option><option value="UPLOADED">Faturası yüklendi</option></select></label><p className="notice">“Faturası yüklendi” seçeneği yalnızca panel durumunu değiştirir. Belgeyi ayrıca “Fatura Yükle” menüsünden ekleyebilirsiniz.</p>{message && <p className="error" role="alert">{message}</p>}<footer><button type="button" className="secondary" onClick={onClose} disabled={update.isPending}>Vazgeç</button><button type="button" onClick={() => update.mutate()} disabled={update.isPending}>{update.isPending ? 'Kaydediliyor…' : 'Durumu kaydet'}</button></footer></div>}</section></div>
}
...
function connectionTestHelp(item: Connection) {
  ...
  if (item.platformCode === 'SHOPIFY' && item.lastErrorCode === 'SHOPIFY_REQUIRED_READ_SCOPE') return 'Shopify uygulama tokenında read_products, read_inventory, read_orders ve read_locations okuma izinleri açık olmalı. İlk sürüm müşteri kişisel verilerini istemez; izinleri açtıktan sonra yeni token oluşturup burada şifreli kaydedin.'
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  const syncPolicyDescription = (resourceType: string) => ({
    ORDERS: item.platformCode === 'SHOPIFY' ? 'Shopify siparişlerini okur ve ödeme, gönderim, iptal ve iade durumlarını panele taşır; ilk tarama son 60 günle sınırlıdır.' : 'Trendyol’daki yeni ve güncellenen siparişleri panele alır; ürün, müşteri, paket ve kargo bilgilerini yerel çalışma ekranlarında güncel tutar.',
     ORDER_RECOVERY: item.platformCode === 'SHOPIFY' ? 'Shopify’da normal taramada kaçırılmış veya geçmiş tarihli siparişleri yeniden arayarak eksik kayıtları tamamlar.' : 'Normal sipariş taramasında kaçırılmış veya geçmiş tarihli siparişleri yeniden arayarak eksik kayıtları tamamlar.',
...
export function OrdersPage() {
  ...
  const resetFilters = () => { const next = { ...initialOrderFilters }; setFilterForm(next); setFilters(next); setPage(1); setColumnFilterOpen(null); setSearchParams(current => { const params = new URLSearchParams(current); params.delete('search'); params.delete('status'); return params }, { replace: true }) }
  ...
  async function exportOrders() {
    setExporting(true)
    try {
    const exportItems = hasLocalFilters ? items : (await loadAllOrderPages(filters)).items
  ...
  return <section className="content f3 orders-page"><div className="page-heading"><div><p className="eyebrow">Sipariş yönetimi</p><h1>Sipariş Yönetimi</h1><p className="lede">Tüm pazar yeri siparişlerinizi tek merkezden yönetin ve takip edin.</p></div><div className="page-heading-actions orders-reference-heading-actions"><button type="button" className="secondary orders-export-action" disabled={!items.length || exporting} onClick={() => void exportOrders()}><UiIcon name="download" className="orders-export-icon" size={18} />{exporting ? 'Dışa aktarılıyor…' : 'Dışa Aktar'}</button><button type="button" className="secondary orders-import-action" onClick={() => setShopifyCsvImportOpen(true)}><UiIcon name="upload" size={18} />İçe aktar</button><button type="button" className="orders-sync-action" onClick={() => setSingleSyncOpen(true)}><UiIcon name="refresh" className="orders-sync-icon" size={18} /> Sipariş Senkronizasyonu</button></div></div>
    ...
    <div className="orders-control-bar" aria-label="Sipariş filtreleri"><label className="order-search"><span className="orders-control-label">Sipariş ara</span><span className="order-search-control"><UiIcon name="search" /><input aria-label="Sipariş ara" value={filterForm.search} onChange={event => applyFilterValue('search', event.target.value)} placeholder="Sipariş no, müşteri veya barkod ara…" /></span></label><label><span>Platform</span><select aria-label="Platform filtresi" value={filterForm.platform} onChange={event => applyFilterValue('platform', event.target.value)}><option value="ALL">Tüm platformlar</option>{platformOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label><span>Sıralama</span><select aria-label="Sipariş sıralaması" value={filterForm.sort} onChange={event => applyFilterValue('sort', event.target.value as OrderFilters['sort'])}><option value="DATE_DESC">En yeni siparişler</option><option value="DATE_ASC">En eski siparişler</option><option value="DUE_DESC">Teslim tarihi yaklaşanlar</option><option value="DUE_ASC">Teslim tarihi uzak olanlar</option></select></label><button type="button" className="secondary orders-filter-reset" onClick={resetFilters}>Filtreleri temizle</button></div>
    {selectedOrders.length > 0 && <div className="orders-bulk-toolbar" role="region" aria-label="Toplu sipariş işlemleri"><strong>{selectedOrders.length} sipariş seçildi</strong><div className="orders-bulk-actions">{cargoEditableOrders.length > 0 && <button type="button" onClick={() => setCourierOrder({ item: cargoEditableOrders[0], items: cargoEditableOrders })}>Kargo firması değiştir</button>}{labelBatchItems.length > 0 && <button type="button" onClick={() => setShippingLabelBatch({ items: labelBatchItems, format: labelSettings.defaultFormat })}>Etiketleri yazdır ({labelBatchItems.length})</button>}<button type="button" className="secondary" onClick={() => setSelectedIds([])}>Seçimi temizle</button></div></div>}
...
export function ReturnsPage() {
  const [status, setStatus] = useState('ALL'); const [customer, setCustomer] = useState(''); const [orderNumber, setOrderNumber] = useState(''); const [claimCode, setClaimCode] = useState(''); const [barcode, setBarcode] = useState(''); const [reason, setReason] = useState(''); const [from, setFrom] = useState(''); const [to, setTo] = useState(''); const [pageSize] = useState(20); const [pageNumber, setPageNumber] = useState(1); const [invoiceStatusFilter, setInvoiceStatusFilter] = useState('ALL'); const [returnColumnFilterOpen, setReturnColumnFilterOpen] = useState<'invoice' | 'status' | null>(null)
  ...
  const filteredCountItems = countItems.filter(matchesInvoiceStatus)
  const visible = items.filter(matchesInvoiceStatus)
  useEffect(() => { setPageNumber(1) }, [appliedFilters, pageSize])
  ...
  function clearFilters() { const next: ReturnFilters = { status: 'ALL', customer: '', orderNumber: '', claimCode: '', barcode: '', reason: '', from: '', to: '' }; setStatus(next.status); setCustomer(next.customer); setOrderNumber(next.orderNumber); setClaimCode(next.claimCode); setBarcode(next.barcode); setReason(next.reason); setFrom(next.from); setTo(next.to); setInvoiceStatusFilter('ALL'); setReturnColumnFilterOpen(null); setAppliedFilters(next); setPageNumber(1) }
