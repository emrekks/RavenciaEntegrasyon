...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const webColorAttributes = productAttributes
  const optionAttributeNames = new Set([...optionAttributes, ...(specialColorOption ? [specialColorOption.attribute] : [])].map(attribute => normalizedAttributeKey(attribute.name)))
  ...
  const mappedOptionCount = optionMappings.filter(item => optionAttributeIds.has(item.localId)).length
  ...
  const optionRemoteItems = remoteItems.filter(item => item.externalId !== webColorRemote?.externalId && !isWebColorAttributeName(item.name))
  return <article className="panel mapping-step"><div className="editor-section-title"><span>4</span><div><h2>Kategori eşleştirmeleri</h2><p>Trendyol kategori alanlarını ürün özellikleri ve kategori seçenekleriyle ayrı bölümlerde eşleyin.</p></div></div><label className="matched-category-select">Eşleşmiş panel kategorisi<select value={attributeCategoryId} onChange={event => setAttributeCategoryId(event.target.value)}>{matchedCategories.map(({ category }) => <option key={category.id} value={category.id}>{category.path}</option>)}</select></label>{references.isLoading || mappings.isLoading || categoryRequirements.isLoading ? <Busy text="Kategori özellikleri ve eşlemeler yükleniyor…" /> : references.isError || mappings.isError || categoryRequirements.isError ? <div className="unknown"><strong>Güncel kategori özellik verisi alınamadı</strong><p>Seçili Trendyol kategorisinin özellik listesini yeniden eşitleyin.</p><button type="button" disabled={sync.isPending} onClick={() => sync.mutate()}>{sync.isPending ? 'Kuyruğa alınıyor…' : 'Kategori özelliklerini eşitle'}</button></div> : <><div className={`mapping-progress ${mappedRequired === requiredItems.length ? 'complete' : ''}`}><strong>{mappedRequired}/{requiredItems.length} ürün özelliği zorunlusu eşlendi</strong><span>{remoteItems.length} toplam alan · {productMappings.length} ürün özelliği · {mappedOptionCount}/{optionCount} seçenek eşlemesi · Trendyol kategori no: {categoryScope}</span></div><div className="mapping-section-toolbar"><div className="button-row"><button type="button" className="secondary" disabled={sync.isPending} onClick={() => sync.mutate()}>{sync.isPending ? 'Alanlar yenileniyor…' : 'Kategori alanlarını yenile'}</button>{optionCount > 0 && <button type="button" disabled={productSync.isPending} onClick={() => productSync.mutate()}>{productSync.isPending ? 'Ürünler eşitleniyor…' : 'Ürün seçeneklerini salt-okunur eşitle'}</button>}</div></div>{optionCount > 0 && <section className="mapping-category-options-section"><div className="mapping-section-heading"><div><h3>Kategori seçenekleri</h3><p>Renk, beden gibi varyant üreten panel seçeneklerini Trendyol alanlarıyla eşleyin.</p></div></div><div className="embedded-attribute-grid mapping-unified-grid mapping-options-grid">{specialColorOption && <WebColorMappingCard connectionId={connectionId} categoryScope={categoryScope} snapshotId={references.data!.snapshotId} localAttribute={specialColorOption.attribute} localAttributes={webColorAttributes} categoryOptionIds={categoryOptionIds} remoteAttribute={specialColorOption.remote} existingMapping={existingWebColorMapping} existingMappings={allMappings} onNotice={onNotice} />}{optionAttributes.map(item => <OptionMappingCard key={item.id} connectionId={connectionId} categoryScope={categoryScope} snapshotId={references.data!.snapshotId} localAttribute={item} remoteAttributes={optionRemoteItems} existingMapping={optionMappings.find(mapping => mapping.localId === item.id) ?? null} onNotice={onNotice} />)}</div></section>}<section className="mapping-category-attributes-section"><div className="mapping-section-heading"><div><h3>Kategori özellikleri</h3><p>Ürün özelliklerini ve Trendyol kategori alanlarını eşleyin.</p></div></div><div className="embedded-attribute-grid mapping-unified-grid mapping-attributes-grid">{productRemoteItems.map(item => <CategoryAttributeCard key={item.externalId} connectionId={connectionId} categoryScope={categoryScope} snapshotId={references.data!.snapshotId} localAttributes={productAttributes} categoryOptionIds={categoryOptionIds} remoteAttribute={item} existingMapping={productMappingByExternal.get(item.externalId) ?? null} usedLocalIds={usedProductLocalIds} onNotice={onNotice} />)}</div></section></>}</article>
}
...
export function WebColorMappingCard({ connectionId, categoryScope, snapshotId, localAttribute, localAttributes, categoryOptionIds, remoteAttribute, existingMapping, existingMappings, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttribute: LocalAttribute; localAttributes: LocalAttribute[]; categoryOptionIds: Set<string>; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; existingMappings: CatalogMapping[]; onNotice: (value: string) => void }) {
  ...
  const categoryOptions = selectableAttributes.filter(attribute => categoryOptionIds.has(attribute.id) || isOptionAttribute(attribute))
  const panelAttributes = selectableAttributes.filter(attribute => !categoryOptions.some(option => option.id === attribute.id))
  ...
  return <section className="attribute-mapping-card option-mapping-card required"><div className="attribute-mapping-card-head"><img className="mapping-platform-logo" src="/trendyol-mark.png" alt="" aria-hidden="true" /><div><strong>{remoteAttribute.name}</strong><small>Kategori seçeneği</small></div></div><div className="mapping-fields compact"><label>Entegrasyon ürün seçeneği<select aria-label="Web Color için panel seçeneği" value={localId} onChange={event => { setLocalId(event.target.value); setShowValues(false) }}><option value="">Seçenek seçin</option>{categoryOptions.length > 0 && <optgroup label="Kategori seçenekleri">{categoryOptions.map(attribute => <option key={attribute.id} value={attribute.id}>{attribute.name}</option>)}</optgroup>}{panelAttributes.length > 0 && <optgroup label="Panel seçenekleri">{panelAttributes.map(attribute => <option key={attribute.id} value={attribute.id}>{attribute.name}</option>)}</optgroup>}</select></label>{mapped ? <button type="button" className="secondary" onClick={() => void remove()}>Eşlemeyi kaldır</button> : <button type="button" onClick={() => void save()} disabled={!localId}>Kaydet</button>}</div>{mapped && selectedAttribute.values.length > 0 && <><button type="button" className="value-mapping-toggle" onClick={() => setShowValues(value => !value)}>{showValues ? 'Değer eşlemelerini gizle' : `Seçenek değerlerini eşle (${selectedAttribute.values.length})`}</button>{showValues && <WebColorValueMappingEditor connectionId={connectionId} categoryScope={categoryScope} attribute={selectedAttribute} externalAttributeId={remoteAttribute.externalId} />}</>}</section>
}
...
function CategoryAttributeCard({ connectionId, categoryScope, snapshotId, localAttributes, categoryOptionIds, remoteAttribute, existingMapping, usedLocalIds, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttributes: LocalAttribute[]; categoryOptionIds: Set<string>; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; usedLocalIds: Set<string>; onNotice: (value: string) => void }) {
  ...
  const selectableAttributes = localAttributes.filter(item => item.id === localId || (!categoryOptionIds.has(item.id) && !isOptionAttribute(item) && !usedLocalIds.has(item.id)))
...
function WebColorValueMappingEditor({ connectionId, categoryScope, attribute, externalAttributeId }: { connectionId: string; categoryScope: string; attribute: LocalAttribute; externalAttributeId: string }) {
  ...
  const localValues = attribute.values?.filter(item => item.isActive) ?? []; const remoteValues = references.data?.items.filter(item => item.isActive) ?? []; const mappingByLocal = new Map((mappings.data ?? []).map(item => [item.localId, item])); const remoteNameById = new Map(remoteValues.map(item => [item.externalId, item.name]))
  ...
  return <div className="web-color-value-editor"><div className="value-mapping-heading"><div><h3>Web Color değer eşleştirmesi</h3><p>Bir Web Color değeri seçin; karşısındaki panel renklerini işaretleyin. {localValues.length} panel rengi · {remoteValues.length} Web Color değeri</p></div><button type="button" disabled={saving} onClick={() => void saveAll()}>{saving ? 'Kaydediliyor…' : 'Tüm eşlemeleri kaydet'}</button></div>{notice && <p role="status" className="notice">{notice}</p>}<div className="web-color-assignment-picker"><label>Web Color değeri<select aria-label="Web Color değeri" value={selectedRemoteId} onChange={event => setSelectedRemoteId(event.target.value)}><option value="">Web Color seçin</option>{remoteValues.map(remote => <option key={remote.externalId} value={remote.externalId}>{remote.name}</option>)}</select></label><div className="web-color-assignment-current"><span>Seçili hedef</span><strong>{remoteNameById.get(selectedRemoteId) ?? 'Web Color seçin'}</strong><small>{selectedLocalIds.size} panel rengi bağlı</small></div></div><div className="web-color-panel-value-picker"><div className="web-color-panel-value-picker-head"><strong>Karşılık gelen panel renkleri</strong><span>{selectedLocalIds.size} seçildi</span></div><div className="web-color-panel-value-grid">{localValues.map(localValue => { const assignedTo = selections[localValue.id]; const selected = selectedLocalIds.has(localValue.id); return <label key={localValue.id} className={`web-color-panel-value${selected ? ' selected' : ''}`}><input type="checkbox" checked={selected} disabled={!selectedRemoteId} onChange={() => toggleLocalValue(localValue.id)} /><span>{localValue.value}</span>{assignedTo && !selected && <small>{remoteNameById.get(assignedTo) ?? 'Web Color'}</small>}</label> })}</div></div>{assignedGroups.length > 0 && <div className="web-color-assignment-groups"><strong>Kayıtlı Web Color grupları</strong><div>{assignedGroups.map(group => <p key={group.remote.externalId}><span>{group.remote.name}</span><small>{group.locals.map(value => value.value).join(' · ')}</small></p>)}</div></div>}</div>
}
