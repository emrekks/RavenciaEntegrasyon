...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  return <div className={rootClass}><div className={`page-heading ${inline ? 'integration-inline-page-heading' : ''}`}><div>{inline ? <p className="lede integration-inline-meta">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p> : <><Link to="/integrations" className="back"><UiIcon name="arrowLeft" /> Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></>}</div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    {item.platformCode === 'TRENDYOL' && readConnectionReady && <section className="panel integration-detail-section sync-policy-panel" aria-labelledby="sync-policy-title"><div className="integration-detail-section-heading"><div><p className="eyebrow">OTOMATİK AKIŞ</p><h2 id="sync-policy-title">Veri eşitleme ayarları</h2><p>Sipariş, iade ve dış yazma akışlarını kategorilerine göre inceleyin; çalışma sıklığını ve güvenlik aralığını aşağıdaki kartlardan yönetin.</p></div><span className="integration-detail-section-badge">ARKA PLANDA</span></div>{syncPolicies.isLoading ? <Busy text="Eşitleme ayarları yükleniyor…" /> : syncPolicies.isError ? <ErrorBox error={syncPolicies.error} /> : <div className="sync-policy-list">{syncPolicyGroups.map(group => <div className="sync-policy-group" key={group.key}><div className="sync-policy-group-heading"><h3>{group.label}</h3><span>{group.policies.length} akış</span></div><div className="sync-policy-group-grid">{group.policies.map(policy => {
      ...
      const intervals: Array<[number, string]> = [[0, 'Anında'], [30, '30 saniye'], [60, '1 dakika'], [120, '2 dakika'], [180, '3 dakika'], [300, '5 dakika'], [600, '10 dakika'], [900, '15 dakika'], [3600, '1 saat'], [86400, '24 saat']]
      const overlaps: Array<[number, string]> = [[120, '2 dakika'], [300, '5 dakika'], [600, '10 dakika'], [900, '15 dakika']]
      const healthStatus = policy.healthStatus ?? 'OFFLINE'
      const healthHelp = healthStatus === 'OFFLINE'
      ...
      return <form className="sync-policy-row" key={policy.id} onSubmit={event => updateSyncPolicy(event, policy)}><div className="sync-policy-identity"><div className="sync-policy-title"><strong>{policyLabel}</strong><Badge value={healthStatus} tooltip={healthHelp} /></div><div className="sync-policy-meta"><small>Son tamamlanan: <DateText value={policy.lastSuccessAt} /></small><small>Son deneme: <DateText value={policy.lastAttemptAt ?? null} /></small><small>{policy.lastRequestCount ?? 0} istek · {policy.lastReceivedCount ?? 0} kayıt · {policy.lastFailedCount ?? 0} hata · {policy.lastRetryCount ?? 0} retry · {policy.lastRateLimitCount ?? 0} rate-limit{policy.consecutiveFailureCount ? ` · ${policy.consecutiveFailureCount} ardışık hata` : ''}</small></div>{healthStatus === 'OFFLINE' && <small className="sync-policy-health-help">{healthHelp}</small>}{externalWriteBlocked && <small className="sync-policy-health-help">Dış yazma kapalı; bu akış otomatik çalıştırılmaz.</small>}{policy.recoveryGapStatus && policy.recoveryGapStatus !== 'OK' && <small className={`sync-policy-gap sync-policy-gap-${policy.recoveryGapStatus.toLowerCase()}`}>Kurtarma durumu: {policy.recoveryGapStatus === 'UNKNOWN' ? 'ilk kontrol bekleniyor' : `${Math.round(policy.recoveryGapDays ?? 0)} günlük pencere`}</small>}</div><label title="Akışın platformu ne sıklıkla kontrol edeceğini belirler."><span>Kontrol sıklığı</span><select name="intervalSeconds" defaultValue={policy.intervalSeconds} disabled={externalWriteBlocked}>{!intervals.some(([value]) => value === policy.intervalSeconds) && <option value={policy.intervalSeconds}>{policy.intervalSeconds} saniye</option>}{intervals.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label title="Son işlenen zaman aralığının bu bölümünü yeniden kontrol eder; geciken veya kaçan kayıtları yakalamaya yardımcı olur."><span>Tekrar payı</span><select name="overlapSeconds" defaultValue={policy.overlapSeconds} disabled={externalWriteBlocked}>{!overlaps.some(([value]) => value === policy.overlapSeconds) && <option value={policy.overlapSeconds}>{Math.round(policy.overlapSeconds / 60)} dakika</option>}{overlaps.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="sync-policy-toggle" title={externalWriteBlocked ? 'Dış yazmayı açmadan bu otomatik akış çalışmaz.' : 'Bu akış açıkken belirlenen sıklıkta otomatik çalışır.'}><input type="checkbox" name="enabled" defaultChecked={policy.enabled} disabled={externalWriteBlocked} /><span>{externalWriteBlocked ? 'Dış yazma kapalı' : 'Otomatik akış açık'}</span></label><button className="sync-policy-save" disabled={saveSyncPolicy.isPending || externalWriteBlocked}>{saveSyncPolicy.isPending ? 'Kaydediliyor…' : 'Ayarı kaydet'}</button></form>
     })}</div></div>)}</div>}</section>}
