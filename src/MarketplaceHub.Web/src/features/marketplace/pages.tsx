...
export function ReturnsPage() {
  ...
  return <section className="content f3 returns-page reference-returns-page">
    <div className="page-heading returns-reference-heading">
      ...
      <div className="page-heading-actions returns-reference-heading-actions"><button type="button" className="returns-sync-action" disabled={!trendyolConnection || sync.isPending} onClick={() => sync.mutate()}><UiIcon name="sync" />{sync.isPending ? 'Eşitleniyor…' : 'İade Senkronizasyonu'}</button></div>
    </div>
...
function CategoryAttributeCard({ connectionId, categoryScope, snapshotId, localAttributes, categoryOptionIds, allowedOptionAttributeIds, remoteAttribute, existingMapping, usedLocalIds, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttributes: LocalAttribute[]; categoryOptionIds: Set<string>; allowedOptionAttributeIds?: Set<string>; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; usedLocalIds: Set<string>; onNotice: (value: string) => void }) {
  ...
  const selectableAttributes = localAttributes.filter(item => item.id === localId || (allowedOptionAttributeIds?.has(item.id) && !usedLocalIds.has(item.id)) || (!categoryOptionIds.has(item.id) && !isOptionAttribute(item) && !usedLocalIds.has(item.id)))
  const panelAttributes = selectableAttributes
  async function save() {
    try {
      if (!localId) throw new Error('Entegrasyon ürün özelliğini seçin.')
      const role = allowedOptionAttributeIds?.has(localId) ? 'OPTION' : 'ATTRIBUTE'
