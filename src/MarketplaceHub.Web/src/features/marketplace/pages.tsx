...
function ReferenceConnectionCard({ item, onSaved, onFeedback }: { item: Connection; onSaved: () => void; onFeedback: (feedback: IntegrationFeedback) => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${hidden ? 'reference-integration-card-hidden' : connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`} onClickCapture={event => { const target = event.target; if (target instanceof Element && target.closest('.reference-menu-popover a')) { event.preventDefault(); setMenuOpen(false); setSettingsOpen(true) } }}>
    ...
    {privateDataHidden ? <div className="reference-integration-private-state" role="status"><strong>Bağlantı pasif</strong><span>Bağlantı bilgileri güvenlik için gizlendi.</span></div> : <div className="reference-integration-body"><div className="reference-data-point"><span>{efaturam ? 'VKN / TCKN' : shopify ? 'Shopify mağazası' : 'Mağaza Kimliği (Seller ID)'}</span><strong>{item.externalStoreId || 'Tanımlanmadı'}</strong></div><div className="reference-data-point"><span>Credential</span><strong>{credentialLabel(item)}</strong></div><div className="reference-data-point"><span>Ortam</span><strong>{item.environment === 'PRODUCTION' ? 'Canlı' : 'Stage'}</strong></div><div className="reference-data-point"><span>Dış yazma</span><strong>{item.externalWritesEnabled ? 'Açık' : 'Salt-okunur'}</strong></div></div>}
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  const [shopifyModelCodeMetafield, setShopifyModelCodeMetafield] = useState('ravencia.model_code')
  ...
  useEffect(() => { if (connection.data?.platformCode === 'SHOPIFY') setShopifyModelCodeMetafield(connection.data.shopifyModelCodeMetafield || 'ravencia.model_code') }, [connection.data?.platformCode, connection.data?.shopifyModelCodeMetafield])
  const saveConnectionDetails = useMutation({ mutationFn: () => { if (!connection.data) throw new Error('Bağlantı bilgisi yükleniyor.'); const value = externalStoreId.trim(); if (!value) throw new Error('Mağaza kimliği veya bağlantı kapsamı zorunludur.'); const modelCodeMetafield = shopifyModelCodeMetafield.trim(); if (connection.data.platformCode === 'SHOPIFY' && !/^[A-Za-z0-9_-]{1,80}\.[A-Za-z0-9_-]{1,80}$/.test(modelCodeMetafield)) throw new Error('Model kodu metafield’i namespace.key biçiminde olmalıdır.'); return hubApi<Connection>(`/connections/${id}`, { method: 'PATCH', headers: { 'If-Match': `\"v${connection.data.version}\"` }, body: JSON.stringify({ displayName: connection.data.displayName, externalStoreId: value, ...(connection.data.platformCode === 'SHOPIFY' ? { shopifyModelCodeMetafield: modelCodeMetafield } : {}) }) }) }, onSuccess: async result => { setExternalStoreId(result.externalStoreId); if (result.platformCode === 'SHOPIFY') setShopifyModelCodeMetafield(result.shopifyModelCodeMetafield || 'ravencia.model_code'); setNotice('Mağaza bilgileri güncellendi. Yeniden bağlantı testi gerekiyor.'); await client.invalidateQueries({ queryKey: ['connection', id] }); await client.invalidateQueries({ queryKey: ['connections'] }) }, onError: reason => setNotice(reason instanceof Error ? reason.message : 'Mağaza bilgileri güncellenemedi.') })
  const saveExternalWrites = useMutation({ mutationFn: (enabled: boolean) => { if (!connection.data) throw new Error('Bağlantı bilgisi yükleniyor.'); return hubApi<Connection>(`/connections/${id}`, { method: 'PATCH', headers: { 'If-Match': `"v${connection.data.version}"` }, body: JSON.stringify({ displayName: connection.data.displayName, externalWritesEnabled: enabled }) }) }, onSuccess: async (_, enabled) => { setExternalWritesEnabled(enabled); setNotice(enabled ? 'Dış yazma açıldı.' : 'Salt-okunur mod etkin; dış yazma ve bağlı otomatik akışlar kapatıldı.'); await client.invalidateQueries({ queryKey: ['connection', id] }); await client.invalidateQueries({ queryKey: ['connections'] }); await client.invalidateQueries({ queryKey: ['sync-policies', id] }) }, onError: reason => setNotice(reason instanceof Error ? reason.message : 'Dış yazma ayarı güncellenemedi.') })
  ...
  const stageManual = item.environment === 'STAGE'; const readConnectionReady = item.status === 'ACTIVE' || item.status === 'VERIFIED'; const stageReady = stageManual && !item.lastErrorCode && readConnectionReady
  ...
  return <div className={rootClass}><div className={`page-heading ${inline ? 'integration-inline-page-heading' : ''}`}><div>{inline ? <p className="lede integration-inline-meta">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p> : <><Link to="/integrations" className="back"><UiIcon name="arrowLeft" /> Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></>}</div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    <section className="panel connection-settings-panel"><div className="connection-settings-section"><h2>Mağaza bilgileri</h2><p>Mağaza adı, model kodu alanı ve bağlantı bilgilerini buradan güncelleyin.</p><form onSubmit={event => { event.preventDefault(); saveConnectionDetails.mutate() }}><label>{item.platformCode === 'TRENDYOL_EFATURAM' ? 'VKN / TCKN' : item.platformCode === 'SHOPIFY' ? 'Shopify mağaza adı' : 'Mağaza kimliği (Seller ID)'}<input value={externalStoreId} onChange={event => setExternalStoreId(event.target.value)} autoComplete="off" required />{item.platformCode === 'SHOPIFY' && <small>Kısa mağaza adı veya magaza-adi.myshopify.com adresi.</small>}</label>{item.platformCode === 'SHOPIFY' && <label>Model kodu metafield’i<input value={shopifyModelCodeMetafield} onChange={event => setShopifyModelCodeMetafield(event.target.value)} pattern="[A-Za-z0-9_-]{1,80}\.[A-Za-z0-9_-]{1,80}" required /><small>Ürün eşleştirmesinde kullanılacak alan. Varsayılan: <code>ravencia.model_code</code>. SKU’dan model kodu tahmin edilmez.</small></label>}<button disabled={saveConnectionDetails.isPending}>{saveConnectionDetails.isPending ? 'Kaydediliyor…' : 'Mağaza bilgisini kaydet'}</button></form></div><div className="connection-settings-divider" /><form className="connection-settings-section" onSubmit={credential}><h2>{item.platformCode === 'SHOPIFY' ? 'Shopify uygulama yetkisi' : 'API bilgileri'}</h2>{item.platformCode === 'TRENDYOL_EFATURAM' ? <><label>Hesap e-postası<input name="email" type="email" autoComplete="username" required /></label><label>Hesap parolası<input name="password" type="password" autoComplete="new-password" required /></label><p>Bu alanlar yalnız şifreli credential kaydında tutulur ve tekrar gösterilmez.</p></> : item.platformCode === 'SHOPIFY' ? <><label>Shopify uygulama tokenı<input name="shopifyAccessToken" type="password" autoComplete="off" placeholder="shpat_…" required /><p>Shopify Admin API tokenı doğrudan kullanılır, şifreli saklanır ve tekrar gösterilmez.</p></label></> : <><label>API key<input name="apiKey" type="password" autoComplete="off" required /></label><label>API secret<input name="apiSecret" type="password" autoComplete="new-password" required /></label></>}<button disabled={run.isPending}>{run.isPending ? 'Kaydediliyor…' : 'Şifreli kaydet'}</button><button type="button" className="secondary" disabled={!item.hasCredential || run.isPending} onClick={() => run.mutate({ path: `/connections/${id}/test-jobs` })}>Bağlantıyı test et</button></form></section>
