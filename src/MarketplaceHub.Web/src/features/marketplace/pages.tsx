...
function SyncPolicyEditorModal({ policy, label, intervals, overlaps, healthHelp, externalWriteBlocked, saving, onClose, onSave }: SyncPolicyEditorModalProps) {
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  const syncPolicyLabel = (resourceType: string) => ({ ORDERS: 'Siparişler ve paket durumları', ORDER_RECOVERY: 'Sipariş geçmişi kurtarma', ORDER_LIFECYCLE: 'Açık sipariş yaşam döngüsü', ORDER_RECONCILE_SHORT: 'Kısa sipariş uzlaştırması', ORDER_RECONCILE_MEDIUM: 'Orta sipariş uzlaştırması', ORDER_RECONCILE_DAILY: 'Günlük sipariş uzlaştırması', ORDER_INVOICE_RECONCILIATION: 'Paket fatura kontrolü', RETURNS: 'İade talepleri', RETURN_LIFECYCLE: 'Açık iade yaşam döngüsü', RETURN_RECONCILE_SHORT: 'Kısa iade uzlaştırması', RETURN_RECONCILE_MEDIUM: 'Orta iade uzlaştırması', RETURN_RECONCILE_DAILY: 'Günlük iade uzlaştırması', STOCK_RECONCILE_SHORT: 'Kısa stok uzlaştırması', STOCK_RECONCILE_MEDIUM: 'Orta stok uzlaştırması', STOCK_RECONCILE_DAILY: 'Günlük stok uzlaştırması', PRICE_WRITE: 'Fiyat gönderimleri', STOCK_WRITE: 'Stok gönderimleri', SHIPMENT_WRITE: 'Kargo ve etiket işlemleri', RETURN_WRITE: 'İade kararları', REFERENCE_DATA: 'Katalog referansları' } as Record<string, string>)[resourceType] ?? resourceType
  ...
  return <div className={rootClass}><div className={`page-heading ${inline ? 'integration-inline-page-heading' : ''}`}><div>{inline ? <p className="lede integration-inline-meta">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p> : <><Link to="/integrations" className="back"><UiIcon name="arrowLeft" /> Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></>}</div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    {item.platformCode === 'TRENDYOL' && readConnectionReady && <section className="panel integration-detail-section sync-policy-panel" aria-labelledby="sync-policy-title"><div className="integration-detail-section-heading"><div><p className="eyebrow">OTOMATİK AKIŞ</p><h2 id="sync-policy-title">Veri eşitleme ayarları</h2><p>Sipariş, iade ve dış yazma akışlarını kategorilerine göre inceleyin; çalışma sıklığını ve güvenlik aralığını aşağıdaki kartlardan yönetin.</p></div><span className="integration-detail-section-badge">ARKA PLANDA</span></div>{syncPolicies.isLoading ? <Busy text="Eşitleme ayarları yükleniyor…" /> : syncPolicies.isError ? <ErrorBox error={syncPolicies.error} /> : <div className="sync-policy-list">{syncPolicyGroups.map(group => <div className="sync-policy-group" key={group.key}><div className="sync-policy-group-heading"><h3>{group.label}</h3><span>{group.policies.length} akış</span></div><div className="sync-policy-group-grid">{group.policies.map(policy => {
       ...
       return <button type="button" className={`sync-policy-row ${policy.enabled && !externalWriteBlocked ? 'is-enabled' : 'is-disabled'}`} key={policy.id} onClick={() => setSyncPolicyOpen(policy)} aria-haspopup="dialog" aria-label={`${policyLabel} ayarlarını aç`}><div className="sync-policy-identity"><div className="sync-policy-title"><strong>{policyLabel}</strong><span className={`sync-policy-boolean ${policy.enabled && !externalWriteBlocked ? 'is-on' : ''}`}><i aria-hidden="true" />{policy.enabled && !externalWriteBlocked ? 'Açık' : 'Kapalı'}</span></div><div className="sync-policy-meta"><small>Son tamamlanan: <DateText value={policy.lastSuccessAt} /></small><small>Son deneme: <DateText value={policy.lastAttemptAt ?? null} /></small><small>{policy.lastRequestCount ?? 0} istek · {policy.lastReceivedCount ?? 0} kayıt · {policy.lastFailedCount ?? 0} hata · {policy.lastRetryCount ?? 0} retry · {policy.lastRateLimitCount ?? 0} rate-limit{policy.consecutiveFailureCount ? ` · ${policy.consecutiveFailureCount} ardışık hata` : ''}</small></div>{externalWriteBlocked && <small className="sync-policy-health-help">Dış yazma kapalı; bu akış otomatik çalıştırılmaz.</small>}{policy.recoveryGapStatus && policy.recoveryGapStatus !== 'OK' && <small className={`sync-policy-gap sync-policy-gap-${policy.recoveryGapStatus.toLowerCase()}`}>Kurtarma durumu: {policy.recoveryGapStatus === 'UNKNOWN' ? 'ilk kontrol bekleniyor' : `${Math.round(policy.recoveryGapDays ?? 0)} günlük pencere`}</small> }<small className="sync-policy-open-hint">Ayrıntıları görmek için tıklayın</small></div></button>
      })}</div></div>)}</div>}{syncPolicyOpen && <SyncPolicyEditorModal policy={syncPolicyOpen} label={syncPolicyLabel(syncPolicyOpen.resourceType)} intervals={syncPolicyIntervals} overlaps={syncPolicyOverlaps} healthHelp={syncPolicyHealthHelp(syncPolicyOpen)} externalWriteBlocked={syncPolicyOpen.requiresExternalWrites === true && !item.externalWritesEnabled} saving={saveSyncPolicy.isPending} onClose={() => setSyncPolicyOpen(null)} onSave={values => saveSyncPolicy.mutate({ policy: syncPolicyOpen, ...values })} />}</section>}
