...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  // Some category snapshots call the marketplace color field simply “Renk”.
  // Treat that required option field as Web Color as well, otherwise it is
  // rendered as a normal category attribute and the color-value editor never
  // becomes reachable.
  const webColorRemote = remoteItems.find(item => isWebColorAttributeName(item.name))
    // Web Color may be exposed by Trendyol as the ordinary “Renk” field and
    // it is not required in every category snapshot. The local Renk option is
    // still the correct adapter source, so keep the field available whenever
    // that source exists instead of limiting it to one category shape.
    ?? (panelColorAttribute && (remoteItems.find(item => isPanelColorAttributeName(item.name)) ?? null))
