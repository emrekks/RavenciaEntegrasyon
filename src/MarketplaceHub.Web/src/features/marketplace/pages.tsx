...
function AttributeValueMappingEditor({ connectionId, categoryScope, attribute, externalAttributeId, allowSharedRemoteValues = false }: { connectionId: string; categoryScope: string; attribute: LocalAttribute; externalAttributeId: string; allowSharedRemoteValues?: boolean }) {
  ...
  async function saveAll() {
    ...
    if (!allowSharedRemoteValues && new Set(chosen).size !== chosen.length) return setNotice('Aynı Trendyol değeri birden fazla panel değerine eşlenemez.')
    setSaving(true); setNotice('')
    try {
      var changed = 0
      for (const localValue of localValues) {
  ...
  return <div className="mapping-step nested value-mapping-editor"><div className="value-mapping-heading"><div><h3>Değer eşleştirmeleri</h3><p>{localValues.length} panel değeri · {remoteValues.length} Trendyol değeri{allowSharedRemoteValues ? ' · aynı Web Color birden fazla panele bağlanabilir' : ''}</p></div><button type="button" disabled={saving} onClick={() => void saveAll()}>{saving ? 'Kaydediliyor…' : 'Tüm eşlemeleri kaydet'}</button></div>{notice && <p role="status" className="notice">{notice}</p>}<div className="value-mapping-rows">{localValues.map(localValue => <label key={localValue.id} className="value-mapping-row"><span>{localValue.value}</span><b><UiIcon name="arrowRight" /></b><select aria-label={`${localValue.value} Trendyol değeri`} value={selections[localValue.id] ?? ''} onChange={event => setSelections(current => ({ ...current, [localValue.id]: event.target.value }))}><option value="">Trendyol değeri seçin</option>{remoteValues.map(remote => <option key={remote.externalId} value={remote.externalId} disabled={!allowSharedRemoteValues && used.has(remote.externalId) && selections[localValue.id] !== remote.externalId}>{remote.name}</option>)}</select><small>{mappingByLocal.has(localValue.id) ? 'Eşlendi' : 'Bekliyor'}</small></label>)}</div></div>
}
