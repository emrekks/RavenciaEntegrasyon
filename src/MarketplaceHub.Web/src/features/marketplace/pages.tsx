...
function ReturnDecisionActions({ item, onNotice, selectedLineIds: controlledSelectedLineIds, onSelectedLineIdsChange }: ReturnDecisionActionsProps) {
  ...
  return <section className={`return-inline-action return-decision-action${isControlled ? ' has-product-selection' : ''}`}>
    ...
    <form className="form-grid" onSubmit={requestDecision}>
      ...
      <button type="submit" disabled={decision.isPending || !lines.length || (action === 'REJECT' && (reasons.isLoading || reasons.isError || !reasonId || (selectedReason?.evidenceRequired && !evidenceAssetIds.length)))}>{action === 'REJECT' ? 'Ret kararını gönder' : 'İadeyi onayla'}</button>
    </form>
    {confirmation && <Modal open title={confirmation.action === 'REJECT' ? 'Ret kararını onayla' : 'İade kararını onayla'} description="Aşağıdaki ürünler için işlem kuyruğa alınacak. Devam etmeden önce özeti kontrol edin." onClose={() => { if (!decision.isPending) setConfirmation(null) }} className="return-decision-confirmation" footer={<><button type="button" className="secondary" disabled={decision.isPending} onClick={() => setConfirmation(null)}>Vazgeç</button><button type="button" disabled={decision.isPending} onClick={() => decision.mutate(confirmation)}>{decision.isPending ? 'Gönderiliyor…' : 'Kararı onayla'}</button></>}>
