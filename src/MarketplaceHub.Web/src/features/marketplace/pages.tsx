...
type ReturnLine = { id: string; externalLineId: string; orderLineId: string; sku: string; barcode: string | null; title: string; quantity: number; disposedQuantity: number; remainingQuantity: number; unitPrice: number; imageUrl: string | null; hasInventoryMapping: boolean; optionSignature?: string | null; color?: string | null; size?: string | null; approvedAt?: string | null; systemNote?: string | null; reasonText?: string | null; sellerDescription?: string | null; platformApprovalReason?: string | null; platformDescription?: string | null }; type ReturnDetail = Omit<ReturnClaim, 'lines'> & { reasonCode: string | null; allowedActions: string[]; lines: ReturnLine[] | null; stockDispositionAvailable: boolean; externalWritesEnabled: boolean }; type ReturnIssueReason = { id: string; name: string; evidenceRequired: boolean }
...
function ReturnInlineActions({ item, selectedLineIds, onSelectedLineIdsChange }: { item: ReturnDetail; selectedLineIds?: string[]; onSelectedLineIdsChange?: (ids: string[]) => void }) {
  ...
  return <div className="return-inline-actions">{notice && <div className="notice return-action-toast" role="status">{notice}</div>}<ReturnReceiptButton item={item} onNotice={setNotice} /><ReturnDecisionActions item={item} onNotice={setNotice} selectedLineIds={selectedLineIds} onSelectedLineIdsChange={onSelectedLineIdsChange} /></div>
}
