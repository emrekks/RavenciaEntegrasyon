...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const categoryAttributeIds = new Set(categoryAttributes.map(attribute => attribute.id))
  ...
  const productRemoteItems = [...remoteItems.filter(item => !optionExternalIds.has(item.externalId) && item.externalId !== webColorRemote?.externalId && !isWebColorAttributeName(item.name))].sort((left, right) => Number(right.isRequired) - Number(left.isRequired) || left.name.localeCompare(right.name, 'tr'))
...
export function WebColorMappingCard({ connectionId, categoryScope, snapshotId, localAttribute, localAttributes, categoryAttributeIds, remoteAttribute, existingMapping, existingMappings, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttribute: LocalAttribute; localAttributes: LocalAttribute[]; categoryAttributeIds: Set<string>; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; existingMappings: CatalogMapping[]; onNotice: (value: string) => void }) {
  ...
  const categoryAttributes = selectableAttributes.filter(attribute => categoryAttributeIds.has(attribute.id))
  const panelAttributes = selectableAttributes.filter(attribute => !categoryAttributeIds.has(attribute.id))
