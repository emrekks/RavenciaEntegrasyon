...
type ProductImagePreview = { url: string; fallbackUrl?: string | null; title: string }
...
function ProductImagePreviewModal({ preview, onClose }: { preview: ProductImagePreview; onClose: () => void }) {
  ...
  return <div className="workspace-modal-backdrop product-image-backdrop" role="presentation" onMouseDown={onClose}><section className="workspace-modal product-image-modal" role="dialog" aria-modal="true" aria-label="Büyük ürün görseli" onMouseDown={event => event.stopPropagation()}><button type="button" className="modal-close product-image-modal-close" onClick={onClose} aria-label="Pencereyi kapat"><UiIcon name="close" /></button><div className="product-image-modal-body"><img src={source} alt="" loading="eager" fetchPriority="high" decoding="async" onError={() => { if (fallback) setSource(fallback) }} /></div></section></div>
}
