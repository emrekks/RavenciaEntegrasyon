...
function ReferenceConnectionCard({ item, onSaved }: { item: Connection; onSaved: () => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${hidden ? 'reference-integration-card-hidden' : connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`} onClickCapture={event => { const target = event.target; if (target instanceof Element && target.closest('.reference-menu-popover a')) { event.preventDefault(); setMenuOpen(false); setSettingsOpen(true) } }}>
    ...
    <footer className="reference-integration-footer"><button type="button" className={`reference-action ${connected ? 'secondary' : 'primary-action'}`} onClick={() => connected ? setDisconnectOpen(true) : activate.mutate()} disabled={disconnect.isPending || activate.isPending || dataVisibility.isPending || test.isPending}>{disconnect.isPending ? 'Pasife alınıyor…' : activate.isPending ? 'Aktif ediliyor…' : connected ? 'Pasife al' : 'Aktif et'}</button><button type="button" className="reference-action link-action" onClick={() => test.mutate()} disabled={test.isPending || disconnect.isPending || activate.isPending || dataVisibility.isPending}>{test.isPending ? 'Kontrol ediliyor…' : 'Bağlantıyı kontrol et'}</button><button type="button" className="reference-action primary-action" onClick={() => setSettingsOpen(true)} disabled={disconnect.isPending || activate.isPending || dataVisibility.isPending || test.isPending}>Bağlantı ayarları</button></footer>
...
export function IntegrationsPage() {
  ...
  return <section className="content f3 integrations-reference-page"><div className="integration-reference-heading"><div><h1>Platform Entegrasyonları</h1><p>Pazar yeri ve fatura sağlayıcılarınızla olan veri akışını yönetin.</p></div><div className="integration-heading-actions"><div className="integration-filter-wrap"><button type="button" className="integration-filter-button" aria-expanded={filterOpen} onClick={() => setFilterOpen(value => !value)}><UiIcon name="grid" /> Filtrele</button>{filterOpen && <div className="integration-filter-menu" role="menu" aria-label="Platform ve bağlantı durumu filtresi"><strong>Bağlantı durumu</strong>{(['ALL', 'ACTIVE', 'ATTENTION'] as const).map(value => <button type="button" role="menuitemradio" aria-checked={filter === value} key={value} className={filter === value ? 'active' : ''} onClick={() => setFilter(value)}><i aria-hidden="true" />{value === 'ALL' ? 'Tüm durumlar' : value === 'ACTIVE' ? 'Aktif bağlantılar' : 'Dikkat gerekenler'}</button>)}<strong className="integration-filter-menu-group">Platform</strong>{(['ALL', 'TRENDYOL', 'TRENDYOL_EFATURAM'] as const).map(value => <button type="button" role="menuitemradio" aria-checked={platformFilter === value} key={value} className={platformFilter === value ? 'active' : ''} onClick={() => setPlatformFilter(value)}><i aria-hidden="true" />{value === 'ALL' ? 'Tüm platformlar' : value === 'TRENDYOL' ? 'Trendyol' : 'Trendyol E-Faturam'}</button>)}</div>}</div><button type="button" className="integration-add-button" onClick={() => setCreateOpen(true)}><UiIcon name="plus" /> Ekle</button></div></div>
