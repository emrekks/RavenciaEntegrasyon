...
function ReturnReferenceRow({ item, order }: { item: ReturnClaim; order: Order | null }) {
  ...
  return <><div className="return-reference-row" role="row" style={{ '--return-line-count': Math.max(1, lines.length) } as CSSProperties}>
    ...
    <span className={`order-reference-invoice ${invoiceStatus === 'FATURA_BEKLIYOR' ? 'invoice-pending' : 'invoice-created'}`}><small>Satış tutarı</small><strong>{money(total)}</strong>{discountAmount > 0 && <><small>Satıcı indirimi: {money(discountAmount)}</small><small>Faturalandırılmış tutar</small><strong>{money(invoicedTotal)}</strong></>}<span className="invoice-state-badge">{invoiceStatusLabel(invoiceStatus)}</span></span>
    <span><b>{item.reasonText ?? 'Belirtilmedi'}</b></span>
    <span className="return-reference-status"><Badge value={item.status} />{item.actionDueAt && <small>{remainingText(item.actionDueAt)}</small>}<button type="button" className="return-detail-trigger" onClick={event => { event.preventDefault(); event.stopPropagation(); navigate(`/returns/${item.id}`) }}><UiIcon name="externalLink" /> Detaylı bilgi</button></span>
