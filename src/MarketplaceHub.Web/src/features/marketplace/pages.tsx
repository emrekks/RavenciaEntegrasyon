...
function OrderReferenceRow({ item, selected, onSelect, openMenu, onMenuChange, onInvoiceCreate, onInvoiceDetails, onInvoiceUpload, onCourierChange, onOrderStatusChange, onProcessOrder, processing, onPrintLabel, onPreviewImage, labelSettings, printedLabels }: { item: Order; selected: boolean; onSelect: (checked: boolean) => void; openMenu: 'invoice' | 'actions' | null; onMenuChange: (value: 'invoice' | 'actions' | null) => void; onInvoiceCreate: () => void; onInvoiceDetails: () => void; onInvoiceUpload: () => void; onCourierChange: () => void; onOrderStatusChange: () => void; onProcessOrder: () => void; processing?: boolean; onPrintLabel: (format: ShippingLabelFormat) => void; onPreviewImage: (preview: ProductImagePreview) => void; labelSettings: ShippingLabelSettings; printedLabels: Set<string> }) {
  ...
  return <article className={`order-reference-row ${item.derivedStatus.toLowerCase()} ${item.isMicroExport ? 'micro-export' : ''} ${openMenu ? 'menu-open' : ''}`}>
    <div className="order-reference-grid" style={{ gridTemplateRows: `repeat(${Math.max(1, lines.length)}, auto)` }}>
      ...
      <div className={`order-reference-invoice ${item.invoiceStatus === 'FATURA_BEKLIYOR' ? 'invoice-pending' : 'invoice-created'}${hasDiscount ? '' : ' invoice-no-discount'}`} aria-label={`Fatura durumu: ${invoiceLabel}`}>{hasDiscount && <><div className="order-invoice-metric"><small>Satış Tutarı:</small><strong>{money(saleTotal)}</strong></div><div className="order-invoice-metric"><span className="order-invoice-label"><small>Satıcı İndirim Tutarı:</small></span><strong>{money(item.discountAmount)}</strong></div></>}<div className="order-invoice-payable"><small>Faturalanacak Tutar:</small><strong>{money(invoiceableTotal)}</strong></div>{isCancelledOrder ? <InvoiceStatusBadge status={item.invoiceStatus} label={invoiceLabel} /> : <div className="order-invoice-actions">{(invoiceNeedsAction || !invoiceDocumentAvailable) && <InvoiceStatusBadge status={item.invoiceStatus} label={invoiceLabel} />}{!invoiceNeedsAction && (item.invoiceId ? <a className="invoice-document-link" href={`/api/v1/invoices/${item.invoiceId}/documents/latest/content`} download>Faturayı Gör</a> : item.invoiceDocumentUrl ? <a className="invoice-document-link" href={item.invoiceDocumentUrl} download>Faturayı Gör</a> : null)}<div className="row-menu" title={invoiceLabel}><button type="button" className="row-menu-trigger" onClick={event => toggleMenu('invoice', event)} aria-expanded={openMenu === 'invoice'}><span>Fatura işlemleri</span><UiIcon name="chevronDown" /></button>{openMenu === 'invoice' && <div ref={menuPopoverRef} className={`row-popover invoice-popover opens-${menuPlacement}`} role="menu">{invoiceNeedsAction ? <>{!isShopifyOrder && <button type="button" role="menuitem" className="create-invoice" onClick={onInvoiceCreate}>Fatura Oluştur</button>}<button type="button" role="menuitem" onClick={onInvoiceDetails}>Fatura Bilgileri</button>{isShopifyOrder && <button type="button" role="menuitem" onClick={() => { onMenuChange(null); setInvoiceStatusModalOpen(true) }}>Fatura Durumunu değiştir</button>}<button type="button" role="menuitem" onClick={onInvoiceUpload}>Fatura Yükle</button></> : <>{item.invoiceId ? <a role="menuitem" className="invoice-menu-view" href={`/api/v1/invoices/${item.invoiceId}/documents/latest/content`} download>Fatura Görüntüle</a> : item.invoiceDocumentUrl ? <a role="menuitem" className="invoice-menu-view" href={item.invoiceDocumentUrl} download>Fatura Görüntüle</a> : null}<button type="button" role="menuitem" onClick={onInvoiceDetails}>Fatura Bilgileri</button>{isShopifyOrder && <button type="button" role="menuitem" onClick={() => { onMenuChange(null); setInvoiceStatusModalOpen(true) }}>Fatura Durumunu değiştir</button>}<button type="button" role="menuitem" className="destructive" disabled>Fatura İptal Et</button></>}</div>}</div></div>}</div>
...
export function OrdersPage() {
  ...
  const pageItems = hasLocalFilters ? items.slice((page - 1) * pageSize, page * pageSize) : items; const selectablePageItems = pageItems.filter(item => item.derivedStatus.toUpperCase() !== 'CANCELLED'); const allPageSelected = selectablePageItems.length > 0 && selectablePageItems.every(item => selectedIds.includes(item.id)); const selectedOrders = pageItems.filter(item => selectedIds.includes(item.id)); const shopifySelectedOrders = selectedOrders.filter(item => item.platformCode === 'SHOPIFY'); const cargoEditableOrders = selectedOrders.filter(item => item.platformCode !== 'SHOPIFY'); const labelBatchItems = selectedOrders.filter(item => Boolean(orderDisplayShipment(item)?.cargoTrackingNumber ?? item.cargoTrackingNumber) && !isMicroExportLabelBlocked(item))
