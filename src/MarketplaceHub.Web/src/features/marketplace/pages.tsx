...
function ReturnReferenceRow({ item, order }: { item: ReturnClaim; order: Order | null }) {
  ...
  return <><div className="return-reference-row" role="row" style={{ '--return-line-count': Math.max(1, lines.length) } as CSSProperties}>
    ...
    <span className={`order-reference-invoice ${invoiceStatus === 'FATURA_BEKLIYOR' ? 'invoice-pending' : 'invoice-created'}`}><small>Satış tutarı</small><strong>{money(total)}</strong>{hasDiscount && <><small>Satıcı indirimi: {money(discountAmount)}</small><small>Faturalandırılmış tutar</small><strong>{money(invoicedTotal)}</strong></>}<span className="invoice-state-badge">{invoiceStatusLabel(invoiceStatus)}</span></span>
