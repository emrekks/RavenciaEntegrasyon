...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  const syncPolicyCategories: Array<{ key: string; label: string; resourceTypes: string[] }> = [
    ...
    { key: 'external-writes', label: 'Dış yazma akışları', resourceTypes: ['PRODUCT_WRITE', 'PRICE_STOCK_WRITE', 'SHIPMENT_WRITE', 'RETURN_WRITE'] },
  ...
  const visibleSyncPolicies = syncPolicies.data?.filter(policy => policy.resourceType !== 'PRODUCTS') ?? []
  ...
  return <div className={rootClass}><div className={`page-heading ${inline ? 'integration-inline-page-heading' : ''}`}><div>{inline ? <p className="lede integration-inline-meta">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p> : <><Link to="/integrations" className="back"><UiIcon name="arrowLeft" /> Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></>}</div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    {item.platformCode === 'TRENDYOL' && readConnectionReady && <section className="panel integration-detail-section sync-policy-panel" aria-labelledby="sync-policy-title"><div className="integration-detail-section-heading"><div><p className="eyebrow">OTOMATİK AKIŞ</p><h2 id="sync-policy-title">Veri eşitleme ayarları</h2><p>Sipariş, iade ve dış yazma akışlarını kategorilerine göre inceleyin; çalışma sıklığını ve güvenlik aralığını aşağıdaki kartlardan yönetin.</p></div><span className="integration-detail-section-badge">ARKA PLANDA</span></div>{syncPolicies.isLoading ? <Busy text="Eşitleme ayarları yükleniyor…" /> : syncPolicies.isError ? <ErrorBox error={syncPolicies.error} /> : <div className="sync-policy-list">{syncPolicyGroups.map(group => <div className="sync-policy-group" key={group.key}><div className="sync-policy-group-heading"><h3>{group.label}</h3><span>{group.policies.length} akış</span></div><div className="sync-policy-group-grid">{group.policies.map(policy => {
      const policyLabel = ({ ORDERS: 'Siparişler ve paket durumları', ORDER_RECOVERY: 'Sipariş geçmişi kurtarma', ORDER_LIFECYCLE: 'Açık sipariş yaşam döngüsü', ORDER_RECONCILE_SHORT: 'Kısa sipariş uzlaştırması', ORDER_RECONCILE_MEDIUM: 'Orta sipariş uzlaştırması', ORDER_RECONCILE_DAILY: 'Günlük sipariş uzlaştırması', ORDER_INVOICE_RECONCILIATION: 'Paket fatura kontrolü', RETURNS: 'İade talepleri', RETURN_LIFECYCLE: 'Açık iade yaşam döngüsü', RETURN_RECONCILE_SHORT: 'Kısa iade uzlaştırması', RETURN_RECONCILE_MEDIUM: 'Orta iade uzlaştırması', RETURN_RECONCILE_DAILY: 'Günlük iade uzlaştırması', STOCK_RECONCILE_SHORT: 'Kısa stok uzlaştırması', STOCK_RECONCILE_MEDIUM: 'Orta stok uzlaştırması', STOCK_RECONCILE_DAILY: 'Günlük stok uzlaştırması', PRODUCT_WRITE: 'Ürün gönderimleri', PRICE_STOCK_WRITE: 'Fiyat ve stok gönderimleri', SHIPMENT_WRITE: 'Kargo ve etiket işlemleri', RETURN_WRITE: 'İade kararları', REFERENCE_DATA: 'Katalog referansları' } as Record<string, string>)[policy.resourceType] ?? policy.resourceType
    ...
    {item.platformCode === 'TRENDYOL' && <section className="panel integration-detail-write-panel"><div className="integration-detail-write-heading"><div><h2>Trendyol’a dış yazma</h2><p>Önce ana anahtarı açın; ardından ürün, fiyat-stok, kargo ve iade gönderimlerini ayrı ayrı etkinleştirip çalışma sıklığını belirleyin.</p></div><span className={`integration-detail-write-status ${externalWritesEnabled ? 'is-enabled' : ''}`}>{externalWritesEnabled ? 'Dış yazma açık' : 'Salt-okunur'}</span></div><label className="integration-write-toggle"><input type="checkbox" checked={externalWritesEnabled} disabled={saveExternalWrites.isPending || !readConnectionReady} onChange={event => saveExternalWrites.mutate(event.target.checked)} /><span><strong>Trendyol’a dış yazmayı etkinleştir</strong><small>{externalWritesEnabled ? 'Aşağıdaki dış yazma akışları kendi ayarlarına göre çalışır.' : 'Panel yalnızca Trendyol’dan veri çeker; dışarıya veri göndermez.'}</small></span></label></section>}
