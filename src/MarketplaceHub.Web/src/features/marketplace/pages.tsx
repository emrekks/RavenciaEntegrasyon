...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const panelColorAttribute = localAttributes.find(attribute => attribute.isActive !== false && isPanelColorAttributeName(attribute.name))
    ?? localAttributes.find(attribute => attribute.isActive !== false && isWebColorAttributeName(attribute.name))
  ...
  const productAttributes = localAttributes.filter(attribute => attribute.isActive !== false)
...
function WebColorMappingCard({ connectionId, categoryScope, snapshotId, localAttribute, remoteAttribute, existingMapping, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttribute: LocalAttribute; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; onNotice: (value: string) => void }) {
  ...
  return <section className={`attribute-mapping-card option-mapping-card web-color-mapping-card ${mapped ? '' : 'unmapped'}`}><div className="attribute-mapping-card-head"><span className="attr-source">WEB</span><div><strong>Panel Renk → Trendyol Web Color</strong><small>{mapped ? 'Önce Web Color değerini seçin, sonra ona karşılık gelen birden fazla panel rengini işaretleyin. Üründe Tek Renk seçilirse yalnız seçilen değer gönderilir.' : 'Paneldeki renkleri Trendyol Web Color alanına bağlayın; aynı hedefe birden fazla panel rengi atanabilir.'}</small></div></div><div className="mapping-fields compact"><div className="mapping-fixed-target"><span>Trendyol karşılığı</span><strong>{remoteAttribute.name}</strong></div>{mapped ? <button type="button" className="secondary" onClick={() => void remove()}>Eşlemeyi kaldır</button> : <button type="button" onClick={() => void save()}>Web Color eşlemesini kaydet</button>}</div>{mapped && localAttribute.values.length > 0 && <><button type="button" className="value-mapping-toggle" onClick={() => setShowValues(value => !value)}>{showValues ? 'Web Color eşlemesini gizle' : `Web Color değerlerini eşle (${localAttribute.values.length})`}</button>{showValues && <WebColorValueMappingEditor connectionId={connectionId} categoryScope={categoryScope} attribute={localAttribute} externalAttributeId={remoteAttribute.externalId} />}</>}</section>
}
