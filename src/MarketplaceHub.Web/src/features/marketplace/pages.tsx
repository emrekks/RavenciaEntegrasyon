...
function WebColorMappingCard({ connectionId, categoryScope, snapshotId, localAttribute, remoteAttribute, existingMapping, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttribute: LocalAttribute; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; onNotice: (value: string) => void }) {
  ...
  return <section className={`attribute-mapping-card option-mapping-card web-color-mapping-card ${mapped ? '' : 'unmapped'}`}><div className="attribute-mapping-card-head"><span className="attr-source">WEB</span><div><strong>Panel Renk → Trendyol Web Color</strong><small>{mapped ? 'Aynı Web Color’a birden fazla panel rengi bağlanabilir. Üründe Tek Renk seçilirse yalnız seçilen değer gönderilir; kapalıysa her varyant kendi eşleşmesini kullanır.' : 'Paneldeki renkleri Trendyol Web Color değerlerine bağlayın; aynı hedefe birden fazla panel rengi atanabilir.'}</small></div></div><div className="mapping-fields compact"><div className="mapping-fixed-target"><span>Trendyol karşılığı</span><strong>{remoteAttribute.name}</strong></div>{mapped ? <button type="button" className="secondary" onClick={() => void remove()}>Eşlemeyi kaldır</button> : <button type="button" onClick={() => void save()}>Web Color eşlemesini kaydet</button>}</div>{mapped && localAttribute.values.length > 0 && <><button type="button" className="value-mapping-toggle" onClick={() => setShowValues(value => !value)}>{showValues ? 'Renk değerlerini gizle' : `Panel renklerini eşle (${localAttribute.values.length})`}</button>{showValues && <AttributeValueMappingEditor connectionId={connectionId} categoryScope={categoryScope} attribute={localAttribute} externalAttributeId={remoteAttribute.externalId} allowSharedRemoteValues />}</>}</section>
}
