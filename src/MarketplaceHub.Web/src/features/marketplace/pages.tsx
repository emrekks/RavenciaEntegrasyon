...
function ReturnReferenceRow({ item, order }: { item: ReturnClaim; order: Order | null }) {
  ...
  </div>{previewImage && <ProductImagePreviewModal preview={previewImage} onClose={() => setPreviewImage(null)} />}{detailOpen && <div className="workspace-modal-backdrop return-detail-backdrop" role="presentation" onMouseDown={() => setDetailOpen(false)}><section className="workspace-modal return-detail-modal" role="dialog" aria-modal="true" aria-label={`${item.orderNumber} iade detayı`} onMouseDown={event => event.stopPropagation()}><header><div><p className="eyebrow">İADE DETAYI</p><h2>Sipariş #{detailItem.orderNumber}</h2><p>{detailItem.customerName}</p></div><button type="button" className="modal-close" onClick={() => setDetailOpen(false)} aria-label="Pencereyi kapat"><UiIcon name="close" /></button></header>{detailQuery.isLoading ? <div className="return-detail-loading"><Busy text="İade detayları yükleniyor…" /></div> : detailQuery.isError ? <div className="return-detail-loading"><ErrorBox error={detailQuery.error} /></div> : <><div className="return-detail-summary"><div><small>Durum</small><Badge value={detailItem.status} /></div><div><small>İade sebebi</small><strong>{detailItem.reasonText ?? 'Belirtilmedi'}</strong></div><div><small>Kargo</small><strong>{detailTrackingNumber ?? 'Takip numarası yok'}</strong></div><div><small>Faturalandırılmış tutar</small><strong>{detailMoney(detailItem.orderAmount || invoicedTotal)}</strong></div><div><small>Talep tarihi</small><strong><DateText value={detailItem.orderedAt} /></strong></div><div><small>Onaylanma tarihi</small><strong>{detailItem.approvedAt ? <DateText value={detailItem.approvedAt} /> : '—'}</strong></div></div><h3>Ürünler</h3><div className="return-detail-products">{detailLines.map(line => { const color = returnLineField(line, ['renk', 'color'], 'color'); const size = returnLineField(line, ['beden', 'size'], 'size'); const approvedAt = returnLineField(line, ['onaylanma', 'approved'], 'approvedAt'); const systemNote = returnLineField(line, ['sistem notu', 'system'], 'systemNote'); const reasonText = returnLineField(line, ['sebep', 'reason'], 'reasonText'); const sellerDescription = returnLineField(line, ['satıcı açıklaması', 'seller'], 'sellerDescription'); const platformReason = returnLineField(line, ['trendyol onay nedeni', 'approval reason'], 'platformApprovalReason'); const platformDescription = returnLineField(line, ['trendyol açıklaması', 'platform description'], 'platformDescription'); return <article className="return-detail-product" key={line.id}><div className="return-detail-product-head">{line.imageUrl ? <img src={line.imageUrl} alt={`${line.title} ürün görseli`} /> : <span className="reference-product-placeholder">↩</span>}<div><strong>{line.title}</strong><small>{line.quantity} adet · {detailMoney(line.unitPrice)}</small></div></div><dl className="return-detail-product-facts"><div><dt>Stok Kodu</dt><dd>{line.sku || '—'}</dd></div><div><dt>Renk</dt><dd>{color}</dd></div><div><dt>Barkod</dt><dd>{line.barcode ?? '—'}</dd></div><div><dt>Beden</dt><dd>{size}</dd></div><div><dt>Onaylanma Tarihi</dt><dd>{approvedAt !== '—' ? <DateText value={approvedAt} /> : detailItem.approvedAt ? <DateText value={detailItem.approvedAt} /> : '—'}</dd></div><div><dt>Sistem Notu</dt><dd>{systemNote !== '—' ? systemNote : detailItem.systemNote ?? '—'}</dd></div><div><dt>Sebep</dt><dd>{reasonText !== '—' ? reasonText : detailItem.reasonText ?? '—'}</dd></div><div><dt>Satıcı Açıklaması</dt><dd>{sellerDescription !== '—' ? sellerDescription : detailItem.sellerDescription ?? '—'}</dd></div><div><dt>Trendyol Onay Nedeni</dt><dd>{platformReason !== '—' ? platformReason : detailItem.platformApprovalReason ?? '—'}</dd></div><div><dt>Trendyol Açıklaması</dt><dd>{platformDescription !== '—' ? platformDescription : detailItem.platformDescription ?? '—'}</dd></div></dl></article> })}</div><ReturnInlineActions item={detailQuery.data!} /></>}<footer><button type="button" className="secondary" onClick={() => setDetailOpen(false)}>Kapat</button></footer></section></div>}</>
}
...
function ReturnDecisionActions({ item, onNotice }: { item: ReturnDetail; onNotice?: (message: string) => void }) {
  const client = useQueryClient(); const lines = item.lines ?? []; const [action, setAction] = useState<'APPROVE' | 'REJECT'>('APPROVE'); const [reasonId, setReasonId] = useState(''); const [evidenceAssetIds, setEvidenceAssetIds] = useState<string[]>([]); const [selectedLineIds, setSelectedLineIds] = useState<string[]>(() => lines.map(line => line.id)); const [confirmation, setConfirmation] = useState<ReturnDecisionDraft | null>(null)
  useEffect(() => { setSelectedLineIds(lines.map(line => line.id)); setConfirmation(null) }, [item.id, item.version, item.lines])
