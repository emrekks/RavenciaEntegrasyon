...
function InvoiceUploadModal({ item, provider, onClose, onFeedback }: { item: Order; provider: Connection | null; onClose: () => void; onFeedback: IntegrationFeedbackHandler }) {
  ...
  async function ensureShopifyInvoice(order: OrderDetail) {
  ...
  const manualStatus = useMutation({
    mutationFn: async (status: 'PENDING' | 'UPLOADED') => {
      ...
      const invoiceId = await ensureShopifyInvoice(order)
