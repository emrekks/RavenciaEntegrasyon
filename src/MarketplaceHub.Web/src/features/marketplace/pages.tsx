...
function ReturnReferenceRow({ item, order, onNotice }: { item: ReturnClaim; order: Order | null; onNotice?: (message: string) => void }) {
...
type ReturnDecisionActionsProps = { item: ReturnDetail; onNotice?: (message: string) => void; selectedLineIds?: string[]; onSelectedLineIdsChange?: (ids: string[]) => void }
...
function ReturnDecisionActions({ item, onNotice, selectedLineIds: controlledSelectedLineIds, onSelectedLineIdsChange }: ReturnDecisionActionsProps) {
  ...
  function requestDecision(event: FormEvent<HTMLFormElement>) {
    ...
    if (externalWritesDisabled) { onNotice?.('Dış yazma kapalı olduğu için iade kararı gönderilemez.'); return }
    if (!selectedLineIds.length) { onNotice?.('İşlem yapılacak en az bir ürün seçin.'); return }
...
function ReturnInlineActions({ item, onNotice, selectedLineIds, onSelectedLineIdsChange }: { item: ReturnDetail; onNotice?: (message: string) => void; selectedLineIds?: string[]; onSelectedLineIdsChange?: (ids: string[]) => void }) {
...
export function ReturnsPage() {
  ...
  return <section className="content f3 returns-page reference-returns-page">
    ...
    {notice && <div role="status" className="notice return-action-toast">{notice}</div>}
