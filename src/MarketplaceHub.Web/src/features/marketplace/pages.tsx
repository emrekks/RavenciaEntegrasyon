...
export function OrdersPage() {
  ...
  return <section className="content f3 orders-page"><div className="page-heading"><div><p className="eyebrow">Sipariş yönetimi</p><h1>Sipariş Yönetimi</h1><p className="lede">Tüm pazar yeri siparişlerinizi tek merkezden yönetin ve takip edin.</p></div><div className="page-heading-actions orders-reference-heading-actions"><button type="button" className="secondary orders-export-action" disabled={!items.length || exporting} onClick={() => void exportOrders()}><UiIcon name="download" className="orders-export-icon" size={18} />{exporting ? 'Dışa aktarılıyor…' : 'Dışa Aktar'}</button><button type="button" className="orders-sync-action" onClick={() => setSingleSyncOpen(true)}><UiIcon name="sync" className="orders-sync-icon" size={18} /> Sipariş Senkronizasyonu</button></div></div>
    ...
    <div className="orders-reference-filter-shell"><div className="order-tabs" role="tablist" aria-label="Sipariş durumları">{statuses.map(([value,label]) => <button type="button" role="tab" aria-selected={filters.status === value} className={filters.status === value ? 'active' : ''} key={value} onClick={() => selectStatus(value)}><span className="status-tab-label">{label}</span><b className="status-tab-count">{summary.isLoading ? '…' : summary.isError ? '—' : tabCount(value)}</b><small className="status-tab-unit">Paket</small></button>)}</div></div>
...
export function ReturnsPage() {
  ...
  return <section className="content f3 returns-page reference-returns-page">
    ...
    <div className="returns-reference-filter-shell"><div className="return-reference-tabs" role="tablist" aria-label="İade durumları">{tabs.map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={status === value} className={status === value ? 'active' : ''} onClick={() => selectStatus(value)}><strong className="status-tab-label">{label}</strong><b className="status-tab-count">{countsQuery.isLoading && status !== 'ALL' ? '…' : value === 'ALL' ? countItems.length : countItems.filter(item => returnGroup(item.status) === value).length}</b><small className="status-tab-unit">Paket</small></button>)}</div>
