...
export function WebColorMappingCard({ connectionId, categoryScope, snapshotId, localAttribute, remoteAttribute, existingMapping, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttribute: LocalAttribute; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; onNotice: (value: string) => void }) {
  const client = useQueryClient(); const [showValues, setShowValues] = useState(false)
  const mapped = existingMapping?.externalId === remoteAttribute.externalId
  async function save() {
    try {
      await hubApi<CatalogMapping>(`/mappings/attributes/${localAttribute.id}`, { method: 'PUT', headers: existingMapping ? { 'If-Match': `"v${existingMapping.version}"` } : {}, body: JSON.stringify({ connectionId, snapshotId, scopeExternalId: categoryScope, externalId: remoteAttribute.externalId, status: 'VERIFIED', role: 'OPTION' }) })
      onNotice('Panel Renk → Trendyol Web Color eşlemesi kaydedildi.'); await client.invalidateQueries({ queryKey: ['attribute-mappings', connectionId, categoryScope] })
    } catch (reason) { onNotice(reason instanceof Error ? reason.message : 'Web Color eşlemesi kaydedilemedi.') }
  }
  async function remove() {
    if (!existingMapping) return
    try { await hubApi<boolean>(`/mappings/attributes/${localAttribute.id}?connectionId=${encodeURIComponent(connectionId)}&scopeExternalId=${encodeURIComponent(categoryScope)}`, { method: 'DELETE', headers: { 'If-Match': `"v${existingMapping.version}"` } }); setShowValues(false); onNotice('Panel Renk → Trendyol Web Color eşlemesi kaldırıldı.'); await client.invalidateQueries({ queryKey: ['attribute-mappings', connectionId, categoryScope] }) } catch (reason) { onNotice(reason instanceof Error ? reason.message : 'Web Color eşlemesi kaldırılamadı.') }
  }
  return <section className="attribute-mapping-card required"><div className="attribute-mapping-card-head"><span className="attr-source">TY</span><div><strong>{remoteAttribute.name}</strong><small>{mapped ? 'Zorunlu alan · eşlendi' : 'Zorunlu alan'}</small></div></div><div className="mapping-fields compact"><label>Entegrasyon ürün özelliği<select aria-label="Web Color için panel özelliği" value={localAttribute.id} disabled><option value={localAttribute.id}>{localAttribute.name}</option></select></label>{mapped ? <button type="button" className="secondary" onClick={() => void remove()}>Eşlemeyi kaldır</button> : <button type="button" onClick={() => void save()}>Kaydet</button>}</div>{mapped && localAttribute.values.length > 0 && <><button type="button" className="value-mapping-toggle" onClick={() => setShowValues(value => !value)}>{showValues ? 'Değer eşlemelerini gizle' : `Değer eşlemelerini aç (${localAttribute.values.length})`}</button>{showValues && <WebColorValueMappingEditor connectionId={connectionId} categoryScope={categoryScope} attribute={localAttribute} externalAttributeId={remoteAttribute.externalId} />}</>}</section>
}
