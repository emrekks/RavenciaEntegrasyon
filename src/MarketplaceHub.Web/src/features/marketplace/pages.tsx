...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  const syncPolicyCategories: Array<{ key: string; label: string; resourceTypes: string[] }> = [
    ...
    { key: 'external-writes', label: 'Dış Yazma Akışları', resourceTypes: ['PRICE_WRITE', 'STOCK_WRITE', 'SHIPMENT_WRITE', 'RETURN_WRITE'] },
  ...
  const syncPolicyDescription = (resourceType: string) => ({
    ...
    {item.platformCode === 'TRENDYOL' && <section className="panel integration-detail-write-panel"><div className="integration-detail-write-heading"><div><h2>Trendyol’a dış yazma</h2><p>Ürün gönderimleri manuel yapılır. Fiyat, stok, kargo ve iade akışlarını ayrı ayrı etkinleştirip çalışma sıklığını belirleyin.</p></div><span className={`integration-detail-write-status ${externalWritesEnabled ? 'is-enabled' : ''}`}>{externalWritesEnabled ? 'Dış yazma açık' : 'Salt-okunur'}</span></div><label className="integration-write-toggle"><input type="checkbox" checked={externalWritesEnabled} disabled={saveExternalWrites.isPending || !readConnectionReady} onChange={event => saveExternalWrites.mutate(event.target.checked)} /><span><strong>Trendyol’a dış yazmayı etkinleştir</strong><small>{externalWritesEnabled ? 'Aşağıdaki otomatik dış yazma akışları kendi ayarlarına göre çalışır.' : 'Panel yalnızca Trendyol’dan veri çeker; dışarıya veri göndermez.'}</small></span></label></section>}
