...
function ReferenceConnectionCard({ item, onSaved, onFeedback }: { item: Connection; onSaved: () => void; onFeedback: (feedback: IntegrationFeedback) => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${hidden ? 'reference-integration-card-hidden' : connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`} onClickCapture={event => { const target = event.target; if (target instanceof Element && target.closest('.reference-menu-popover a')) { event.preventDefault(); setMenuOpen(false); setSettingsOpen(true) } }}>
    ...
    {privateDataHidden ? <div className="reference-integration-private-state" role="status"><strong>Bağlantı pasif</strong><span>Bağlantı bilgileri güvenlik için gizlendi.</span></div> : <div className="reference-integration-body"><div className="reference-data-point"><span>{efaturam ? 'VKN / TCKN' : 'Mağaza Kimliği (Seller ID)'}</span><strong>{item.externalStoreId || 'Tanımlanmadı'}</strong></div><div className="reference-data-point"><span>Credential</span><strong>{credentialLabel(item)}</strong></div><div className="reference-data-point"><span>Ortam</span><strong>{item.environment === 'PRODUCTION' ? 'Canlı' : 'Stage'}</strong></div><div className="reference-data-point"><span>Dış yazma</span><strong>{item.externalWritesEnabled ? 'Açık' : 'Salt-okunur'}</strong></div></div>}
    <footer className="reference-integration-footer"><span>{efaturam ? 'Ravencia hesabı' : 'Ravencia mağazası'}</span><button type="button" className="reference-action primary-action" onClick={() => setSettingsOpen(true)} disabled={disconnect.isPending || activate.isPending || test.isPending}><UiIcon name="settings" /> Yönet</button></footer>
...
export function IntegrationsPage() {
  ...
  const filterLabel = `${view === 'ACTIVE' ? 'Bağlı olanlar' : 'Tümü'} · ${platformFilter === 'ALL' ? 'Tüm platformlar' : platformFilter === 'TRENDYOL' ? 'Trendyol' : 'Trendyol E-Faturam'} · ${filter === 'ACTIVE' ? 'Aktif bağlantılar' : filter === 'ATTENTION' ? 'Dikkat gerekenler' : 'Tüm durumlar'}`
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  if (!activePlatformCodes.has(item.platformCode)) return <div className={rootClass}><Link to="/integrations" className="back"><UiIcon name="arrowLeft" /> Platformlar</Link><div className="unknown"><strong>Bağlantı aktif kapsam dışında</strong><p>ADR-016 uyarınca panel yalnız Trendyol ve Trendyol E-Faturam ile ilerler. Kapsam dışı tarihsel bağlantılara işlem sunulmaz.</p></div></div>
  ...
  const syncPolicyDescription = (resourceType: string) => ({
    ORDERS: 'Trendyol’daki yeni ve güncellenen siparişleri panele alır; ürün, müşteri, paket ve kargo bilgilerini yerel çalışma ekranlarında güncel tutar.',
  ...
  return <div className={rootClass}><div className={`page-heading ${inline ? 'integration-inline-page-heading' : ''}`}><div>{inline ? <p className="lede integration-inline-meta">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p> : <><Link to="/integrations" className="back"><UiIcon name="arrowLeft" /> Platformlar</Link><h1>{item.displayName}</h1><p className="lede">{item.environment} · {item.platformCode === 'TRENDYOL_EFATURAM' ? 'yerel kapsam' : 'mağaza'} {item.externalStoreId} · API {item.apiVersion}</p></>}</div><Badge value={item.status} /></div>{notice && <div role="status" className="notice">{notice}</div>}
    ...
    {item.platformCode === 'TRENDYOL' && readConnectionReady && <section className="panel integration-detail-section sync-policy-panel" aria-labelledby="sync-policy-title"><div className="integration-detail-section-heading"><div><p className="eyebrow">OTOMATİK AKIŞ</p><h2 id="sync-policy-title">Veri eşitleme ayarları</h2><p>Sipariş, iade ve dış yazma akışlarını kategorilerine göre inceleyin; çalışma sıklığını ve güvenlik aralığını aşağıdaki kartlardan yönetin.</p></div><span className="integration-detail-section-badge">ARKA PLANDA</span></div>{syncPolicies.isLoading ? <Busy text="Eşitleme ayarları yükleniyor…" /> : syncPolicies.isError ? <ErrorBox error={syncPolicies.error} /> : <div className="sync-policy-list">{syncPolicyGroups.map(group => <div className="sync-policy-group" key={group.key}><div className="sync-policy-group-heading"><h3>{group.label}</h3><span>{group.policies.length} akış</span></div><div className="sync-policy-group-grid">{group.policies.map(policy => {
...
function ReturnDecisionActions({ item, onNotice, selectedLineIds: controlledSelectedLineIds, onSelectedLineIdsChange }: ReturnDecisionActionsProps) {
  ...
  const decision = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      const response = result.rawStatus ? ` Trendyol yanıtı: ${result.rawStatus}.` : ''
      onNotice?.(`${result.status === 'APPROVED' ? 'İade kabulü' : result.status === 'REJECTED' ? 'İade reddi' : 'İade kararı'} Trendyol’a anlık olarak iletildi.${response}`)
      await client.invalidateQueries({ queryKey: ['return', item.id] })
      await client.invalidateQueries({ queryKey: ['returns'] })
    },
...
function ReturnInlineActions({ item, selectedLineIds, onSelectedLineIdsChange }: { item: ReturnDetail; selectedLineIds?: string[]; onSelectedLineIdsChange?: (ids: string[]) => void }) {
  const [notice, setNotice] = useState('')
  return <div className="return-inline-actions">{notice && <div className="notice return-action-toast" role="status">{notice}</div>}{item.decisionPending ? <div className="unknown"><strong>Karar Trendyol’a gönderildi</strong><p>Bu iade kararı Trendyol’a anlık olarak iletildi. Trendyol’dan kesin durum yanıtı gelene kadar yeni bir karar gönderilemez.</p></div> : <><ReturnReceiptButton item={item} onNotice={setNotice} /><ReturnDecisionActions item={item} onNotice={setNotice} selectedLineIds={selectedLineIds} onSelectedLineIdsChange={onSelectedLineIdsChange} /></>}</div>
}
