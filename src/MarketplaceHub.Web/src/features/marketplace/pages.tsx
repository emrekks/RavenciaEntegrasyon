...
function ReturnDecisionActions({ item, onNotice }: { item: ReturnDetail; onNotice?: (message: string) => void }) {
  ...
  return <section className="return-inline-action return-decision-action"><div><h3>İade işlemi</h3><p>İade paketi kontrol edildikten sonra kabul veya ret kararını buradan gönderin.</p></div><form className="form-grid" onSubmit={event => { event.preventDefault(); const data = new FormData(event.currentTarget); decision.mutate({ action, reasonCode: action === 'REJECT' ? reasonId : null, explanation: action === 'REJECT' ? String(data.get('explanation') ?? '') : null }) }}><label>Karar<select value={action} onChange={event => setAction(event.target.value as 'APPROVE' | 'REJECT')}>{decisionActions.map(value => <option key={value} value={value}>{statusLabel(value)}</option>)}</select></label>{action === 'REJECT' && <><label>Trendyol ret nedeni<select value={reasonId} onChange={event => setReasonId(event.target.value)} required><option value="">Seçin</option>{reasons.data?.map(reason => <option key={reason.id} value={reason.id}>{reason.name}</option>)}</select></label><label>Açıklama<textarea name="explanation" maxLength={500} required /></label><label>Kanıt (PDF/JPEG/PNG, en fazla 10 MiB)<input name="file" type="file" accept="application/pdf,image/jpeg,image/png" onChange={event => { const file = event.currentTarget.files?.[0]; if (file) void uploadFile(file) }} /></label><small>{evidenceAssetIds.length} kanıt hazır{selectedReason?.evidenceRequired ? ' · bu neden için zorunlu' : ''}</small></>}<button disabled={decision.isPending || (action === 'REJECT' && (!reasonId || (selectedReason?.evidenceRequired && !evidenceAssetIds.length)))}>{decision.isPending ? 'Gönderiliyor…' : action === 'REJECT' ? 'Ret kararını gönder' : 'İadeyi onayla'}</button></form></section>
}
