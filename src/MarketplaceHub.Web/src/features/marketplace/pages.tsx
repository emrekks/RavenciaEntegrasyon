...
function CategoryAttributeWorkspace({ connectionId, localCategoryId, localCategories, localAttributes, onNotice }: { connectionId: string; localCategoryId: string; localCategories: LocalCategory[]; localAttributes: LocalAttribute[]; onNotice: (value: string) => void }) {
  ...
  const webColorAttributes = [...optionAttributes, ...(panelColorAttribute ? [panelColorAttribute] : []), ...(specialColorAttribute ? [specialColorAttribute] : [])]
    .filter((attribute, index, items) => items.findIndex(candidate => candidate.id === attribute.id) === index)
...
export function WebColorMappingCard({ connectionId, categoryScope, snapshotId, localAttribute, localAttributes, remoteAttribute, existingMapping, existingMappings, onNotice }: { connectionId: string; categoryScope: string; snapshotId: string; localAttribute: LocalAttribute; localAttributes: LocalAttribute[]; remoteAttribute: ReferenceItem; existingMapping: CatalogMapping | null; existingMappings: CatalogMapping[]; onNotice: (value: string) => void }) {
