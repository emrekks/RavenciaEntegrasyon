...
function OrderReferenceRow({ item, selected, onSelect, openMenu, onMenuChange, onInvoiceCreate, onInvoiceDetails, onInvoiceUpload, onCourierChange, onProcessOrder, processing, onPrintLabel, onPreviewImage, labelSettings, printedLabels }: { item: Order; selected: boolean; onSelect: (checked: boolean) => void; openMenu: 'invoice' | 'actions' | null; onMenuChange: (value: 'invoice' | 'actions' | null) => void; onInvoiceCreate: () => void; onInvoiceDetails: () => void; onInvoiceUpload: () => void; onCourierChange: () => void; onProcessOrder: () => void; processing?: boolean; onPrintLabel: (format: ShippingLabelFormat) => void; onPreviewImage: (preview: ProductImagePreview) => void; labelSettings: ShippingLabelSettings; printedLabels: Set<string> }) {
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  const stageManual = item.platformCode !== 'SHOPIFY' && item.environment === 'STAGE'; const displayEnvironment = item.platformCode === 'SHOPIFY' ? 'PRODUCTION' : item.environment; const readConnectionReady = item.status === 'ACTIVE' || item.status === 'VERIFIED'; const stageReady = stageManual && !item.lastErrorCode && readConnectionReady
   ...
   const syncPolicyCategories: Array<{ key: string; label: string; resourceTypes: string[]; kind?: 'external-write-parent'; description?: string }> = [
    { key: 'products', label: 'Ürün Akışları', resourceTypes: ['PRODUCTS'] },
  ...
  const visibleSyncPolicies = syncPolicies.data?.filter(policy => !['PRODUCT_WRITE', 'PRICE_STOCK_WRITE'].includes(policy.resourceType)) ?? []
  ...
  const syncPolicyDescription = (resourceType: string) => ({
    PRODUCTS: item.platformCode === 'SHOPIFY' ? 'Shopify ürünlerini ve varyant bağlantılarını günlük olarak okur; eşleşme durumunu kontrol eder, Ravencia’nın ana fiyat ve stoğunu değiştirmez.' : 'Trendyol ürün bağlantılarını günlük olarak kontrol eder ve eksik veya değişen katalog kayıtlarını panele alır.',
...
export function OrdersPage() {
  ...
  const pageItems = hasLocalFilters ? items.slice((page - 1) * pageSize, page * pageSize) : items; const selectablePageItems = pageItems.filter(item => item.derivedStatus.toUpperCase() !== 'CANCELLED'); const allPageSelected = selectablePageItems.length > 0 && selectablePageItems.every(item => selectedIds.includes(item.id)); const selectedOrders = pageItems.filter(item => selectedIds.includes(item.id)); const labelBatchItems = selectedOrders.filter(item => Boolean(item.packages?.[0]?.cargoTrackingNumber ?? item.cargoTrackingNumber) && !isMicroExportLabelBlocked(item))
