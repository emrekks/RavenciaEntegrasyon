...
function ReferenceConnectionCard({ item, onSaved, onFeedback }: { item: Connection; onSaved: () => void; onFeedback: (feedback: IntegrationFeedback) => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${hidden ? 'reference-integration-card-hidden' : connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`} onClickCapture={event => { const target = event.target; if (target instanceof Element && target.closest('.reference-menu-popover a')) { event.preventDefault(); setMenuOpen(false); setSettingsOpen(true) } }}>
    ...
    <header className="reference-integration-header">
     ...
     {privateDataHidden ? <div className="reference-integration-private-state" role="status"><strong>Bağlantı pasif</strong><span>Bağlantı bilgileri güvenlik için gizlendi.</span></div> : <div className="reference-integration-body"><div className="reference-data-point"><span>{efaturam ? 'VKN / TCKN' : shopify ? 'Shopify mağazası' : 'Mağaza Kimliği (Seller ID)'}</span><strong>{item.externalStoreId || 'Tanımlanmadı'}</strong></div><div className="reference-data-point"><span>Credential</span><strong>{credentialLabel(item)}</strong></div><div className="reference-data-point"><span>Ortam</span><strong>{shopify || item.environment === 'PRODUCTION' ? 'Canlı' : 'Stage'}</strong></div><div className="reference-data-point"><span>Dış yazma</span><strong>{item.externalWritesEnabled ? 'Açık' : 'Salt-okunur'}</strong></div></div>}
...
function IntegrationDetailWorkspace({ id, inline = false }: { id: string; inline?: boolean }) {
  ...
  const saveConnectionDetails = useMutation({ mutationFn: () => { if (!connection.data) throw new Error('Bağlantı bilgisi yükleniyor.'); const value = externalStoreId.trim(); if (!value) throw new Error('Mağaza kimliği veya bağlantı kapsamı zorunludur.'); return hubApi<Connection>(`/connections/${id}`, { method: 'PATCH', headers: { 'If-Match': `\"v${connection.data.version}\"` }, body: JSON.stringify({ displayName: connection.data.displayName, externalStoreId: value }) }) }, onSuccess: async result => { setExternalStoreId(result.externalStoreId); setFeedback({ kind: 'success', message: 'Mağaza bilgileri güncellendi. Yeniden bağlantı testi gerekiyor.' }); await client.invalidateQueries({ queryKey: ['connection', id] }); await client.invalidateQueries({ queryKey: ['connections'] }) }, onError: reason => setFeedback({ kind: 'error', message: reason instanceof Error ? reason.message : 'Mağaza bilgileri güncellenemedi.' }) })
  ...
  const syncPolicyDescription = (resourceType: string) => ({
    ...
    ORDER_RECOVERY: 'Normal sipariş taramasında kaçırılmış veya geçmiş tarihli siparişleri yeniden arayarak eksik kayıtları tamamlar.',
    ORDER_LIFECYCLE: 'Açık siparişlerin paket, kargo ve teslimat durumlarını izler; durum değişikliklerini panelde günceller.',
    ...
    PRICE_WRITE: 'Panelde yapılan fiyat değişikliklerini Trendyol’a gönderir. Kapalıysa fiyat güncellemeleri dış platforma yazılmaz.',
    STOCK_WRITE: 'Paneldeki stok değişikliklerini Trendyol’a gönderir. Kapalıysa stok güncellemeleri dış platforma yazılmaz.',
    SHIPMENT_WRITE: 'Kargo firması, etiket ve paket dış yazma işlemlerini Trendyol’a aktarır.',
    RETURN_WRITE: 'İade kabul veya ret kararlarını Trendyol API’sine doğrudan gönderir; yanıt beklenerek paneldeki iade durumu güncellenir.',
    REFERENCE_DATA: 'Kategori, özellik ve diğer katalog referanslarını Trendyol’dan çekerek eşleştirme ekranlarını güncel tutar.'
...
export function OrdersPage() {
  ...
  function showBulkNotice(message: string, kind: 'success' | 'error' = 'success') { setBulkNotice(message); setBulkNoticeKind(kind); setBulkNoticeVersion(current => current + 1) }
