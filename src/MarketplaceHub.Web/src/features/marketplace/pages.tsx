...
function ReferenceConnectionCard({ item, onSaved }: { item: Connection; onSaved: () => void }) {
  ...
  return <article className={`integration-card reference-integration-card ${hidden ? 'reference-integration-card-hidden' : connected ? 'reference-integration-card-active' : 'reference-integration-card-error'}`} onClickCapture={event => { const target = event.target; if (target instanceof Element && target.closest('.reference-menu-popover a')) { event.preventDefault(); setMenuOpen(false); setSettingsOpen(true) } }}>
    ...
    <div className="reference-integration-body"><div className="reference-data-point"><span>{efaturam ? 'VKN / TCKN' : 'Mağaza Kimliği (Seller ID)'}</span><strong>{item.externalStoreId || 'Tanımlanmadı'}</strong></div><div className="reference-data-point"><span>Credential</span><strong>{credentialLabel(item)}</strong></div><div className="reference-data-point"><span>Ortam</span><strong>{item.environment === 'PRODUCTION' ? 'Canlı' : 'Stage'}</strong></div><div className="reference-data-point"><span>Dış yazma</span><strong>{item.externalWritesEnabled ? 'Açık' : 'Salt-okunur'}</strong></div></div>
    {notice && <div className="reference-card-notice" role="status">{notice}</div>}
    <footer className="reference-integration-footer"><span>{efaturam ? 'Ravencia hesabı' : 'Ravencia mağazası'}</span><button type="button" className="reference-action primary-action" onClick={() => setSettingsOpen(true)} disabled={disconnect.isPending || activate.isPending || dataVisibility.isPending || test.isPending}><UiIcon name="settings" /> Yönet</button></footer>
    ...
    {visibilityOpen && <ReferenceActionDialog id={`visibility-${item.id}`} title="Platform verilerini gizle" description="Platformdan alınan yerel veriler gizlenecek; hiçbir kayıt silinmeyecek. Bağlantı pasif kalır ve veriler tekrar gösterilebilir." confirmLabel={dataVisibility.isPending ? 'Gizleniyor…' : 'Verileri gizle'} confirmDisabled={dataVisibility.isPending} onClose={() => setVisibilityOpen(false)} onConfirm={() => dataVisibility.mutate(true)} />}
    {deleteOpen && <ReferenceActionDialog id={`delete-${item.id}`} title="Bağlantıyı ve verileri sil" description="Bağlantı ve bu mağazaya bağlı sipariş, iade, fatura, ürün ve eşleştirme kayıtları kalıcı olarak silinir." confirmLabel={deepDelete.isPending ? 'Temizleniyor…' : 'Kalıcı olarak sil'} confirmDisabled={deleteConfirmation !== item.displayName || deepDelete.isPending} destructive onClose={() => { setDeleteOpen(false); setDeleteConfirmation('') }} onConfirm={() => deepDelete.mutate()}><label className="reference-action-confirmation">Onay için <b>{item.displayName}</b> yazın<input value={deleteConfirmation} onChange={event => setDeleteConfirmation(event.target.value)} /></label></ReferenceActionDialog>}
