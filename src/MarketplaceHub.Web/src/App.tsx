...
function ShippingLabelSettingsPanel({ settings, onChange, onSave }: { settings: ShippingLabelSettings; onChange: (value: ShippingLabelSettings) => void; onSave: () => void }) {
  ...
  function saveTemplate() {
    const name = window.prompt('Şablon adı', `${format === 'a4' ? 'A4' : 'Sticker'} şablonu`)
  ...
  function applyTemplate(id: string) {
    ...
    if (template) onChange(JSON.parse(JSON.stringify(template.settings)) as ShippingLabelSettings)
  }
