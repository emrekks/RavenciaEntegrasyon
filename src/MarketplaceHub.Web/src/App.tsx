...
function Shell({ me }: { me: Me }) {
  ...
  return <div className={`app-shell stitch-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main>
      <header className="topbar"><div className="topbar-workspace"><div className="breadcrumb"><strong>Operasyon Merkezi</strong><span>{current}</span></div></div><WorkspaceTopActions me={me} onLogout={logout} /></header>
...
function Security() {
  ...
  function resetScopeOption(scope: string) {
    ...
    return <label key={scope}><input type="checkbox" checked={resetScopes.includes(scope)} onChange={event => toggleResetScope(scope, event.target.checked)} /><span className="database-scope-label"><strong>{detail.label}</strong><span className="database-scope-help" role="img" tabIndex={0} data-tooltip={detail.description} aria-label={`${detail.label}: ${detail.description}`}>!</span></span></label>
  }
