...
function Shell({ me }: { me: Me }) {
  ...
  return <div className={`app-shell stitch-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    <aside>
      ...
      <div className="settings-nav">{item('/settings', 'settings', 'Sistem Ayarları')}<div className="sidebar-utility-links"><a href="mailto:destek@ravencia.com">? <span>Destek</span></a><a href="/openapi/v1.json" target="_blank" rel="noreferrer">▤ <span>Dokümantasyon</span></a></div><button type="button" className="logout-link" onClick={() => void logout()}>{icon('logout')}Çıkış Yap</button></div>
    </aside>
    <main>
      <header className="topbar"><div className="topbar-workspace"><div className="breadcrumb"><strong>Operasyon Merkezi</strong><span>{current}</span></div><form className="global-search" onSubmit={submitGlobalSearch}><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg><input aria-label="Global arama" value={globalSearch} onChange={event => setGlobalSearch(event.target.value)} placeholder="Sipariş, ürün veya müşteri ara..." /><kbd>⌘ K</kbd></form></div><WorkspaceTopActions me={me} onLogout={logout} /></header>
