...
function Shell({ me }: { me: Me }) {
  ...
  const item = (to: string, iconName: string, label: string) => <NavLink to={to} onClick={() => setMobileNavOpen(false)}>{icon(iconName)}<span>{label}</span></NavLink>
  return <div className={`app-shell stitch-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    {mobileNavOpen && <button type="button" className="mobile-nav-backdrop" aria-label="Menüyü kapat" onClick={() => setMobileNavOpen(false)} />}
    <aside className={`app-sidebar ${mobileNavOpen ? 'mobile-open' : ''}`}>
      ...
      <nav aria-label="Ana menü"><span className="nav-section">Genel bakış</span>{item('/dashboard', 'dashboard', 'Dashboard')}<span className="nav-section">Katalog</span>{item('/products', 'products', 'Ürünler')} {item('/catalog/categories', 'mappings', 'Kategoriler')} {item('/catalog/brands', 'platforms', 'Markalar')} {item('/imports', 'products', 'İçe aktarımlar')} {item('/inventory', 'products', 'Stok')}<span className="nav-section">Satış operasyonları</span>{item('/orders', 'orders', 'Siparişler')} {item('/shipments', 'orders', 'Gönderiler')} {item('/returns', 'returns', 'İadeler')} {item('/invoices', 'invoices', 'Faturalar')}<span className="nav-section">Entegrasyonlar</span>{item('/integrations', 'platforms', 'Platformlar')} {item('/mappings/categories', 'mappings', 'Kategori eşleme')}<span className="nav-section">Sistem</span>{item('/jobs', 'jobs', 'İşlem takibi')}</nav>
      <div className="settings-nav">{item('/settings', 'settings', 'Sistem Ayarları')}<button type="button" className="logout-link" onClick={() => void logout()}>{icon('logout')}<span>Çıkış Yap</span></button></div>
    </aside>
    <main>
      <header className="topbar"><button type="button" className="sidebar-mobile-toggle" aria-label={mobileNavOpen ? 'Menüyü kapat' : 'Menüyü aç'} aria-expanded={mobileNavOpen} onClick={() => setMobileNavOpen(value => !value)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d={mobileNavOpen ? 'm6 6 12 12M18 6 6 18' : 'M4 7h16M4 12h16M4 17h16'} /></svg></button><div className="topbar-workspace"><div className="breadcrumb"><strong>Operasyon Merkezi</strong><span>{current}</span></div><form className="global-search" onSubmit={submitGlobalSearch} role="search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6" /><path d="m16 16 4 4" /></svg><input aria-label="Global arama" placeholder="Sipariş, ürün veya SKU ara" value={globalSearch} onChange={event => setGlobalSearch(event.target.value)} /><kbd>⌘ K</kbd></form></div><WorkspaceTopActions me={me} onLogout={logout} /></header>
      <Routes><Route path="/dashboard" element={<Dashboard me={me} />} /><Route path="/products" element={<ProductsPage />} /><Route path="/products/new" element={<NewProductPage />} /><Route path="/products/:id" element={<ProductDetailPage />} /><Route path="/catalog/categories" element={<CategoriesPage />} /><Route path="/catalog/brands" element={<BrandsPage />} /><Route path="/catalog/attributes" element={<AttributesPage />} /><Route path="/imports" element={<ImportsPage />} /><Route path="/imports/:id" element={<ImportDetailPage />} /><Route path="/inventory" element={<InventoryPage />} /><Route path="/integrations" element={<IntegrationsPage />} /><Route path="/integrations/:id" element={<IntegrationDetailPage />} /><Route path="/mappings/categories" element={<MappingPage kind="categories" />} /><Route path="/orders" element={<OrdersPage />} /><Route path="/orders/:id" element={<Navigate to="/orders" replace />} /><Route path="/shipments" element={<ShipmentsPage />} /><Route path="/shipments/:id" element={<ShipmentDetailPage />} /><Route path="/returns" element={<ReturnsPage />} /><Route path="/returns/:id" element={<ReturnDetailPage />} /><Route path="/invoices" element={<InvoicesPage />} /><Route path="/invoices/:id" element={<Navigate to="/orders" replace />} /><Route path="/jobs" element={<JobsPage me={me} />} /><Route path="/settings/billing" element={<Navigate to="/settings" replace />} /><Route path="/settings/security" element={<Navigate to="/settings?tab=security" replace />} /><Route path="/settings" element={<Security />} /><Route path="*" element={<Navigate to="/dashboard" replace />} /></Routes>
    </main>
  </div>
}
...
export function App() {
  ...
  if (me.isLoading) return <Status title="Oturum kontrol ediliyor" />
...
function ChangePassword() { const client = useQueryClient(); const [message, setMessage] = useState('İlk girişte parolanızı değiştirmeniz gerekir.'); async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); try { await api('/change-password', { method: 'POST', body: JSON.stringify({ currentPassword: data.get('current'), newPassword: data.get('next') }) }); await client.invalidateQueries({ queryKey: ['me'] }) } catch { setMessage('Parola değiştirilemedi; politika ve mevcut parolayı kontrol edin.') } } return <main className="auth-page"><section className="auth-card"><div className="auth-card-heading"><span className="auth-mark" aria-hidden="true">R</span><div><p className="auth-eyebrow">HESAP GÜVENLİĞİ</p><h1>Parolanızı değiştirin</h1></div></div><p role="status">{message}</p><form onSubmit={submit}><label htmlFor="current-password">Geçerli parola<input id="current-password" name="current" type="password" autoComplete="current-password" required /></label><label htmlFor="next-password">Yeni parola<input id="next-password" name="next" type="password" minLength={15} maxLength={64} autoComplete="new-password" required /><small>En az 15, en fazla 64 karakter kullanın.</small></label><Button variant="primary" type="submit">Parolayı değiştir</Button></form></section></main> }
function MfaChallenge() { const client = useQueryClient(); const [error, setError] = useState(''); async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); try { await api('/mfa/challenge', { method: 'POST', body: JSON.stringify({ code: data.get('code'), recoveryCode: data.get('recovery') || null }) }); await client.invalidateQueries({ queryKey: ['me'] }) } catch { setError('Kod geçersiz veya daha önce kullanılmış.') } } return <main className="auth-page"><section className="auth-card"><div className="auth-card-heading"><span className="auth-mark" aria-hidden="true">R</span><div><p className="auth-eyebrow">GÜVENLİ OTURUM</p><h1>İki adımlı doğrulama</h1></div></div><p>Authenticator uygulamanızdaki 6 haneli kodu girin.</p><form onSubmit={submit}><label htmlFor="mfa-code">Doğrulama kodu<input id="mfa-code" name="code" inputMode="numeric" pattern="[0-9]{6}" autoComplete="one-time-code" /></label><label htmlFor="mfa-recovery">Kurtarma kodu (alternatif)<input id="mfa-recovery" name="recovery" autoComplete="one-time-code" /></label>{error && <div role="alert" className="error">{error}</div>}<Button variant="primary" type="submit">Doğrula</Button></form></section></main> }
...
function Status({ title, detail }: { title: string; detail?: string }) {
  return <main className="auth-page auth-status-page" role="status"><section className="auth-card auth-status-card"><span className="auth-mark" aria-hidden="true">R</span><p className="auth-eyebrow">RAVENCIA OPERASYON MERKEZİ</p><h1>{title}</h1>{detail && <p>{detail}</p>}<span className="auth-status-loader" aria-hidden="true" /></section></main>
 }
