...
import { defaultShippingLabelSettings, loadShippingLabelSettings, saveShippingLabelSettings, shippingLabelBlockCatalog, shippingLabelFields, type ShippingLabelAlignment, type ShippingLabelBlock, type ShippingLabelBlockKind, type ShippingLabelField, type ShippingLabelSettings } from './shipping-label'
...
function ShippingLabelSettingsPanel({ settings, onChange, onSave }: { settings: ShippingLabelSettings; onChange: (value: ShippingLabelSettings) => void; onSave: () => void }) {
  const [draggedBlock, setDraggedBlock] = useState<{ format: 'a4' | 'sticker'; blockId: string } | null>(null)
  ...
  function removeBlock(format: 'a4' | 'sticker', blockId: string) {
    onChange({ ...settings, layout: { ...settings.layout, [format]: settings.layout[format].filter(block => block.id !== blockId) } })
  }
  ...
  return <div className="panel shipping-settings-panel">
    ...
    <section className="shipping-layout-editor" aria-labelledby="shipping-layout-editor-title">
      <div className="shipping-layout-editor-heading"><div><h3 id="shipping-layout-editor-title">Etiket içerik sırası</h3><p>Blokları sürükleyerek yer değiştirin veya oklarla sıralayın. Her bloğun içeriğini, hizasını ve görünürlüğünü ayrıca düzenleyin.</p></div><span>Değişiklikler “Ayarları kaydet” ile uygulanır.</span></div>
      <div className="shipping-layout-editor-grid">
        {(['a4', 'sticker'] as const).map(format => <fieldset key={format} className="shipping-layout-format"><legend>{formatLabels[format]}</legend><div className="shipping-layout-format-head"><span>{settings.layout[format].length} içerik bloğu</span><small>Sürükle-bırak veya oklarla sırala</small><button type="button" className="secondary" onClick={() => resetLayout(format)}>Varsayılana dön</button></div><div className="shipping-layout-add"><select aria-label={`${formatLabels[format]} blok ekle`} value={addKind} onChange={event => setAddKind(event.target.value as ShippingLabelBlockKind)}>{shippingLabelBlockCatalog.map(block => <option key={block.kind} value={block.kind} disabled={block.kind !== 'custom' && settings.layout[format].some(current => current.kind === block.kind)}>{block.kind === 'custom' ? 'Özel içerik bloğu' : `${block.label}${settings.layout[format].some(current => current.kind === block.kind) ? ' (ekli)' : ''}`}</option>)}</select><button type="button" className="secondary" onClick={() => addBlock(format)}>Blok ekle</button></div>{addKind === 'custom' && <div className="shipping-custom-block-form"><label>Yeni blok başlığı<input value={customTitle} maxLength={120} onChange={event => setCustomTitle(event.target.value)} placeholder="Örn. Mağaza notu" /></label><label>Elle yazılacak içerik<textarea value={customText} maxLength={500} rows={2} onChange={event => setCustomText(event.target.value)} placeholder="Bu blokta görünecek metin" /></label></div>}<div className="shipping-layout-builder"><div className="shipping-layout-list" role="list" aria-label={`${formatLabels[format]} içerik sırası`}>{settings.layout[format].map((block, index) => { const catalog = shippingLabelBlockCatalog.find(option => option.kind === block.kind); const availableFields = shippingLabelFields.filter(field => !block.fields.includes(field.id)); return <article key={`${format}-${block.id}`} className={`shipping-layout-item${draggedBlock?.format === format && draggedBlock.blockId === block.id ? ' is-dragging' : ''}`} draggable onDragStart={() => setDraggedBlock({ format, blockId: block.id })} onDragOver={event => event.preventDefault()} onDrop={() => { if (draggedBlock?.format === format) moveLayoutItem(format, settings.layout[format].findIndex(current => current.id === draggedBlock.blockId), index); setDraggedBlock(null) }} onDragEnd={() => setDraggedBlock(null)} role="listitem"><span className="shipping-layout-drag" aria-hidden="true">⠿</span><span className="shipping-layout-order">{index + 1}</span><div className="shipping-layout-item-main"><div className="shipping-layout-item-title"><label className="shipping-layout-block-title">Blok adı<input value={block.title} maxLength={120} onChange={event => updateBlock(format, block.id, { title: event.target.value })} /></label><small>{catalog?.description}</small></div><label className="shipping-layout-inline-field">Alan ekle<select aria-label={`${block.title} alan ekle`} value="" onChange={event => { if (event.target.value) toggleField(format, block, event.target.value as ShippingLabelField) }}><option value="">Seçin…</option>{availableFields.map(field => <option value={field.id} key={field.id}>{field.label}</option>)}</select></label><div className="shipping-layout-field-chips">{block.fields.length ? block.fields.map(field => <button type="button" className="shipping-layout-field-chip" key={field} onClick={() => toggleField(format, block, field)} title="Bu alanı kaldır">{shippingLabelFields.find(option => option.id === field)?.label} ×</button>) : <span>Alan yok · özel metin kullanılabilir</span>}</div>{block.kind === 'custom' && <label className="shipping-layout-custom-text">Blok metni<textarea value={block.text} maxLength={500} rows={2} onChange={event => updateBlock(format, block.id, { text: event.target.value })} /></label>}</div><label className="shipping-layout-align">Hizalama<select aria-label={`${block.title} hizalama`} value={block.align} onChange={event => updateBlock(format, block.id, { align: event.target.value as ShippingLabelAlignment })}><option value="left">Sol</option><option value="center">Orta</option><option value="right">Sağ</option></select></label><div className="shipping-layout-item-actions"><button type="button" className="icon-button" aria-label={`${formatLabels[format]} ${index + 1}. sırayı yukarı taşı`} disabled={index === 0} onClick={() => shiftLayoutItem(format, index, -1)}>↑</button><button type="button" className="icon-button" aria-label={`${formatLabels[format]} ${index + 1}. sırayı aşağı taşı`} disabled={index === settings.layout[format].length - 1} onClick={() => shiftLayoutItem(format, index, 1)}>↓</button><button type="button" className="icon-button shipping-layout-delete" aria-label={`${block.title} bloğunu kaldır`} onClick={() => removeBlock(format, block.id)}>×</button></div></article> })}</div><ShippingLayoutPreview format={format} layout={settings.layout[format]} /></div></fieldset>)}
      </div>
    </section>
...
function ShippingPreviewBlock({ block }: { block: ShippingLabelBlock }) {
  if (block.kind === 'trackingBarcode' || block.kind === 'packageBarcode') return <div className="shipping-preview-barcode"><span className="shipping-preview-bars" aria-hidden="true" /><strong>{block.kind === 'trackingBarcode' ? '73300036563130080' : '9236253'}</strong></div>
