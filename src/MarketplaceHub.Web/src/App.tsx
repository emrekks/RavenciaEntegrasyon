...
function dashboardRefreshTime(timestamp: Date | null) {
  if (!timestamp) return 'Henüz kontrol edilmedi'
  const seconds = Math.max(0, Math.floor((Date.now() - timestamp.getTime()) / 1_000))
...
function Dashboard({ me }: { me: Me }) {
  ...
  const refreshDashboard = async () => {
    setRefreshing(true)
    try {
      await Promise.all([connections.refetch(), orders.refetch(), returns.refetch(), invoices.refetch(), products.refetch(), jobs.refetch()])
...
function Security() {
  ...
  async function resetOperationalData() {
    ...
    try {
      const result = await hubApi<{ products: number; orders: number; returns: number; invoices: number; categories: number; brands: number; options: number }>('/settings/data-reset', { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ scopes: resetScopes, confirmation: resetConfirmation }) })
      setMessage(`Temizlik tamamlandı: ${result.products} ürün, ${result.categories} kategori, ${result.brands} marka, ${result.options} seçenek, ${result.orders} sipariş, ${result.returns} iade, ${result.invoices} fatura.`)
