...
function Dashboard({ me }: { me: Me }) {
  const [revenueDays, setRevenueDays] = useState<1 | 3 | 7>(7)
  ...
  revenueCutoff.setDate(revenueCutoff.getDate() - (revenueDays - 1))
  const revenueOrders = orderItems.filter(item => new Date(item.orderedAt) >= revenueCutoff && (revenuePlatform === 'ALL' || (item.platformDisplayName || 'Belirtilmemiş') === revenuePlatform))
  const revenueSeries = Array.from({ length: revenueDays }, (_, index) => {
    const date = new Date(revenueCutoff)
    date.setDate(revenueCutoff.getDate() + index)
