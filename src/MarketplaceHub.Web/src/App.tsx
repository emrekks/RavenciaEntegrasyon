...
function IntegrationLines({ pageRef, loginCardRef, nodeRefs }: { pageRef: RefObject<HTMLDivElement | null>; loginCardRef: RefObject<HTMLDivElement | null>; nodeRefs: MutableRefObject<Record<string, HTMLDivElement | null>> }) {
  ...
  return <svg className="rv-lines" aria-hidden="true" viewBox={`0 0 ${window.innerWidth} ${window.innerHeight}`} preserveAspectRatio="none">{paths.map(path => <g key={path.id}><path d={path.d} /><circle className="rv-data-pulse" r="3"><animateMotion dur="5.4s" repeatCount="indefinite" path={path.d} keyPoints="1;0;1" keyTimes="0;0.5;1" calcMode="linear" /></circle></g>)}</svg>
}
