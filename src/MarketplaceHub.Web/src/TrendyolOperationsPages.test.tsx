...
test('retries a local E-Faturam payload rejection and waits for the provider invoice number', async () => {
  ...
  globalThis.fetch = vi.fn((input, init) => {
    ...
    if (url.endsWith('/api/v1/invoices/invoice-1')) return json(submitted
      ? { id: 'invoice-1', version: 7, status: 'SUBMITTED', invoiceNumber: 'RVN2026000000001', lastErrorCode: null }
      : { id: 'invoice-1', version: 5, status: 'REJECTED', invoiceNumber: null, lastErrorCode: 'EFATURAM_FISCAL_PAYLOAD_INVALID' })
    return json({}, 404)
  }) as typeof fetch
