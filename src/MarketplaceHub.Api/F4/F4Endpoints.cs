...
public static class F4Endpoints
{
    public static IEndpointRouteBuilder MapF4Endpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        api.MapPost("/invoices/{id:guid}/stage-capability-probe-jobs", async (Guid id, ConfirmedAction command, HttpContext http, IF4BillingService service, UserManager<ApplicationUser> users) => await EnqueueProtected(id, command, http, users, (tenant, version, key) => service.EnqueueStageCapabilityProbeAsync(tenant, id, version, key, http.TraceIdentifier, http.RequestAborted)));
