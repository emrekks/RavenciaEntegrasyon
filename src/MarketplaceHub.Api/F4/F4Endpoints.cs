...
public static class F4Endpoints
{
    public static IEndpointRouteBuilder MapF4Endpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        api.MapPost("/invoices/{id:guid}/submit-jobs", async (Guid id, ConfirmedAction command, HttpContext http, IF4BillingService service, UserManager<ApplicationUser> users, AppDbContext db) => await EnqueueProtected(id, command, http, users, db, (tenant, version, key) => service.EnqueueSubmitAsync(tenant, id, version, key, http.TraceIdentifier, http.RequestAborted), requireSensitiveConfirmation: false));
