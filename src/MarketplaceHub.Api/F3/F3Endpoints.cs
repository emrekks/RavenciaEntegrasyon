...
public static class F3Endpoints
{
    public static IEndpointRouteBuilder MapF3Endpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        api.MapGet("/connections/{id:guid}/efaturam-settings", async (Guid id, HttpContext http, IF3ConnectionService service) => Tenant(http) is { } tenant ? WithEtag(http, await service.GetEfaturamSettingsAsync(tenant.TenantId, id, http.RequestAborted), x => x.Version) : Unauthorized(http));
