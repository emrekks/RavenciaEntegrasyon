...
public sealed class ApiSurfaceTests
{
    ...
    public void Api_source_keeps_F2_and_adds_only_approved_F3_route_families()
    {
        ...
        foreach (var required in new[] { "/connections", "/efaturam-settings", "/orders", "/shipments", "/returns", "/hooks/{connectionPublicId:guid}/{routeToken}" }) Assert.Contains(required, f3, StringComparison.Ordinal);
    ...
    public void F4_surface_contains_only_invoice_and_billing_families_with_write_guards()
    {
        ...
        foreach (var required in new[] { "/billing/legal-entity-profile", "/billing/invoice-policies/", "/billing/taxpayers/", "/invoices", "/submit-jobs", "/reconcile-jobs", "/marketplace-delivery-jobs", "/cancellation-jobs", "/documents/{documentId:guid}/content" }) Assert.Contains(required, source, StringComparison.Ordinal);
        ...
        foreach (var forbidden in new[] { "/reports", "/accounting", "/erp", "/tenants", "/users", "Shopify", "Hepsiburada", "N11", "Pazarama" }) Assert.DoesNotContain(forbidden, source, StringComparison.OrdinalIgnoreCase);
    }
